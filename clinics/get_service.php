...
if ($_REQUEST['mode'] == "Listing") {
	if (isset($_REQUEST['id']) && isset($_REQUEST['fname'])) {
		...
		$query = "select * from service_master where id like('" . $id . "%') and serv_name like('" . $fname . "%')";
	} else if (isset($_REQUEST['fname'])) {


		$fname = $_REQUEST['fname'];
		$getdata = "select * from service_master where serv_name like('" . $fname . "%')";
		$pdata = mysqli_fetch_assoc(mysqli_query($con, $getdata));
		$id = $pdata['id'];
		$query = "select * from service_master where id = '" . $id . "'";
	} else if (isset($_REQUEST['id'])) {

		$id = $_REQUEST['id'];
		$query = "select * from service_master where id like('" . $id . "%') ";
	} else {
...
<table width="1215" border="1" id="results" cellpadding="4" cellspacing="0">
	...
	<tr>
		...
		<td width="60" style="color:#ac0404; font-size:14px; font-weight:bold;">Service Name</td>
		<td width="30" style="color:#ac0404; font-size:14px; font-weight:bold;">Amount</td>
		<td width="20" style="color:#ac0404; font-size:14px; font-weight:bold;">Edit</td>
		<td width="20" style="color:#ac0404; font-size:14px; font-weight:bold;">Delete</td>
	</tr>
	...
	?>
			...
			<tr>
				...
				<td> <a href='edit_service.php?id=<?php echo $row[0]; ?>'> Edit </a></td>
