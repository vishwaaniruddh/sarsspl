...
<body>
    <?php $Q="select name,email,mobile  from clients where code='".$_GET["mid"]."'";
            $ret = mysql_query($Q);
...
{
    //alert(<?php echo $_GET["pr"];?> + '  ');
    $.ajax({
        /*url: 'paymentMethod.php',*/
        url: 'payumoney/pay.php?mid=<?php echo $_GET["mid"]; ?>&month=<?php echo $mont?>&price=<?php echo $_GET["pr"]?>&name=<?php echo $row['name']?>&email=<?php echo $row['email']?>&mobile=<?php echo $row['mobile']; ?>',
        //url: 'payumoney/pay.php',
        dataType: 'html',
        //type: 'POST',
        //data: 'month='+<?php echo $mont?>+'&price='+<?php echo $_GET["pr"]?>+'&name='+"<?php echo $row['name']?>"+'&email='+"<?php echo $row['email']?>", 
