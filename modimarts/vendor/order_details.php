                                ...
                                <div class="row">
                                    <div id="content" class="col-sm-9" style="left: 5%;">
                                        ...
                                        <table class="table table-bordered table-hover" style="background-color: white;">
                                            ...
                                            $query2 = mysqli_query($con1,"SELECT qty,item_id,oid,product_name FROM order_details WHERE oid ='".$oid1."'  OR  mrc_id='".$merchantid."'");
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-right">
                                                                <select name='status' id='status' onchange="order_status(<?php echo $rows6['id'];?>);">
                                                                    ...
                                                                    <option id='5' value='5' <?php if($rows6['status']==5){echo 'selected';} ?>>reject</option>
                                                                </select>
                                                            </td>
                                                            <input type="hidden" name="tracking_id" value="tracking_id" value="" >
...
function track_id(id) {
    //alert(id.value);
    var trackid = id.value;
    var status = document.getElementById("status").value;
    // var trackid1 = document.getElementById('trackid').value;
    // document.getElementById('tracking_id').value = trackid;
    change_status(<?php echo $_GET['id'] ;?>,status,trackid);
    //$('#myModal').hide();
    $('#myModal').modal('hide');
}
...
function change_status(oid,status,track_id=0)
{
    alert(oid+' : '+status+' : '+ track_id);
    $.ajax({
        type: "POST",
        url: "change_status.php",
		//datatype:'json',	
        data:'status='+status+"&oid="+oid+"&track_id="+track_id,
