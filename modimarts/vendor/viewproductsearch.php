...
if($catid==1){
    ...
    $View = "select * from fashion where  status = 1 and ccode='$id' and code='$catprodid' ";
}
else if($catid==190){
    if($catprodid=="")
    $View = "select * from electronics where status = 1 and ccode='$id' ";
    else
    $View = "select * from electronics where status = 1 and ccode='$id' and code='$catprodid' ";
}
else if($catid==218){
    if($catprodid=="")
    $View = "select * from grocery where status = 1 and ccode='$id' ";
    else
    $View = "select * from grocery where status = 1 and ccode='$id' and code='$catprodid' ";
}
else if($catid==482){
    if($catprodid=="")
    $View = "select * from Resale where status = 1 and ccode='$id' ";
    else
    $View = "select * from Resale where status = 1 and ccode='$id' and code='$catprodid' ";
}
else{
    if($catprodid=="")
    $View = "select * from products where status = 1 and ccode='$id' ";
    else
    $View = "select * from products where status = 1 and ccode='$id' and code='$catprodid' ";
}
    ...
    while($res=mysqli_fetch_array($qrys)) 
    {
        ...
        <tr height="50" border="0" ><td width="20%" align="center" border="0" >  
            ...
            */
            ...
            ?>
            <img src="../ecom/<?php echo $sltingf["img"]; ?>" alt="image" height="50" ><br><?php if($paudio){ ?><a href="play.php?fil=<?php echo '../ecom/userfiles/'.$id."/audio/".$paudio; ?>" >Play Music</a><?php } if($pvideo){ ?><br><a href="play.php?fil=<?php echo '../ecom/userfiles/'.$id."/audio/".$pvideo; ?>" >Play Video</a><?php } ?></td>
