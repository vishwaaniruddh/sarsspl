            ...
            <td align="center" colspan="2"><b><font color="#000000" size="+1">YOUR PRODUCT DETAILS</font></b> </td>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <div id="res">
<?php 
$clintct=mysql_query("SELECT cid,subscribe,email FROM `clients` WHERE `code` = '".$id."'");
...
?>
            ...
            <select id="Maincat" name="Maincat" class="form form-full" onchange="chkcategory();setSubCategory2('Maincat','Maincategory','newSubCat2');getBrands('Maincat',this.value);" required>
                <option value="">Select Category</option>
                <optgroup label="Add New Category">
                <option value="AddNewCategory">Create Category</option></optgroup>
                <?php 
  ...
  <tr id="hd_NewCategory1" Style="display:none">
    ...
    <tr>
      <td>Select Brand</td>
      <td>
        <select name="pbrand"  id="pbrand" onchange="showDiv(this,'brand'),getProducts('pbrand');">
              <option value="">Select</option>
                <? /*php $brand=mysql_query("SELECT * FROM `brand`");
                    while($fetchBrand=mysql_fetch_array($brand)){
                ?>
                <option value="<?php echo $fetchBrand['id'];?>"><?php echo $fetchBrand['brand'];?></option>
                <?php  } */?>
        ...
        <div id="showbrand" style="display:none;">
           New Brand <input type="text" name="new_brand" id="brand"/>
        </div>
        </td>
    </tr>
 <!-- </div>-->
 <tr>
    <td>Select Product</td>
        ...
        <select name="pname"  id="pname" onchange="showDiv(this,'product');">
            <option value="">Select</option>
                <?php /*$prod=mysql_query("SELECT * FROM `product_model` where status=1");
                    while($fetchProd=mysql_fetch_array($prod)){
                ?>
       ...
       $.ajax({
            ...
            success: function(msg){
                ...
                for(i=0;i<jsr[0]['image'].length;i++){
                     img+='<img src="http://sarmicrosystems.in/oc1/'+jsr[0]['image'][1]+'" width="17%">';
                }
                $('#pimage').append(img);
...
<table class="tables"  align="center" cellpadding="4" cellspacing="0" id="attatch1">
    <tr>
        <td ><div align="left">Current Price</div></td>
        <td ><input type="text" name="price" id="price" class="form form-full" onchange="a()" onclick="subscrip();" required/></td>
        <td >
          <div align="left">Additional Price</div>
        </td>
        <td  ><input type="text" name="new_price" id="new_price" class="form form-full" onchange="a()" /></td>
    </tr>
    <script>
        function a(){
