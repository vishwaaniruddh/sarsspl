...
<div id="ordershow">
             ...
             <div id="show"></div>
              ...
              <table class="table-responsive table-striped table table-bordered" id="example">
                <thead>
                  <tr>
                    <th>Order Id</th>
                    <th>Customer</th>
                ...
                <tbody>
                  ...
                  $order_sql  = mysqli_query($con1, "select * from order_details where mrc_id='" . $id . "' AND status='1' order by id DESC");
        while ($ord_sql_result = mysqli_fetch_assoc($order_sql)) {

          $udata=mysqli_query($con1,"SELECT * FROM `new_order` WHERE oid='".$ord_sql_result['oid']."' ");
          $usdata=mysqli_fetch_assoc($udata);

         $orderpro= mysqli_query($con1, "select * from order_details where mrc_id='" . $id . "' AND oid ='".$ord_sql_result['oid']."'");
         $itemid=$ord_sql_result['id'];
         $shipping_status=$ord_sql_result['shipping_status'];
         $prototal=0;
        while ($prodata=mysqli_fetch_assoc($orderpro)) {
         
         $tamount=$prodata['total_amt'];
        $prototal=$prototal+$tamount;
         
        }


                    $username = $usdata['name'];
                    $address = $usdata['address'];
                    $city = $usdata['city'];
                    $pincode = $usdata['zip'];
                    $state = $usdata['state'];
...
$odata=mysqli_query($con1,"SELECT * FROM `Order_ent` WHERE id='".$ord_sql_result['oid']."' ");
          $ordata=mysqli_fetch_assoc($odata);

                                            if($ordata['status']==0)
                                            {
                                                $p="Pending";
                                            } else if($ordata['status']==1)
                                            {
                                                $p="Accepted";
                                            }
                                            else if($ordata['status']==2)
                                            {
                                                $p="Processing";
                                            }
                                            else if($ordata['status']==3)
                                            {
                                                $p="Dispatch";
                                            }
                                            else if($ordata['status']==4)
                                            {
                                                $p="compltete";
                                            }
                                            else if($ordata['status']==5)
                                            {
                                                $p="Reject";
                                            }
                                            else {
                   ...
                   ?>
                   <tr>
                     <td>txn-<?=$ord_sql_result['oid']?></td>
                     <td><?=$username?></td>
                     <td><?=$primary_address?></td>
                     <td><?=$ord_sql_result['product_name']?></td>
                     <td><?=$prototal?></td>
                     <td><?=date('D,d-m-Y',strtotime($ordata['date']))?></td>
                     <td><a href="#" class="btn btn-primary">VIew</a></td>
                     <td><a href="#" class="btn btn-primary"><?=$p?></a></td>
                     <td><?php if($shipping_status==0){ ?><a class="btn btn-primary" onclick="Chengedelivary(<?=$itemid?>)"><p id="shipping_status">No</p></a><?php }else{
                      ?>
                      <a href="#" class="btn btn-primary" >Track Product</a>
                      <?php
                     } ?></td>
                   </tr>
                 <?php } ?>
      ...
      <script>
        function Chengedelivary(oid)
        {

          $.ajax({
             type: "POST",
             url: "changeshippingstatus.php",
