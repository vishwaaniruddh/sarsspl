...
if (!is_dir("../ecom/".$dirtLrgsize)) 
{
    if(!mkdir("../ecom/".$dirtLrgsize,0777, true))
    {
        $error = error_get_last();
        $rr="Make dir error".$error['message'];
        $errormsg_arr[]=$rr;
...
if(isset($_POST['shipping_in_area']))
{
    $shipping_in_area = $_POST['shipping_in_area']; 
} 

if(isset($_POST['shipping_out_state']))
{
    $shipping_out_state = $_POST['shipping_out_state']; // Displaying Selected Value
}
...
if($_POST['pname']=='otherproduct') {
   // $prname=mysqli_real_escape_string($_POST['new_product']);
    //echo $prname;
    /*$brand_qry=mysqli_query($con1,"INSERT INTO `product_model`(`product_model`, `brand_id`) VALUES ('$prname','$pbrand')");*/
    $qry_pmodel=mysqli_query($con1,"INSERT INTO `product_model`(`product_model`, `description`, `price`, `others`,discount,discount_type,brand_id,category_id,Long_desc,offer_price,allmart_commission,`related_grp_id`,`weight`,`type`) VALUES ('$prname','$p_other','$price','$others','$discntamt','$dicnttyp','$pbrand','$pcat','$Longdesc','$ttlprs','$allmart_commission','$related_grp_id','".$weight."','".$type."')");
...
if (!$copied) 
{
  ...
  $errors=1;
    ...
    if(!resize("../ecom/".$imgpath,"../ecom/".$dirtLrgsize.$image_name, 1000, 1000))
    {
        $error = error_get_last();
        $rr="Resize midsize error".$error['message'];
...
if($_POST['Maincat']=="AddNewCategory") {
    ...
    if($aa!=0){
        ...
        $Maincate= $rowa1[4];
        $p_type=mysqli_real_escape_string($con1,$rowa1[1]);
    ...
    if($Maincate==1){
        ...
        $sql = "INSERT INTO `fashion`(`name`, `ccode`, `category`, `photo`,`price`,total_amt,thumbs,midsize,size,color,brand,size_id,product_type,keyword1,offer_price,allmart_commission,shipping_in_area,shipping_out_state,is_provide_shipping,is_shipping) VALUES ('$pname','$ccode','$pcat','$dirs','$price','$ttlprs','','','$hidden_size','$hidden_color','$pbrand','$hidden_sizeid','$p_type','$key','$ttlprs','$allmart_commission','$shipping_in_area', '$shipping_out_state','".$is_provide_shipping."','".$is_shipping."')";
        ...
        $sql = "INSERT INTO `electronics`(`name`, `ccode`, `category`, `photo`, `price`,total_amt,thumbs,midsize,size,color,brand,size_id,product_type,keyword1,offer_price,allmart_commission,is_provide_shipping,is_shipping) VALUES ('$pname','$ccode','$pcat','$dirs','$price','$ttlprs','','','$hidden_size','$hidden_color','$pbrand','$hidden_sizeid','$p_type','$key','$ttlprs','$allmart_commission','".$is_provide_shipping."','".$is_shipping."')";
        ...
        $sql = "INSERT INTO `grocery`(`name`, `ccode`, `category`, `photo`, `price`,total_amt,thumbs,midsize,size,color,brand,size_id,product_type,keyword1,offer_price,allmart_commission,is_provide_shipping,is_shipping) VALUES ('$pname','$ccode','$pcat','$dirs','$price','$ttlprs','','','$hidden_size','$hidden_color','$pbrand','$hidden_sizeid','$p_type','$key','$ttlprs','$allmart_commission','".$is_provide_shipping."','".$is_shipping."')";
        ...
        $sql = "INSERT INTO `promotion_product`(`name`, `ccode`, `category`, `photo`, `price`,total_amt,thumbs,midsize,size,color,brand,size_id,product_type,keyword1,offer_price,allmart_commission,is_provide_shipping,is_shipping) VALUES ('$pname','$ccode','$pcat','$dirs','$price','$ttlprs','','','$hidden_size','$hidden_color','$pbrand','$hidden_sizeid','$p_type','$key','$ttlprs','$allmart_commission','".$is_provide_shipping."','".$is_shipping."')";
        $qry=mysqli_query($con1,"INSERT INTO `promotion_product`(`name`, `ccode`, `category`, `photo`, `price`,total_amt,thumbs,midsize,size,color,brand,size_id,product_type,keyword1,offer_price,allmart_commission,shipping_in_area,shipping_out_state,is_provide_shipping,is_shipping) VALUES ('$pname','$ccode','$pcat','$dirs','$price','$ttlprs','','','$hidden_size','$hidden_color','$pbrand','$hidden_sizeid','$p_type','$key','$ttlprs','$allmart_commission','$shipping_in_area', '$shipping_out_state','".$is_provide_shipping."','".$is_shipping."')");
        ...
        if (is_array($spfc))
        {
            for($i=0;$i<count($spfc);$i++)
            {  
                $qry1=mysqli_query($con1,"INSERT INTO `promotion_product_Specification`(`product_id`, `product_specification`,`specificationname`,category) VALUES('".$insid."','".$_POST['specification1'][$i]."','".$_POST['specification'][$i]."','".$pcat."')");
            }
        }
        
        for($a=0;$a<count($image_name3);$a++)
        {
            $sql2="INSERT INTO `promotion_product_img`(`product_id`, `img`,thumbs,midsize,category,largeSize) values('".$insid."','".$image_name3[$a]."','".$image_name4[$a]."','".$image_name5[$a]."','".$pcat."','".$image_name6[$a]."')";
            $result2 = mysqli_query($con1,$sql2);
            if(!$result2)
            {
                $errormsg_arr[]=mysqli_error($con1);
        ...
        $key = $pbrand." ".$p_type;
        $sql = "INSERT INTO `products`(`name`, `ccode`, `category`, `photo`, `price`,total_amt,thumbs,midsize,size,color,brand,size_id,product_type,keyword1,offer_price,allmart_commission,is_provide_shipping,is_shipping) VALUES ('$pname','$ccode','$pcat','$dirs','$price','$ttlprs','','','$hidden_size','$hidden_color','$pbrand','$hidden_sizeid','$p_type','$key','$ttlprs','$allmart_commission','".$is_provide_shipping."','".$is_shipping."')";
    ...
    if(isset($_FILES['video']['name']) && $_FILES['video']['name'] != ''){
        ...
        add_video($name,$size,$tmp,$videodir,$insid,$con1);
    }
...
<?php } else {  
    ...
    for($a=0;$a<count($image_name3);$a++)
    {
        if (file_exists("../ecom/".$image_name3[$a])) 
        {
            unlink("../ecom/".$image_name3[$a]);
        }
        if (file_exists("../ecom/".$image_name4[$a])) 
        {
            unlink("../ecom/".$image_name4[$a]);
        }
        if (file_exists("../ecom/".$image_name5[$a])) 
        {
            unlink("../".$image_name5[$a]);
        }
        if (file_exists("../ecom/".$image_name6[$a])) 
        {
            unlink("../ecom/".$image_name6[$a]);
        }
    }
