...
if($Maincate==1)
{
    $qry2=mysqli_query($con1,"select f.*,p.product_model,p.brand_id,p.description as pdescription,p.price as pprice,p.others as pothers,p.discount as pdiscount,p.discount_type as pdiscount_type,p.long_desc as pLong_desc,p.status as pstatus from product_model p join fashion f on p.id=f.name where p.status=1 and p.id='".$pid."'");
...
else if($Maincate==190)
{
    ...
    $qry2=mysqli_query($con1,"select e.*,p.product_model,p.brand_id,p.description as pdescription,p.price as pprice,p.others as pothers,p.discount as pdiscount,p.discount_type as pdiscount_type,p.long_desc as pLong_desc,p.status as pstatus from product_model p join electronics e on p.id=e.name where p.status=1 and  p.id='".$pid."'");
...
else if($Maincate==218)
{
    ...
    $qry2=mysqli_query($con1,"select g.*,p.product_model,p.brand_id,p.description as pdescription,p.price as pprice,p.others as pothers,p.discount as pdiscount,p.discount_type as pdiscount_type,p.long_desc as pLong_desc,p.status as pstatus from product_model p join grocery g on p.id=g.name where p.status=1 and  p.id='".$pid."'");
...
else 
{
    ...
    $qry2=mysqli_query($con1,"select g.*,p.product_model,p.brand_id,p.description as pdescription,p.price as pprice,p.others as pothers,p.discount as pdiscount,p.discount_type as pdiscount_type,p.Long_desc as plong_desc,p.status as pstatus from product_model p join products g on p.id=g.name where p.status=1 and  p.id='".$pid."'");
