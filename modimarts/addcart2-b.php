...
$dt = date("Y-m-d");
 ...
 $getprocc = getrecentproduct($usrid);
 $numersofpro=count($getprocc);
 $adprocccode=getmerchantcode($cid,$prodid);
 ...
 if (in_array('588', $getprocc)) { $Allow=1; } else { $Allow=0; }
}
else
{
  $Allow=1  ;
...
else
{
 ...
 if (in_array('588', $getprocc)) { $Allow=0; } else { $Allow=1; }
}
else
{
  $Allow=1  ;
