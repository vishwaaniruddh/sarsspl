...
function AddCartCall($userid,$product_id,$size,$color,$qty)
{
    $curl = curl_init();
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://thebrandtadka.com/api/index.php?mod=ApiMobile&api_key=VarifyTADKA7563&company_id=448%20&action=addToCart&token=02c276c93ce8508fc578476b99ab4c80&product_id='.$product_id.'&member_id=1094462&size_id='.$size.'&color_id='.$color.'&qty='.$qty,
...
function APICheckout($userid,$parameters)
{
    ...
    $postdata='action=CheckoutEwallet&api_key=VarifyTADKA7563&token=02c276c93ce8508fc578476b99ab4c80&company_id=448&member_id=1094462&'.$parameters;
$curl = curl_init();
curl_setopt_array($curl, array(
  CURLOPT_URL => $url,
  CURLOPT_RETURNTRANSFER => true,
    ...
    if (!mysqli_num_rows($select_sql)) {
        ...
        foreach ($crtids as $key => $val) {
            ...
            if($outside_product)
            {
                ...
                $res        = AddCartCall('1094218',$pid,$size,$color,$qty);
                array_push($resdata,$res);
                $totalpro=$totalpro+$qty;
                    // }
            }
