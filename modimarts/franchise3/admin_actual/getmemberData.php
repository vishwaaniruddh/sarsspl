...
function total_qualify($id)
{
    ...
    return $qualify;
        ...
        {
        $search_param = "%{$id}%";
        $query=  "created_at like '".$search_param."'";
        }
        else
        {
            $query= $type."=".$id; 
...
$sql = mysqli_query($con, "select * from new_member where  $query AND level_id<>'Null' AND level_id<>'0' order by level_id ASC ");
...
while ($sql_result = mysqli_fetch_assoc($sql)) {
        ...
        if ($status==0) {
            $statusA="<a class='btn btn-danger btn-sm'>Disactive</a>";
        }
