...
$catid         = $_GET['catid'];
   $catid    = mysqli_real_escape_string($con1, $catid);
$total_records = 0;
...
$sqlbrdcr = mysqli_query($con1, "select * from main_cat where id ='" . $catid . "'");
      ...
      <script type="text/javascript">
            ...
            function displayRecords(lim, off,filter=0) {
                ...
                $.ajax({
                  ...
                  success: function(html) {
                    ...
                    if(total==0)
                     {
                      $("#loader_message").html('<button class="btn btn-default" type="button">coming soon.</button>').show()
                    }
                     else if (offset >=total)
                    {
                      ...
                      $("#loader_message").html('<button class="btn btn-default" onclick="getmoredata()" type="button">Show More Details</button>').show();
                    }
                    window.busy = false;
                  }
                });
            }
            function getfirstdata(lim, off,filter=0) {
                var total_record = document.getElementById('total_record').value;
                $.ajax({
                  type: "GET",
                  async: false,
            ...
            $(document).ready(function() {
                ...
                if (busy == false) {
                    busy = true;
                    // start to load the first set of data
                    getfirstdata(limit, offset,filter);
                }
                ...
                $.ajax({
                  ...
                  success: function(html) {
                    ...
                    if (offset >=total) {
                      ...
                      $("#loader_message").html('<button class="btn btn-default" onclick="getmoredata()" type="button">Show More Records</button>').show();
                    }
                    window.busy = false;
                  }
                });
