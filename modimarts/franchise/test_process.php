...
<form  method="post" action="member.php" style="padding:0 !important;margin:-19px !important;min-width: 360px !important;">
    	        ...
    	        <tr>
                	...
                	<td class="" style="text-align:center">&nbsp;&nbsp;
                	  <?php while($sql2fetch=mysqli_fetch_array($runsql2)){ if($sql2fetch['file']!=''){ ?>
                	        <img src="<?php echo $sql2fetch['file']?>" width="90px" />
                	  <?php  } } ?>
                	</td>
    ...
    <div id="waiting_list" class="tab-pane fade">
      <!--<h3>Waiting List</h3>-->
    	...
    	<table align="center" class=" table-bordered table-striped">
    		...
    		//$srn=1;
    		$cn=0;
    		//$query = "select m.name,m.id,m.file,c.dasignation_name from committee_structure c left join member m on m.position_id=c.id and  m.loc_id=".$loc_id." and m.level_id=".$level_id." and m.status=0";
            //$result = mysqli_query($conn,$query);
            //var_dump($table);
			...
			{ 
               // $sql_final_committee="select id,name,file from member where position_id=".$_row['id']." and level_id= ".$level_id." and loc_id=".$loc_id." and verify='Y' and status=1";
            	...
            	$runsql_waiting=mysqli_query($conn,$sql_waiting_committee);
            	$runnum_waiting=mysqli_num_rows($runsql_waiting);
            	//var_dump($sql_waiting_committee);
               echo '<tr>';
               //echo '<td class="th-prop">'. $srn.'</td>';
               echo '<td>'.$_row['id'].'</td>';
               ?>
               ...
               $srn=1;
    			while($data = mysqli_fetch_array($runsql_waiting))
    			{
                  ...
                  ?>
            	  <tr>
            	    <td class="th-prop" width="50px"><?php echo $srn; ?></td>
