...
if (($_POST['mem_id']) != '' || ($_POST['mem_name']) != '' || ($_POST['mem_mobile']) != '') {
    ...
    if ($mem_id) {
        $query = "select * from greetings_member where id = '" . $mem_id . "' and status='1' ";
    } else if ($mem_name) {
        $query = "select * from greetings_member where  name like '" . $mem_name . "' and status='1' ";
    } else if ($mem_mobile) {
        $query = "select * from greetings_member where mobile like '" . $mem_mobile . "' and status='1'";
    }
    ...
    if ($sql_result = mysqli_fetch_assoc($sql)) {
        ...
        $position      = $sql_result['position_name'];
        ...
        $image_sql        = mysqli_query($con, "select * from greetings_member_images where member_id='" . $id . "' and type='passport'");
        $image_sql_result = mysqli_fetch_assoc($image_sql);
        $image            = $image_sql_result['image'];
        $image            = str_replace("https://www.", "https://", $image);

        $data = ['id' => $id, 'star' => $star, 'position' => $position, 'position_name' => $position_name, 'mobile' => $mobile, 'name' => $name, 'status' => $status, 'image' => $image, 'total_intro' => $total_intro, 'active_intro' => $active_intro, 'qualify' => $qualify, 'days_over' => $days_over, 'bread' => $bread, 'commission' => $commission, 'created_at' => $from_date,'country'=>$country,'zone'=>$zone,'state'=>$state,'division'=>$division,'district'=>$district,'taluka'=>$taluka,'pincode'=>$pincode,'village'=>$village];
    ...
    $sql = mysqli_query($con, "select * from greetings_member where country='" . $country . "'and zone='" . $zone . "' and state='" . $state . "' and division='" . $division . "' and district='" . $district . "' and taluka='" . $taluka . "' and pincode='" . $pincode . "' and village='" . $village . "' and status='1' ");
