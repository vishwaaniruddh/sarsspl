...
$sql_statement = 'SELECT * FROM Productviewtable ' .
    ...
    ' `allmart_ecommerce`.`Approval_products`.`keyword1` AS `keyword1` from `allmart_ecommerce`.`Approval_products` where 1 ';
...
if ($_REQUEST['proname'] != "") {
    $sql_statement .= " and  ccode='" . $_REQUEST['proname'] . "' ";
}
...
<input type="text" name="proname" action="" >
...
//    ==============================================================
    ...
    if ($Maincate == 1) {
        $slting = mysqli_query($con1, "SELECT * FROM `fashion_img` where product_id='" . $row[0] . "' limit 0,1");
    } else if ($Maincate == 190) {
        $slting = mysqli_query($con1, "SELECT * FROM `electronics_img` where product_id='" . $row[0] . "' limit 0,1");
    } else if ($Maincate == 218) {
        $slting = mysqli_query($con1, "SELECT * FROM `grocery_img` where product_id='" . $row[0] . "' limit 0,1");
    } else if ($Maincate == 482) {
        $slting = mysqli_query($con1, "SELECT * FROM `Resale_img` where product_id='" . $row[0] . "' limit 0,1");
    } else {
        $slting = mysqli_query($con1, "SELECT * FROM `product_img` where product_id='" . $row[0] . "' limit 0,1");
    }
...
//ruchi Get product name by id
    $prod            = mysqli_query($con1, "SELECT * FROM product_model where id='" . $row['name'] . "'");
    $product_details = mysqli_fetch_assoc($prod);
//ruchi Get brand name by id
    $brand      = mysqli_query($con1, "SELECT brand FROM brand where id='" . $row['brand'] . "'");
    ...
    ?>
        ...
        ?>
                <tr class="alternate-row">
                    ...
                    <td>
                        <?php
$cat = mysqli_query($con1, "select name from main_cat where id='" . $row['category'] . "'");
        ...
        if ($nrs > 0) {
            ...
            $f      = false;
            $newcat = mysqli_query($con1, "select name from approval_category where product_id='" . $row['code'] . "'");
