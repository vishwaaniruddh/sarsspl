...
if (isset($_POST['sendOrderToShipRocket'])) {
	...
	$query= mysqli_query($con1,"INSERT INTO `order_shipping`(`oid`, `item_id`, `channel_id`, `shipment_id`, `order_ids`, `sendOrderShipRocket`, `sendOrderToShipRocket`,`pro_ids`) VALUES ('".$oid."','".$item_id."','".$channel_id."','".$shipment_id."','".$order_ids."','".$responce1."','".$responce2."','".$ids."')");
	...
	if($query)
	{
		 ...
		 for ($i=0; $i <count($getids) ; $i++) { 
		 	$show_email_sql = mysqli_query($con1,"UPDATE order_details SET `track_Status`='1',track_id='".$order_ids."'  where id='".$getids[$i]."' ");
		 }
...
if (isset($_POST['generate_awb'])) {
	...
	$query= mysqli_query($con1,"UPDATE `order_shipping` SET `awb_code`='".$awb_code."',`generate_awb`='".$responce2."' WHERE order_ids='".$order_ids."'");
...
if (isset($_POST['generate_pickup'])) {
	...
	$query= mysqli_query($con1,"UPDATE `order_shipping` SET `generate_pickup`='".$responce2."' WHERE order_ids='".$order_ids."' AND shipment_id='".$shipment_id."'");
...
if (isset($_POST['generateManifest'])) {
	...
	$query= mysqli_query($con1,"UPDATE `order_shipping` SET `generateManifest`='".$responce2."' WHERE order_ids='".$order_ids."' AND shipment_id='".$shipment_id."'");

	if($query)
	{
		$getdata=mysqli_query($con1,"SELECT order_ids FROM order_shipping WHERE order_ids='".$order_ids."' AND shipment_id='".$shipment_id."' ");
...
if (isset($_POST['generateLabel'])) {
	...
	$query= mysqli_query($con1,"UPDATE `order_shipping` SET `generateLabel`='".$responce2."' WHERE order_ids='".$order_ids."' AND shipment_id='".$shipment_id."'");
...
if (isset($_POST['CheckCourier'])) {
	...
	$responce2 = $_REQUEST['responce2'];
             ...
             $query= mysqli_query($con1,"UPDATE `order_shipping` SET `courier_list`='".$responce2."' WHERE order_ids='".$order_ids."'");
	if($query)
	{ echo "1";	}
	else
	{echo "0";  }
