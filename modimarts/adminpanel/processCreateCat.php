...
if($basecat!="")
{
	$sqlbrdcr = mysqli_query($con1,"select * from main_cat where id ='".$basecat."' order by name");
	...
	}else
	{
	    ...
	    while($exs==0)
	    {
	        //echo "select * from main_cat where id ='".$iddbr."'";
	       	$sqlbrdcr2 = mysqli_query($con1,"select * from main_cat where id ='".$iddbr."' order by name");
		...
		{
	    	$sqlbrdcr23 = mysqli_query($con1,"select * from main_cat where id ='".$idbrdcrmbarr[$c]."' order by name");
...
$chkqr=mysqli_query($con1,"select * from main_cat where name='".$cname."' and under='".$catid."'");
//echo "select * from main_cat where name='".$cname."' and under='".$catid."'";
$chknrws=mysqli_num_rows($chkqr);

if($chknrws==0 )
{
	$qry="insert into main_cat(name,under,cat_img,base_cat,status) values('".mysqli_real_escape_string($con1,$cname)."','".$catid."','".$thumb_src."','".$strr."',1)";
	$res=mysqli_query($con1,$qry);
    $base=mysqli_insert_id($con1);

    $curr_dt=date('Y-m-d H:i:s');
	$subAdminWork=mysqli_query($con1,"insert into audit_log (user_id,action,description,date_time,srid,line_no,table_name)values('".$_SESSION['SESS_USER_NAME']."','Add','Add Category In Category Table','".$curr_dt."','".$_SESSION['lastSubID']."','". $base." ','main_cat') ");
		
    $chk=mysqli_query($con1,"select * from main_cat where id='".$base."'");
    $fetchchk=mysqli_fetch_array($chk);

    if($fetchchk['under']==0){
        $updateBasecat=mysqli_query($con1,"update main_cat set base_cat='".$base."' where id='".$base."'");
    }
