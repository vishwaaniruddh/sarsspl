...
//echo $Maincate;
if($Maincate==1){	
	 $slting=mysql_query("SELECT * FROM `fashion` where  code='".$_GET['pcode']."'");
	 //echo "SELECT * FROM `fashion` where  code='".$_GET['pcode']."'";
	 $pimg=mysql_query("SELECT * FROM `fashion_img` where product_id='".$_GET['pcode']."'");
	 $pspecification=mysql_query("SELECT * FROM `fashionSpecification` where product_id='".$_GET['pcode']."'");
}
else if($Maincate==190)
{	
     $slting=mysql_query("SELECT * FROM `electronics` where  code='".$_GET['pcode']."'");
     //echo "SELECT * FROM `electronics` where name='".$_GET['pcode']."'";
     $pimg=mysql_query("SELECT * FROM `electronics_img` where product_id='".$_GET['pcode']."'");
	 $pspecification=mysql_query("SELECT * FROM `electronicsSpecification` where product_id='".$_GET['pcode']."'");
}
else if($Maincate==218)
{	
     $slting=mysql_query("SELECT * FROM `grocery` where code='".$_GET['pcode']."'");
     //echo "SELECT * FROM `grocery` where code='".$_GET['pcode']."'";
     $pimg=mysql_query("SELECT * FROM `grocery_img` where product_id='".$_GET['pcode']."'");
	 $pspecification=mysql_query("SELECT * FROM `grocerySpecification` where product_id='".$_GET['pcode']."'");
} 
else if($Maincate==482)
{	
     $slting=mysql_query("SELECT * FROM `Resale` where  code='".$_GET['pcode']."'");
     //echo "SELECT * FROM `Resale` where status=0 and code='".$_GET['pcode']."'";
     //$pimg=mysql_query("SELECT * FROM `fashion_img` where product_id='".$_GET['pid']."'");
	 //$pspecification=mysql_query("SELECT * FROM `fashionSpecification` where product_id='".$_GET['pid']."'");
} 
else
{	
     $slting=mysql_query("SELECT * FROM `products` where code='".$_GET['pcode']."'");
     //echo "SELECT * FROM `products` where status=0 and code='".$_GET['pcode']."'";
     $pimg=mysql_query("SELECT * FROM `product_img` where product_id='".$_GET['pcode']."'");
	 $pspecification=mysql_query("SELECT * FROM `productspecification` where product_id='".$_GET['pcode']."'");
}

$result=mysql_fetch_assoc($slting);
//var_dump($result);

if(isset($_GET['pid']) && $_GET['pid']>0){
    $pid=$_GET['pid'];
    $qry =mysql_query("SELECT * FROM product_model where id = '".$pid."'");
    $pdetails = mysql_fetch_assoc($qry);
    $pname=$pdetails['name'];
    //var_dump($pdetails);echo '<br>new : ';
    
    /*$qry1 =mysql_query("SELECT * FROM product_additional_details where product_id = '".$pid."'");*/
    $qry1 =mysql_query("SELECT * FROM product_additional_details where product_id = '".$_GET['pcode']."'");
                        ...
                        <div class="tab-pane fade show active in" id="home" role="tabpanel" aria-labelledby="home-tab">
                            ...
                            <div class="row register-form">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group display-prop">
                                        ...
                                        <select name="brand" class="selectpicker form-control">
                                          <?php 
                                           //get category
                                          $qry_br =mysql_query("SELECT* FROM brand where status!=2");
                                          while($row=mysql_fetch_assoc($qry_br)){ ?>
                                    ...
                                    <div class="form-group display-prop">
                                        ...
                                        <select name="product" class="selectpicker form-control">
                                            <?php 
                                            //get category
                                            $qry_pr =mysql_query("SELECT* FROM product_model where status!=2");
                                            while($row=mysql_fetch_assoc($qry_pr)){ ?>
                                    ...
                                    <div class="form-group display-prop">
                                        <label >Image</label>&nbsp;
                                        <?php while($img=mysql_fetch_assoc($pimg)){?>
                                    ...
                                    <div class="form-group display-prop">
                                        ...
                                        <ol>
                                            <?php 
                                        	    while($row = mysql_fetch_assoc($pspecification)) {  ?>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group display-prop">
                                        <?php 
                                            $qry_brand =mysql_query("SELECT* FROM brand where status!=2 and id=".$_GET['bid']);
                                            //echo "SELECT* FROM brand where status!=2 and id=".$_GET['bid'];
                                            $br=mysql_fetch_assoc($qry_brand);
                                            //var_dump($br);
                                            if($result1['brand_id']){
                                        ?>
                                        ...
                                        <select name="brand1" class="selectpicker form-control">
                                          <?php 
                                           //get category
                                          $qry_brands =mysql_query("SELECT* FROM brand where status!=2");
                                          while($rows=mysql_fetch_assoc($qry_brands)){ ?>
                                     ...
                                     <div class="form-group display-prop">
                                         <?php 
                                            $qry_product =mysql_query("SELECT* FROM product_model where status!=2 and id=".$result['name']);
                                            //echo "SELECT* FROM product_model where status!=2 and id=".$result['name'];
                                            $prod=mysql_fetch_assoc($qry_product);
                                        ...
                                        <?php 
                                        //get category
                                        $qry_prod =mysql_query("SELECT* FROM product_model where status!=2");
                                        while($row=mysql_fetch_assoc($qry_prod)){ ?>
