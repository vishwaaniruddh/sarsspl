...
<!--
    ...
    <script
      ...
      src="/assets/header.js?v=10373096426439681222"
...
if (isset($_POST['txnid'])) {
  ...
  if(isset($_POST['ProformaBill'])){ 
    $update=mysqli_query($con1,"UPDATE `proforma_Order_ent` SET `converted`='1' AND `deleted_at`='".$date."' WHERE id='".$_POST['ProformaBill']."'");
   ...
   $userid = $_SESSION['SESS_USER_NAME'];  
    ...
    if (!mysqli_num_rows($select_sql)) {
        ...
        for ($i = 0; $i < $procount; $i++) {
            ...
            $discountP = get_percentage($mrp, $product_amt);
            ...
            if ($outside_product) {
                ...
                AddProductSale($pid, $product_name, $cat_id, '1508', $product_amt, $qty, $HSN, $sale_id, $gst, $mrp, $discountP, $totalAmount, $is_franchise);
                // for ($c = 0; $c < $qty; $c++) {
            ...
            if ($totalpro > 0) {
                ...
                $updatedata = mysqli_query($con1, "UPDATE `Order_ent` SET `other_res`='" . $other_res . "', `ord_resquest`='".$parameter."' WHERE `id`='" . $order . "'");
        ...
        curl_setopt($ch, CURLOPT_URL, "/invoice/invoice.php?order_id=" . $order);
...
function AddCartCall($userid, $product_id, $size, $color,$qty)
{
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL            => 'https://thebrandtadka.com/api/index.php?mod=ApiMobile&api_key=VarifyTADKA7563&company_id=448&action=addToCart&token=02c276c93ce8508fc578476b99ab4c80&product_id=' . $product_id . '&member_id=1094462&size_id=' . $size . '&color_id=' . $color.'&qty='.$qty,
