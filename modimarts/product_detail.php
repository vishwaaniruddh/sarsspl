...
if ($cid == 80) {
    ...
    if ($Maincate == 1) {
        $qrylatf = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt` ,`franchise_price`, `color` ,`maxqty`,`minqty`,`size`,Long_desc,shipping_in_area,shipping_out_state,`maxqty`,`minqty`,is_provide_shipping FROM `fashion` WHERE code='" . $pid . "'";
    } else if ($Maincate == 190) {
        $qrylatf = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt`,`franchise_price`, `color` ,`size`,`maxqty`,`minqty`,Long_desc,shipping_in_area,shipping_out_state,is_provide_shipping FROM `electronics` WHERE code='" . $pid . "'";
    } else if ($Maincate == 218) {
        $qrylatf = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt`, `color` ,`size`,`maxqty`,`franchise_price`,`minqty`,Long_desc,shipping_in_area,shipping_out_state,is_provide_shipping FROM `grocery` WHERE code='" . $pid . "'";
    } else if ($Maincate == 760) {
        $qrylatf = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt`, `color` ,`maxqty`,`minqty`,`size`,Long_desc,shipping_in_area,shipping_out_state,is_provide_shipping FROM `kits` WHERE code='" . $pid . "'";
    }
    ...
    $qrylatf="SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt`, `color` ,`size`,Long_desc,shipping_in_area,shipping_out_state FROM `services` WHERE code='".$pid."'";
        ...
        $qrylatf = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt`, `color` ,`size`,`maxqty`,`minqty`,Long_desc,shipping_in_area,shipping_out_state,is_provide_shipping FROM `promotion_product` WHERE code='" . $pid . "'";
    } else {
        $qrylatf = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt`, `color` ,`size`,`maxqty`,`franchise_price`,`minqty`,Long_desc,shipping_in_area,shipping_out_state,is_provide_shipping FROM `products` WHERE code='" . $pid . "'";
    }
}
...
$prod         = mysqli_query($con1, "SELECT product_model,status,is_online FROM product_model where id='" . $latstprnrws['name'] . "' ");
...
$prod_id    = mysqli_real_escape_string($con1, $_GET['prod_id']);
    ...
    if ($categogy == '761') {
        ...
        $mrp   = get_kit_info($cust_pid, 'total_amt');
        ...
        $mrp   = $latstprnrws['total_amt'];
        $amount2=$latstprnrws['franchise_price'];
        if($_SESSION['mem_id']!='' && $amount2!=0){ $amount   = $amount2 ; }
        $pro_name = $product_name['product_model'];
        $status = $product_name['status'];
        $is_online = $product_name['is_online'];
        $pro_img  = '/ecom/' . $frtu[0];
    ...
    $discount=round((1 - ($amount / $mrp)) * 100,0);
    ...
    // $prolink=$baseurl."product_detail.php?pid=".$cust_pid."&catid=".$categogy."&prod_id=".$prod_id."&prodnm=".$new;
    ...
    $msg= $prolink;
            ...
            $getreview=mysqli_query($con1,"SELECT * FROM `product_review` WHERE status='1' AND product_id='".$pid."'");
            $countreview=mysqli_num_rows($getreview);
            foreach ($getreview as $key => $rat) {
                $a[]=$rat['rating_count'];
            }
                              ...
                              >
                                ...
                                <div class="product_single_price" id="pricebox" style="display: block;">
                                  ...
                                  <div class="product_price">
                                    <div class="grid-link__org_price"
                                      id="ProductPrice"><span>Rs <?php echo $amount; ?></span>
                                    </div>
                                        ...
                                        if ($weight>0) {
                                            ...
                                            <div class="swatch" data-option-index="0" style="display: block;">
                                                ...
                                                <div data-value="<?=$prspcf['product_specification']?>" class="swatch-element 1-kg available">
                                                    ...
                                                    <label for="<?=$prspcf['product_id']?>-<?=$prspcf['id']?>-<?=$prspcf['product_specification']?>"><?=$weight?>-<?=$type?></label>
                                                </div>
                                  ...
                                  <div class="product-single__quantity" style="display: block;">
                                    ...
                                    <div class="shipping-charges" style="float: left;width: 100%;font-weight: bold;margin-top: 15px;display: block;">
                                     <?php  if($prod_id!=1513){ ?><label>Shipping Charges : </label><?php } else { ?> <label>GST Charges : </label><?php }?>
                                        <span style="font-size: 18px;color: #ff6c26;">
                                            <?php
                                            if($is_provide_shipping!=2){
                                             if($prod_id==1513){
                                              ?>
                                                 <b>It's inclusive of 12% GST</b> 
                                            <?php 
                                           }else {
                                ...
                                <?php if($prod_id==1513){?>
                                        ...
                                        <script type="text/javascript">
                                            $(document).ready(function() {
                                                $('#btnShow').click(function(){
                                                    $("#dialog").show();
                                                    $("#frame").attr("src", "/assets/doc/Greetingspdf.pdf");
                                                }); 
                                            });
                                        </script>
                                      <label>Pdf : </label>
                                      <span id="certificates" style="font-size: 18px;color: #ff6c26;">
                                          <a href="/assets/doc/Greetingspdf.pdf" target="_blank">Show Pdf</a>
                                      </span>
                                    ...
                                    <div style="width: 100%">
                                      ...
                                      <div id="lbcontent"> <a href="/ecom/userfiles/safetycard/Certificates-MySafetyCard.pdf" target="_blank"><small style="font-size: 12px;">(Haffkine, Bombay Test House, Patent, Euro Cert - ISO, FDA, CE, GMP)</small></a></div>
                                    </div>
                                   ...
                                   <div class="product_button_section">
                                    ...
                                    <button
                                      ...
                                      if($status==1)
                                      {
                                        if($is_online){
                                        ?>
                                      onclick="add_to_card('<? echo $categogy;?>','<? echo  $cust_pid;?>','<? echo $amount; ?>','<? echo $pro_img;?>','<? echo urlencode($pro_name);?>','<? echo  $prod_id;?>','<?php echo $shipping; ?>','<?php echo $shipping_charges; ?>','<?=$mrp?>','<?=$discount?>')"
                                      <?php 
                                      }
                                      else { ?>
                                        disabled
...
<div class="spr-container">
  ...
  <div class="spr-content">
    ...
    <div class="spr-form" id="reviewform" style="display:none;">
        <form method="post" action="/addreviewprocess.php" class="new-review-form" enctype= "multipart/form-data" >
...
foreach ($getreview as $key => $review) {
   ...
   ?>
    ...
    <div class="spr-review" id="spr-review-105032506">
      ...
      ?>
  <div class="column">  
    <img src="/<?=trim($images[$i])?>" style="width:80px" onclick="openModal();currentSlide(1)" class="hover-shadow cursor">
  </div>
<?php }} ?>
...
function resetRating() {
  ...
  */
      ...
      <script>
        var notyf = new Notyf();
        function add_to_card(cid,prodid,price,image,pname,pid,shipping,shipping_charges,mrp,discount)
        {
        try
        {
   ...
   // alert(quntity);
        ...
        type: 'POST',
        url:'/addcart2.php',
        data:'prodid='+prodid+'&cid='+cid+'&price='+price+'&image='+image+'&pname='+pname+'&pid='+pid+'&shipping='+shipping+'&shipping_charges='+shipping_charges+'&quantity='+quntity+'&specifiid='+specifiid+'&mrp='+mrp+'&discount='+discount,
        ...
        function buy_now(cid,prodid,price,image,pname,pid,shipping,shipping_charges,mrp,discount)
        {
            try
            {
                var proid=$("#selectedid").val();
