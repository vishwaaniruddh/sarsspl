...
// $sql = mysqli_query($con,"select * from new_pincode where pincode ='".$pin."'");
// $sql_result = mysqli_fetch_assoc($sql);
// $taluka = $sql_result['taluka'];
// $pin_id = $sql_result['id'];

$talula_sql = mysqli_query($con,"select * from new_taluka where taluka = '".$taluka."' and status=1");
$talula_sql_result = mysqli_fetch_assoc($talula_sql);
$tal_id = $talula_sql_result['id'];
...
$data = ['zone'=>$zone,'state'=>$state,'division'=>$division,'district'=>$district,'taluka'=>$taluka,'taluka_id'=>$tal_id];
