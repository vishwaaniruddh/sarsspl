...
if($check_sql_result = mysqli_fetch_assoc($check_sql)){
    ...
    $update = "update cart set qty = '".$new_quantity."',p_price = '".$price."', total_amt = '".$total_amount."', final_amt = '".$total_amount."' ,outside_product='".$outside_product."',mrp='".$mrp."',discount='".$discount."'  where user_id = '".$usrid."' and pid='".$prodid."' and cat_id='".$cid."' AND variant_id='".$specifiid."'";
    ...
    $total_amount  = $qty * $price; 
      ...
      $insert = "INSERT INTO `cart`(`user_id`,guest_id ,`pid`, `qty`, `dt`,p_price,total_amt,final_amt,color,size,cat_id,status,product_name,image,prodid,shipping_in_area,shipping_out_state,variant_id,outside_product,gst,hsn,mrp,discount) VALUES ('".$usrid."','".$usrid."','".$prodid."','".$qty."','".$dt."','".$price."','".$total_amount."','".$total_amount."','".$color."','".$size."','".$cid."','0','".$pname."','".$image."','".$pid."','".$shipping_in_area."','".$shipping_out_state."','".$specifiid."','".$outside_product."','".$gst."','".$hsn."','".$mrp."','".$discount."')";
