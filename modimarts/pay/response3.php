...
if($status=='success'){
    ...
    if(!$select_sql_result && $userid>0) { //to prevent duplicate entries
     ...
     $insert="insert into Order_ent(user_id,date,amount,status,pmode,pdfpath,cmplt_status,transaction_stats,transaction_id,cartids_inorder,shipping_charges) values('".$userid."','".$date."','".$amount."','0','online','','','".$status."','".$txnid."','".$json_cart."','".$shipping_charges."')";
