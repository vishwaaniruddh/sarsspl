...
if($Maincate==1) {
    ...
    $View="select p.* from (select pm.product_model,pm.offer_price,pm.allmart_commission,pm.category_id,pd.code,pd.price,pd.total_amt,pd.name from product_model as pm join electronics as pd on pm.id=pd.name where pd.name!='$pid' AND pm.category_id in ($id,'".$catid."') and pm.status=1  order by pd.price LIMIT $limit OFFSET $offset ) as p group by p.product_model order by p.offer_price $orderby";
    $getipuaevts1 =mysqli_query($con1,$View);
}
else if($Maincate==218) {

    $View="select p.* from (select pm.product_model,pm.offer_price,pm.allmart_commission,pm.category_id,pd.code,pd.price,pd.total_amt,pd.name from product_model as pm join grocery as pd on pm.id=pd.name where pd.code!='$pid' AND pm.category_id in ($id,'".$catid."') and pm.status=1  order by pd.price LIMIT $limit OFFSET $offset ) as p group by p.product_model order by p.offer_price $orderby";
    $getipuaevts1 =mysqli_query($con1,$View);
...
/*else if($Maincate==757) {
    ...
    $View="select p.* from (select pm.product_model,pm.offer_price,pm.allmart_commission,pm.category_id,pd.code,pd.price,pd.total_amt,pd.name from product_model as pm join products as pd on pm.id=pd.name where pd.code!='$pid' AND pm.category_id in ($id,'".$catid."') and pm.status=1  order by pd.price LIMIT $limit OFFSET $offset ) as p group by p.product_model order by p.offer_price $orderby";
    $getipuaevts1 =mysqli_query($con1,$View);
}
...
<div class="products product-hover-11">
  ...
  <div class="product-container">
    ...
    <a href="https://allmart.world/<?=strurl($name)?>/P/<?=strcode($pid)?>/<?=strcode($catid)?>/<?=strcode($prod_id)?>" class="grid-link">
      ...
      <a href="https://allmart.world/<?=strurl($name)?>/P/<?=strcode($pid)?>/<?=strcode($catid)?>/<?=strcode($prod_id)?>" class="grid-link">
        <img src="<?=$pro_img?>" title="<?=$name?>" class="featured-image" alt="<?=$name?>" style="min-height: 191px;" />
      </a>
    </a>
  ...
  <div class="product-detail" title="<?=$name?>">
    ...
    <a href="https://allmart.world/<?=strurl($name)?>/P/<?=strcode($pid)?>/<?=strcode($catid)?>/<?=strcode($prod_id)?>" style="height: 3em;" title="<?=$name?>" class="grid-link__title"><?=substr($name, 0, 30)?></a>
    <div class="grid-link__meta">
      ...
      onclick="addtocart('<?=$catid;?>','<?=$pid?>','<?=$price?>','<?=$pro_img?>','<?=$name;?>','<?=$prod_id;?>')"
      title="Add to Cart">
  ...
  $releprod = mysqli_query($con1,"SELECT * FROM `related_group_products` WHERE product_status='1' AND group_id='".$relepro['related_grp_id']."' AND pro_id<>'$pid'");
    while($querycatf=mysqli_fetch_array($releprod))
      { 
                ...
                $price=getproductprice($catid,$pid);
                $pro_img=$querycatf['pro_img'];
...
<div class="products product-hover-11">
  ...
  <div class="product-detail" title="<?=$name?>">
    ...
    <a href="https://allmart.world/<?=strurl($name)?>/P/<?=strcode($pid)?>/<?=strcode($catid)?>/<?=strcode($prod_id)?>" style="height: 3em;" title="<?=$name?>" class="grid-link__title"><?=substr($name, 0, 25)?></a>
    <div class="grid-link__meta">
      <div class="product_price">
        <div class="grid-link__org_price" id="ProductPrice">
          Rs. <?=$price?>
