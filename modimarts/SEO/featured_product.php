...
$proopt = mysqli_query($con1, "SELECT * FROM `products_extra_option` WHERE is_featured ='1' AND product_status='1'");
...
<div class="products product-hover-11">
  ...
  <div class="product-container" style="max-height: 213px;">
    ...
    <a href="https://allmart.world/<?=strurl($name)?>/P/<?=strcode($pid)?>/<?=strcode($catid)?>/<?=strcode($prod_id)?>" class="grid-link">
      ...
      <a href="https://allmart.world/<?=strurl($name)?>/P/<?=strcode($pid)?>/<?=strcode($catid)?>/<?=strcode($prod_id)?>" class="grid-link">
        <img src="<?=$pro_img?>" title="<?=$name?>" class="featured-image" alt="<?=$name?>" style="width: 213px;height: 213px;" />
      </a>
    </a>
