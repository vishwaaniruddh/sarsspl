...
$limit = (intval($_REQUEST['limit']) != 0 ) ? $_REQUEST['limit'] : 10;
...
$count = $_REQUEST['total_record'];
/*while($data = mysqli_fetch_assoc($sql)){
    echo "<h4>". $data['product_name']."</h4>";
}*/
$total_records = mysqli_num_rows($getipuaevts1);
    ...
    <li class="grid__item item-row wide--one-fifth post-large--one-fifth large--one-third medium--one-half small--one-half"
                            ...
                            >
                              ...
                              <div class="products product-hover-11">
                                ...
                                <? if($_REQUEST['catid']==761){ ?>

                                <a href="product_detail.php?pid=<?php echo $rws2['code'];?>&catid=761&prod_id=<?php echo $rws2['name'];?>&prodnm=<?=$productname?>">

                              <? } else { ?>

                                <a href="product_detail.php?pid=<?php echo $rws2['code'];?>&catid=<?php echo $rws2['category_id'];?>&prod_id=<?php echo $rws2['name'];?>&prodnm=<?=$productname?>">
         ...
         <!-- <a href="javascript:void(0)" title="Quick View" id="mushrooms" class="quickview-button quick-view-text product_link" data-view="mushrooms"><i class="fa fa-search"></i></a>        -->
                                ...
                                <div class="product-detail">
                                  ...
                                  ?>
                                  <form action="product_detail1.php" method="post" id="prodet">
                                    <input type="hidden" name="pid" value="<?=$rws2['code']?>">
                                    <input type="hidden" name="catid" value="<?=$rws2['category_id']?>">
                                    <input type="hidden" name="prod_id" value="<?=$rws2['name']?>">
                                    <input type="hidden" name="prodnm" value="<?=$productname?>">
                                   <a href="#" title="<?=$prod_name?>" onclick="$('#prodet').submit();">
