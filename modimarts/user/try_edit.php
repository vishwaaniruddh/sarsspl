...
$cmp=$_GET['cmp'];
$sql = mysqli_query($con1,"select * from clients where code='".$cmp."'");

    $row = mysqli_fetch_array($sql); 
        
        $sql1 = mysqli_query($con1,"select * from Bank_Details where Merchant_id='".$row['code']."'");
...
function disfn1(st){
 ...
 <!-- <ul class="breadcrumb">
    ...
    <form action="processEditCustomer.php" method="post" enctype="multipart/form-data" id="loginForm"  class="form-horizontal" autocomplete="OFF" onsubmit="return finalval();">
...
<?php } ?>
        ...
        <input type="hidden" name="companycat" id="companycat">
              
               <input class="inp-form" type="hidden" name="code" size="50" value="<?php echo $row[0]; ?>" readonly/>
...
<?php } 
$citydets=mysqli_query($con1,"select * from cities where code='".$row[2]."'");
$citydetsrow=mysqli_fetch_array($citydets);


$areadets=mysqli_query($con1,"select * from cities where code='".$row[2]."'");
$areadetsrw=mysqli_fetch_array($citydets);
...
<input type="text" id="contactPerson"  name="contactPerson" class="form-control" class="inp-form" value="<?php echo $row['contact']; ?>" placeholder="Enter character only"  required autofocus/>
                          </div>
          </div> 
...
$catarr=explode(",",$row['cid']);
  ...
  <!--
     <input type="checkbox" onclick="catselected1();" id="c" name="cat[]" value="<?php echo $rows[0]; ?>" <?php if(in_array($rows[0], $catarr)) {echo "checked"; }  ?>    <?php if(in_array($rows[0], $catarr)) { echo "disabled";} ?>  /> <?php echo $rows[1]; ?><br/>
