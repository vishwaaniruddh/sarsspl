...
function refferal_txn($franchise_id,$userid,$refferral_code=null,$amount,$order_id)
{
    global $con;

    if($franchise_id!=''){

    $addtxn=mysqli_query($con,"INSERT INTO `franchise_referral_txn`(`user_id`, `franchise_id`, `refferral_code`, `amount`, `order_id`) VALUES ('".$userid."','".$franchise_id."','0','".$amount."','".$order_id."')");
}
}
...
function getfranchise($client_id)
{
    ...
    $pincode = $row['pincode'];
       ...
       $sql1 = mysqli_query($con,"SELECT * FROM new_pincode WHERE pincode = '".$pincode."' and status=1");
       $row1 = mysqli_fetch_assoc($sql1);
       $id = $row1['id'];
            // $pincode = $row['pincode'];

    $getfra        = mysqli_query($con, "SELECT * FROM `new_member` WHERE pincode='" . $id . "' AND  country='" . $country . "' AND status=1");
...
function commission($txn_id, $commision, $mem, $date, $promotion,$reffid,$order_id,$comm_id)
{

    global $con;
    global $con1;

    if($comm_id){ $notgive=$userid;}else{ $notgive=0; }
    ...
    if ($village > 0) {
        $vil_sql        = mysqli_query($con, "select * from new_member where village='" . $village . "' and id<>'".$reffid."' and id<>'".$notgive."' and status=1");
    ...
    if ($pincode > 0) {
        ...
        $pin_sql        = mysqli_query($con, "select * from new_member where pincode='" . $pincode . "' and id<>'".$reffid."' and id<>'".$notgive."' and village=0 and status=1");
    ...
    // return;
    if ($taluka > 0) {
        $tal_sql        = mysqli_query($con, "select * from new_member where taluka='" . $taluka . "' and id<>'".$reffid."' and id<>'".$notgive."' and pincode=0 and status=1");
    ...
    if ($district > 0) {
        $dis_sql        = mysqli_query($con, "select * from new_member where district='" . $district . "' and id<>'".$reffid."' and id<>'".$notgive."' and taluka=0 and status=1");
    ...
    if ($division > 0) {
        $div_sql        = mysqli_query($con, "select * from new_member where division='" . $division . "' and id<>'".$reffid."' and id<>'".$notgive."' and district=0 and status=1");
    ...
    if ($state > 0) {
        ...
        $state_sql        = mysqli_query($con, "select * from new_member where state='" . $state . "' and id<>'".$reffid."' and id<>'".$notgive."' and division=0 and status=1");
    ...
    if ($zone > 0) {
        $zone_sql        = mysqli_query($con, "select * from new_member where zone='" . $zone . "' and id<>'".$reffid."' and id<>'".$notgive."' and state=0 and status=1");
    ...
    if ($country > 0) {
        $country_sql        = mysqli_query($con, "select * from new_member where country='" . $country . "' and id<>'".$reffid."' and id<>'".$notgive."' and zone=0 and status=1");
...
{
    global $con1;
    $sql = mysqli_query($con1, "select SUM(qty) as ttlqty from cart where user_id='" . $userid . "' and status=0 AND outside_product='1'");
    $sql_result = mysqli_fetch_assoc($sql);
        $cart_quantity = $sql_result['ttlqty'];
 
    return $cart_quantity;
}

function GetttlAmount($userid)
{
    global $con1;
    $sql = mysqli_query($con1, "select SUM(final_amt) as ttlamt from cart where user_id='" . $userid . "' and status=0 AND outside_product='1'");
    $sql_result = mysqli_fetch_assoc($sql);
        $cart_quantity = $sql_result['ttlamt'];
 
    return $cart_quantity;
}

function getoutsidepro($user_id)
{
    global $con1;
    $sql = mysqli_query($con1, "select * from cart where user_id='" . $user_id . "' and status=0 AND outside_product='1'");
    $ttlqty=0;
    while ($sql_result = mysqli_fetch_assoc($sql)) {
        $cart_quantity = $sql_result['qty'];
        $ttlqty=$ttlqty+$cart_quantity;
    }
    return $ttlqty;
}
...
if ($order_status === "Success") {
    ...
    if (!mysqli_num_rows($select_sql)) {
        ...
        $json_cart = implode(",", $arr);
         foreach ($crtids as $key => $cval) {
           $is_franchise         = get_cart_info($cval, 'is_franchise');
         }
          $ttlout_product=0;
         foreach ($crtids as $key => $cval) {
          $out_product      = get_cart_info($cval, 'outside_product');
          $ttlout_product=$ttlout_product+$out_product;
         }



        $insert    = "insert into Order_ent(user_id,date,amount,status,pmode,pdfpath,cmplt_status,transaction_stats,transaction_id,cartids_inorder,shipping_charges,ord_response,Notes,is_franchise) values('" . $userid . "','" . $date . "','" . $amount . "','0','online','','','" . $order_status . "','" . $txnid . "','" . $json_cart . "','" . $shoppingcharge . "','".$ord_response."','".$Notes."','".$is_franchise."')";
        ...
        if ($ttlout_product)
        {
         $ttq=Gettotalcart($userid);
         $ttamount=GetttlAmount($userid);
          $purchase_id=addPurchase($userid,$ttq,$ttamount,$order);
          $sale_id=addsale($userid,$ttq,$ttamount,$order,$is_franchise);

        //   echo "Before";
        // var_dump($ttlout_product);
        // echo "<br/>";var_dump($purchase_id);
        // echo "<br/>";var_dump($sale_id);
        ...
        foreach ($crtids as $key => $val) {
            ...
            $mrc_id               = getccode($cat_id,$pid );
            $outside_product      = get_cart_info($val, 'outside_product'); 
            $prodata              = getproductprice($cat_id, $pid);
            if($outside_product){
            $HSN                  = get_cart_info($val, 'hsn');
            $gst                  = get_cart_info($val, 'gst');
            }
            else
            {
              $HSN                  = $prodata['HSN'];
            ...
            $mrp                 = get_cart_info($val, 'mrp'); 
            $discount            = get_cart_info($val, 'discount'); 
            $is_franchise        = get_cart_info($val, 'is_franchise'); 

            $size                 = get_cart_info($val, 'size'); 
            $color                = get_cart_info($val, 'color'); 
            $totalout             = getoutsidepro($userid);
            ...
            $insert_order_details = "insert into order_details(oid,item_id,rate,qty,discount,rejected_qty,status,total_amt,mrc_id,cat_id,track_id,track_Status,color,size,date,product_name,image,HSN,gst,cart_res,outside_product,mrp) values ('" . $order . "','" . $item_id . "','" . $product_amt . "','" . $qty . "','".$discount."','','1','" . $product_amt * $qty . "','".$mrc_id ."','".$cat_id."','','','','','" . $date . "','" . $product_name . "','" . $product_image . "','".$HSN."','".$gst."','".$resdata."','".$outside_product."','".$mrp."')";
...
<td align="center" valign="top"><table border="0" cellpadding="10" cellspacing="0" width="600" id="m_3716871841364271743template_footer"><tbody><tr><td valign="top" style="padding:0;border-radius:6px"><table border="0" cellpadding="10" cellspacing="0" width="100%"><tbody><tr><td colspan="2" valign="middle" id="m_3716871841364271743credit" style="border-radius:6px;border:0;color:#8a8a8a;font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;font-size:12px;line-height:150%;text-align:center;padding:24px 0">                </td></tr></tbody></table></td></tr></tbody></table></td></tr></tbody></table><div class="yj6qo"></div><div class="adL">
        ...
        if ($email != '') {
            ...
            $smtmail->setFrom('noreply@allmart.world', 'Allmart Ecommerce LLP ');
            ...
            $smtmail->addBCC('enquiry.allmart@gmail.com'); 
...
//  if(!$smtmail->Send()) {
        ...
        while ($show_order_sql_result1 = mysqli_fetch_assoc($show_email_sql)) {
            ...
            $total_amt1    = $single_price1 * $_prod_qty;
            ...
            if (!empty($sql_result1)) {
                ...
                $allmart_commission = (($single_price1/100)*$sql_result1['allmart_commission']);
            ...
            if($refid!='' && $refid!=0)
            {
                ...
                $amount = round($allmart_commission / 2, 5);
                refferal_txn($refid,$userid,'',$amount,$order_id);
            }   
            $neworderid=   $order_id."-".$promotion;  

               
            // commission($txn_id, $allmart_commission, $franchise_id, $_orderdate, $promotion,$refid,$neworderid,$is_franchise);
        }
    }
