...
$amt=$qtty * $prdrws['price']; 
$name=$qtty * $prdrws['product_model'];  
$date = date('Y-m-d H:i:s');

$cartdets=mysqli_query($con1,"select * from cart where user_id='".$_GET['gid']."' and status=0");
$nrwss1=mysqli_num_rows($cartdets);
if($nrwss1>0)
{
    $cartdets1=mysqli_query($con1,"select sum(qty) from cart where user_id='".$_GET['gid']."' and status=0");
...
//===================code for Round Robin (End)============================== 
 ...
 {
$qrycartsumamt=mysqli_query($con1,"select sum(total_amt) from cart where user_id='".$id."' and status=0 and id in($cartids)");
...
$fetchorder=mysqli_insert_id($con1);
...
$qryorderdetails=mysqli_query($con1,"INSERT INTO `order_details`(`oid`, `item_id`, `rate`, `qty`, `discount`,total_amt,mrc_id,cat_id,track_id,status,color,size,date) VALUES ('".$fetchorder."','".$prdid."','".$pric."','".$qtty."','".$prdrws[8]."','".$pric."','".$client_fetch['code']."','".$cate_id."','".$a."','pending','".$cCod."','".$cSize."','".$date."')");
...
$qrycart=mysqli_query($con1,"select * from cart where user_id='".$id."' and status=0 and id in($cartids)");
...
$qryorderdetails=mysqli_query($con1,"INSERT INTO `order_details`(`oid`, `item_id`, `rate`, `qty`, `discount`,total_amt,mrc_id,cat_id,track_id,status,color,size) VALUES ('".$fetchorder."','".$fetchcart[2]."','".$fetchcart[6]."','".$fetchcart[3]."','0','".$fetchcart[10]."','".$fetchp[2]."','".$c."','".$a."','pending','".$fetchcart['color']."','".$fetchcart['size']."')");
if(!$qryorderdetails)
{
  $errs++;  
}
...
$updtcrt=mysqli_query($con1,"update cart set status='1',order_id='".$fetchorder."' where user_id='".$id."' and status=0 and id in($cartids)");
if(!$updtcrt)
{
  $errs++;  
}
...
<!-- BOLT Production/Live //-->
 <script id="bolt" src="https://checkout-static.citruspay.com/bolt/run/bolt.min.js" bolt-color="e34524" bolt-logo="http://boltiswatching.com/wp-content/uploads/2015/09/Bolt-Logo-e14421724859591.png"></script>
   ...
   <style type="text/css">
	...
	<form action="#" id="payment_form" class="form-horizontal" >
    <!--<input type="hidden" id="udf5" name="udf5" value="BOLT_KIT_PHP7" />-->
    <input type="hidden" id="udf5" name="udf5" value="<?php echo $fetchorder; ?>" />
    <input type="hidden" id="surl" name="surl" value="<?php echo getCallbackUrl(); ?>" /> 
    <div class="dv hide"> 
    <span class="text"><label>Merchant Key:</label></span>
    <span><input type="text" id="key" name="key" placeholder="Merchant Key" value="usA7CkHP" /></span>
    </div>
    <!--BhA3IR5V-->
    
    <div class="dv hide">
    <span class="text"><label>Merchant Salt:</label></span>
    <span><input type="text" id="salt" name="salt" placeholder="Merchant Salt" value="qjR3FURgMf" /></span>
    </div>
    <!--ViHyr9bb8t-->
    <div class="dv hide">
    <span class="text"><label>Transaction/Order ID:</label></span>
    <span><input typtexte="text" id="txnid" name="txnid" placeholder="Transaction ID" value="<?php echo  "Txn" . rand(10000,99999999)?>" readonly/></span>
    </div>
    
    <!--<div class="dv">
    <span class="text control-label col-sm-2" for="email"><label>Amount:</label></span>
    <span ><input type="text" id="amount" name="amount" placeholder="Amount" value="1"  readonly/></span>    
    </div>
    -->
    <div class="dv hide">
    <span class="text"><label>Product Info:</label></span>
    <span><input type="text" id="pinfo" name="pinfo" placeholder="Product Info" value="<?php echo $name;?>" readonly/></span>
    </div>
    
    <!--<div class="dv">
    <span class="text control-label col-sm-2" for="email"><label>First Name:</label></span>
    <span><input type="text" id="fname" name="fname" placeholder="First Name" value="<?php echo $udata['Firstname']; ?>" readonly/></span>
    </div>
    -->
    <!--<div class="dv">
    <span class="text control-label col-sm-2" for="email"><label>Email ID:</label></span>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="amount" name="amount"  value="1" readonly>
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="fname" name="fname"  value="<?php echo $udata['Firstname']; ?>" >
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="email" name="email" placeholder="Email ID" value="<?php echo $udata['email']; ?>" readonlyid="email"  name="email">
      </div>
    </div>
    ...
    <div>
        ...
        <span>
        <a href="index.php"><input type="button" value="Cancel"  class="btn btn-warning" /></a>
        </span>
        </div>
	</form>
</div>
