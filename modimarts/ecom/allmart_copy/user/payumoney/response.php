...
if (isset($_POST ['key'])) {
    ...
    if($pinfo[0]=='sb') {
    	$qry =mysql_query("SELECT s.id as sid,s.period,s.type,sd.rate,sd.discount,sd.grace_type,sd.grace_period FROM subscriptions s join subscription_details sd on s.id=sd.subscription_id where sd.status!=2 and sd.id = ".$pinfo[1]);
	...
	if ($status == 'success' ) {
        $subscribe_done=mysql_query("update `clients` set subscribe ='Active' WHERE `code` = ".$pinfo[2]);
        if($pinfo[0]=='sb') {
    		...
    		$result = mysql_query($sub_details);
    		//echo $sub_details;
    	} else if($pinfo[0]=='sl'){
    		...
    		$result = mysql_query($ofr_details);
    	} else if($pinfo[0]=='ad') {
