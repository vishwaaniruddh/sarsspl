...
?>        
      ...
      <div class="data grid-12">
        ...
        <div class="grid-12">
          <div class="widget">
            <header class="widget-header">
              ...
              <h3 class="widget-header-title"><strong>WelCome To  User Panel</strong></h3>
            </header>
   	...
   	<table class="tables" align="center" cellpadding="4" cellspacing="0">
		<?php $prtrakqry=mysqli_query($con1,"select * from product_track where cid='".$cid."' order by srno DESC LIMIT 0,10"); 
				$numpg=mysqli_num_rows($prtrakqry);$i=1;
				if($numpg>0){
		?>
            <tbody><tr>
			<th>Sr no</th><th>PRODUCT NAME</th><th>IP ADDRESS</th><th>BROWSER</th><th>TIMESTAMP</th></tr>
            <?php 
			  while($prtrak=mysqli_fetch_row($prtrakqry)){?>
            <tr><td><?php echo $i; ?></td>
            <td><?php $prd=mysqli_query($con1,"select name from products where code='".$prtrak[2]."'");
						$product=mysqli_fetch_row($prd);
						echo $product[0];
			?></td>
            <td><?php echo $prtrak[3]; ?></td>
            <td><?php echo $prtrak[6]; ?></td>
            <td><?php echo $prtrak[4]; $i++; ?></td></tr>
             <?php } //end while loop?></tbody><?php }
			 else echo "No Product Views";
			 ?>
