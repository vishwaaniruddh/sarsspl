...
?>
  ...
  // echo "okhere".$_POST["latitude"];
    ...
    $arrdist=array();
    $arraycid=array();
        while($rfrws=mysql_fetch_array($qravdm))
        {
        
        
           $qrgetds=mysql_query("SELECT * FROM `clients` where code='".$rfrws['merchant_id']."'");
            $frds=mysql_fetch_array($qrgetds);
if($frds["Latitude"]!="")
        {
        ...
        if(count($arrdist2)>0 )
       {
        ...
        else
        {
                $key = array_search('$arrdist2[0]', $arrdist);
                  ...
                  $qravdmf=mysql_query("select * from advertise_booking where  slot='".$sliderid."' and slot_pos='".$ca."' and dt='".$todaysdtt."' and merchant_id='".$arraycid[$key]."'");
                    $qravdmfr=mysql_fetch_array($qravdmf);
...
<div class="image">
  ...
  <div class="product-meta">
        <h6 class="name"><a href="details.php?prid=<?php echo $detsarr[$itr]['code'];?>&catid=<?php echo $detsarr[$itr]['category'];?>"><p class="b"><?php echo $detsarr[$itr]['name'];?></p></a></h6>
