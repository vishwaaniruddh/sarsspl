...
if(isset($_POST['id']) && $_POST['id']>0){
   ...
   // echo $sql_update;exit;
    ...
    $sql_check="select mobile from member where mobile=".$Mobile;
    $runsql=mysqli_query($conn,$sql_check);
    $count_committee=mysqli_num_rows($runsql);
    if($count_committee==0){
        $sql="insert into member(position_id,name,address,email,mobile,pincode,entrydt,file,mailcode,addedby,country,state,city,district,taluka,village,zone,verify,level_id,loc_id,status,DOB,created_by,RefByMobile) values('".$position_id."','".$Name."','','".$Gmail."','".$Mobile."','".$Pincode."','".$dates."','".$imgdir."','".$code."','". $_SESSION['id']."','".$country."','".$state."','".$city."','".$District."','".$talukaa."','".$Village."','".$Zone."','".$verify."','".$hdLevel."','".$hdLoc."','".$status."','".$datepicker."','".$_SESSION["email"]."','".$RefMobile."')";
...
if($runsql){?>
   ...
   //   alert("Verification link sent to your mail id please check it.");
   <?php if($IsSuccess == 1){ 
            echo 'alert("Successfully Applied & Wait for admin Approval");';
       } else {
            echo 'alert("Member with same mobile number already exists.");';
       } ?>
       window.open("member3.php","_self");</script>
   <? } ?>
   <?php if(isset($_SESSION['user_name']) && ($_SESSION['user_name']=='Admin')){?>
    
      <script>     window.open("member3.php","_self");</script>
    <?php } ?>
