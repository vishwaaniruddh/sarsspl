...
if(isset($_REQUEST['sendcard'])){
    $IsSendCard=$_REQUEST['sendcard'];
} else {
    $IsSendCard = 0;
}
// echo $village;
				...
				html2canvas($("#html-content-holder"), { 
					onrendered: function(canvas) { 
						...
						mobilenum: myNum
    						...
    						console.log('saved');
    						let IsSendCard = <?php echo $IsSendCard; ?>;
    						if(IsSendCard === 1){
                                window.location.replace("http://avoservice.in/whatsapp.php?mobile=<?php echo $m;
                                if(isset($IsAgent)){
                                    echo "&agent=".$IsAgent;
                                }
                                //echo $m; ?>");
