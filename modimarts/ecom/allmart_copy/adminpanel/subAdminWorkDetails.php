...
$sql_statement = 'SELECT * FROM audit_log where 1 ';
	...
	$merid=$_POST['merid'];
	 ...
	 if($fromdt!="" && $todt!="" )
    {
      $sql_statement.=" and date_time between '".$fromdt." 00:00:00"."' and '".$todt." 23:59:59"."' ";
    }
	...
	//echo $sql_statement;
			...
			if($merid!="")
			{
			    ...
			    $sql_statement.= " and user_id = '".$merid."'";
			}
...
<div id="content">
 ...
 <tr height="30" ><td>
     <select class="inp-form" name="merid" id="merid">
  <option value="">Select Merchant</option>
  <?php
  
  
  $qMer=mysql_query("select DISTINCT(user_id) from audit_log where 1 ");
  while($getMerchantName=mysql_fetch_array($qMer)){
  
  
  $query1 = "SELECT username FROM admin_login where id='$getMerchantName[0]'";

$result1 = mysql_query($query1);
$row1=mysql_fetch_row($result1);


	$email = mysql_query("SELECT name FROM subAdminLogin where email='$row1[0]'");
    $ro=mysql_fetch_row($email);
  
  
  ?>
  
   <option value="<?php echo $getMerchantName[0];?>"><?php echo $ro[0];?></option>
  <?php }?>
  
</select>
     <input type="date" style="margin-left:20px;"  name="fromdate" class="inp-form" size="50" placeholder="From date" />
     <input type="date" style="margin-left:20px;"  name="todate" class="inp-form" placeholder="To date"  />
			...
			<!--  start table-content  -->
			  ...
			  <!--  start product-table ..................................................................................... -->
				...
				<tr>
                  ...
                  <!--  <th class="table-header-repeat line-left minwidth-1"><p style="font-size:16px; color:#FFF;">Srno</p>	</th>-->
					...
					<th class="table-header-options line-left"><p style="font-size:16px; color:#FFF;">Date Time</p></th>
				</tr>
                <?php
			
				$query = "SELECT user_id,action,description,date_time,id FROM audit_log where 1";
				
					if($merid!="")
			{
			    $query.= " and user_id = '".$merid."'";
			}
				
				
					 if($fromdt!="" && $todt!="" )
                        {
                          $query.=" and date_time between '".$fromdt." 00:00:00"."' and '".$todt." 23:59:59"."' ";
                        }
				
				$query.= " order by date_time  ASC LIMIT " . $from . "," . $offset;
...
while($row=mysql_fetch_array($result)){
	$query1 = "SELECT username FROM admin_login where id='$row[0]'";
...
$row1=mysql_fetch_row($result1);
	...
	$email = mysql_query("SELECT name FROM subAdminLogin where email='$row1[0]'");
    $ro=mysql_fetch_row($email);
//echo "SELECT name FROM subAdminLogin where email='$row1[0]'";
				?>
        	...
        	<?PHP
				...
				doPages($offset, 'subAdminWorkDetails.php', '', $total_records); 
			?>    
            </td>
		</tr>
