...
$sql_statement = 'SELECT * FROM Resale where 1';
...
window.open('edit_resaleProduct.php?id='+id+"&cid="+cid,'parentcon',"scrollbars=yes, resizable=no,width=750, height=400, top="+top+", left="+left);
}catch(exc)
{
...
function srfn()
{
   ...
   var adstatus= document.getElementById("adstatus").value;
    ...
    window.open("resale_productapproval.php?page=1&adstatus="+adstatus+"&uplaodby="+uplaodby,"_self");
}
...
$.ajax({
   type: 'POST',    
url:'resale_processproductapprove.php',
...
btn=btnid;
document.getElementById('h').value=adid;
...
function reason(){
  ...
  if(re!=""){
   //  var cod=ad;
     var cod= document.getElementById("h").value;
...
<div id="content">
	<!--  start page-heading -->
	<div id="page-heading">
		<h1>Resale Products Approval</h1>
	</div>
	<!-- end page-heading -->
<b>Product  Status</b>
...
<?php
$query1234 =mysql_query("SELECT * FROM Registration where resale_merchant='1'  order by Firstname asc");
		...
		<div id="content-table-inner">
			...
			<!--	<div id="message-green">
			  ...
			  <!--  start product-table ..................................................................................... -->
                ...
                <?php
				$query = "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`,  `price`, `others`, `discount`, `discount_type`, `total_amt`, `status`,product_type FROM `Resale` WHERE 1";
...
while($row=mysql_fetch_array($result)){
	$query1 = "SELECT id,Firstname,email FROM Registration where id='".$row['ccode']."'";
//	echo "$query1".$query1;
   $ctid=$row['category'];
    ...
    $query_categ = mysql_query("SELECT name FROM resale_category where id= ".$row['category']);
    $categ=mysql_fetch_assoc($query_categ);
    //var_dump($categ);
      ...
      if($row['status']==2){
        ...
        $disable = '';
				...
				<tr class="alternate-row">
					...
					<!--<td><input type="button" onclick="fnc('<?php echo $row['code']; ?>','<?php echo $ctid;?>');" value="View"></td>-->
					<td>
					    <?php if($adstats==2) {
                         ?>
				        ...
				        <a href="edit_resaleProduct.php?id=<?php echo $row['code']; ?>&cid=<?php echo $ctid;?>" target="blank"><input type="button"  value="View">
				<?php } ?>
				<input type="hidden" id="em" name="em" value="<?php echo $row1['email']; ?>" />
			<input type="hidden" id="rhd" name="rhd" value="<?php echo $row[0]; ?>" />
				</td>
				</tr>
                <?php $i1++; } ?>
                <tr>
        	...
        	<?PHP
				...
				doPages($offset, 'resale_productapproval.php', '', $total_records); 
			?>    
            </td>
		</tr>
    ...
    function doPages($page_size, $thepage, $query_string, $total=0) {
        ...
        if ($total != 0){
            ...
            echo '<p id="total_count">(total '.$total.$adstats.' records)</p></div>';
        }
