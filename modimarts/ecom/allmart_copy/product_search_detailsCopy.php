...
*/
$View="SELECT p.*,ct.name as city FROM Productviewtable p join `clients` cl on p.ccode = cl.code join `cities` ct on cl.city = ct.code  where p.status='1' ";
$View.= " and (p.name like'%".$srchtxt."%' or  p.description like'%".$srchtxt."%' or p.product_type like'%".$srchtxt."%' or p.brand like'%".$srchtxt."%' or p.keyword1 like'%".$srchtxt."%')";

	if($_POST["minv"]!="" && $_POST["maxv"]!="")
	{
$View.= " and p.total_amt>='".$minv."'  and  p.total_amt<='".$maxv."'";
$View.= " and ct.name='".$city."'";
    }
...
if($sort!="")
{
  if($sort==1)
  {
      $View.=" order by p.name ASC ";
  }
  elseif($sort==2)
  {
      $View.=" order by p.name desc ";
  }
  elseif($sort==3)
  {
      $View.=" order by p.total_amt ASC ";
  }
  elseif($sort==4)
  {
      $View.=" order by p.total_amt DESC ";
  }
}
else{
    
    $View.="order by p.code asc,";
}
echo $View;
echo 'lat'.$l;
