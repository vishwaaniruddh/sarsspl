...
$qr=mysqli_query($con1,"select id,time,duration from scheduled_ads where entrydt='".$dtn."' and time<='".$tym."' order by time desc");
...
if((strtotime($pldt)-strtotime($entrydt1))>$nrws[2])
{
    echo 1;
}
else
{
echo strtotime($pldt)-strtotime($entrydt1)."#####".$nrws[0];
}
