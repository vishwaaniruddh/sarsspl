...
<!--- Ruchi -->
<?php 
$qry=mysqli_query($con1,"SELECT * FROM `resale_category` where  status='1' order by id ");
//$data=mysqli_fetch_row($qry);
$cnt=1;
while($Resale = mysqli_fetch_array($qry)){
    //var_dump($Resale['id']);
...
$cnt++;
?>
<a class="img" style="color:white" title="<?php echo $Resale['name'];?>" href="resale_ViewAll.php?cat=<?php echo $id; ?>&Heading=<?php echo $Resale['name'];?>">
    <button style="top:82px;" class="btnreqmt" >View All</button>
</a>

<h3  style="margin-left: 58px;margin-top: 7px;margin-bottom: 5px;heigth:24px"><?php echo $Resale['name'];?></h3>
...
<div class="hover01 column">
  <div class="container" style="height:280px;width:1307px;padding-top: 3px;">
    <div class="row text-center">
        ...
        $qrylatf=mysqli_query($con1,"SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt` , `color` ,`size`,Long_desc FROM `Resale` where category=".$id." and status='1'  ORDER BY code DESC ");
        //echo "SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`, `price`, `others`, `discount`, `discount_type`, `total_amt` , `color` ,`size`,Long_desc FROM `Resale` where category=".$id." and status='1'  ORDER BY code DESC ";
        $i=1;
        $num=mysqli_num_rows($qrylatf);
        if($num>=1){
            while($row = mysqli_fetch_array($qrylatf)){
                $qryimg=mysqli_query($con1,"SELECT img,thumbs,midsize,largeSize FROM `Resale_img` WHERE `product_id`='".$row["code"]."' and category='".$row["category"]."' order by id asc limit 0,1");
                ...
                if($i<=6){ ?>
                    <div class="col-lg-2">
                        <div class="thumbnail" style="margin-bottom:0px">
                            <figure>     
                                <a title="<?php echo $row['name'];?>" href="resale_Details.php?prid=<?php echo $row['code'];?>&catid=<?php echo $row['category'];?>"> <img class="hover13 column" src="<? echo $Resaleimg[0];?>" alt="Paris" style="width:180px;height:164px"/></a>
                            </figure>
                            <p class="b"><strong><?php echo $row["name"]; ?></strong></p>
                            <p><i class="fa fa-inr "></i><?php echo $row["price"]; ?></p>
                            <a class="img" style="color:white" title="<?php echo $row['name'];?>" href="resale_Details.php?prid=<?php echo $row['code'];?>&catid=<?php echo $row['category'];?>"><button class="btn" style="background-color:#63bcf9"> BuyNow  </button> </a>
                        </div>
                    </div>
                    <?
                }
            ...
            while($Resale = mysqli_fetch_array($qrylatf)){
                ...
                if($id<=6){ ?>
                    <div class="col-lg-2">
                        <div class="thumbnail" style="margin-bottom:0px">
                            <figure>     
                                <a title="<?php echo $Resale['name'];?>" href="resale_Details.php?prid=<?php echo $Resale['code'];?>&catid=<?php echo $Resale['category'];?>"> <img class="hover13 column" src="<? echo $Resaleimg[0];?>" alt="Paris" style="width:180px;height:164px"/></a>
                            </figure>
