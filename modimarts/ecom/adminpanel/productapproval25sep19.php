...
'union all select `sarmicro_Merabazzar`.`Approval_products`.`code` AS `code`,`sarmicro_Merabazzar`.`Approval_products`.`name` AS `name`,'.
 '`sarmicro_Merabazzar`.`Approval_products`.`ccode` AS `ccode`,`sarmicro_Merabazzar`.`Approval_products`.`description` AS `description`,'.
 '`sarmicro_Merabazzar`.`Approval_products`.`category` AS `category`,`sarmicro_Merabazzar`.`Approval_products`.`photo` AS `photo`,'.
 '`sarmicro_Merabazzar`.`Approval_products`.`price` AS `price`,`sarmicro_Merabazzar`.`Approval_products`.`others` AS `others`,'.
' `sarmicro_Merabazzar`.`Approval_products`.`discount` AS `discount`,`sarmicro_Merabazzar`.`Approval_products`.`discount_type` AS `discount_type`,'.
 '`sarmicro_Merabazzar`.`Approval_products`.`quentity` AS `quentity`,`sarmicro_Merabazzar`.`Approval_products`.`total_amt` AS `total_amt`,'.
 '`sarmicro_Merabazzar`.`Approval_products`.`status` AS `status`,`sarmicro_Merabazzar`.`Approval_products`.`thumbs` AS `thumbs`,'.
 '`sarmicro_Merabazzar`.`Approval_products`.`midsize` AS `midsize`,`sarmicro_Merabazzar`.`Approval_products`.`Reason` AS `Reason`,'.
' `sarmicro_Merabazzar`.`Approval_products`.`size` AS `size`,`sarmicro_Merabazzar`.`Approval_products`.`brand` AS `brand`,'.
' `sarmicro_Merabazzar`.`Approval_products`.`color` AS `color`,`sarmicro_Merabazzar`.`Approval_products`.`size_id` AS `size_id`,'.
' `sarmicro_Merabazzar`.`Approval_products`.`product_type` AS `product_type`,`sarmicro_Merabazzar`.`Approval_products`.`Long_desc` AS `Long_desc`,'.
' `sarmicro_Merabazzar`.`Approval_products`.`keyword1` AS `keyword1` from `sarmicro_Merabazzar`.`Approval_products` where 1 ';
...
if($Maincate==1){	
	 $slting=mysql_query("SELECT * FROM `fashion_img` where product_id='".$row[0]."' limit 0,1");
}
else if($Maincate==190)
{	
     $slting=mysql_query("SELECT * FROM `electronics_img` where product_id='".$row[0]."' limit 0,1");
}
else if($Maincate==218)
{	
     $slting=mysql_query("SELECT * FROM `grocery_img` where product_id='".$row[0]."' limit 0,1");
} 
else if($Maincate==482)
{	
     $slting=mysql_query("SELECT * FROM `Resale_img` where product_id='".$row[0]."' limit 0,1");
} 
else
{	
     $slting=mysql_query("SELECT * FROM `product_img` where product_id='".$row[0]."' limit 0,1");
}
...
$prod = mysql_query("SELECT product_model FROM product_model where id='".$row['name']."'");
...
?>
            <?php if($row['status']!=1){?>
				<tr class="alternate-row">
        			...
        			<?php	$cat=mysql_query("select name from main_cat where id='".$row['category']."'");
            			$nrs=mysql_num_rows($cat);
            			if($nrs>0)
            			{
            			    $f=true;
            			    $rowr = mysql_fetch_assoc($cat);
        			 ?>
					<td><?php echo $rowr['name']; ?></td>
					<?php } else
					{ $f=false; ?>
					<td>Category Not Approved</td>
					<?php }?>
					...
					<td>
					    ...
					    <a href="EditProductApproval.php?pid=<?php echo $row['name']; ?>&bid=<?php echo $row['brand']; ?>&mid=<?php echo $Maincate;?>&uid=<?php echo $row['ccode']; ?>" target="blank"><input type="button"  value=" Approve "></a>
					   <button  id="apps<?php echo $i1;?>" onclick="approvefunc('<?php echo $row['code']; ?>',this.id,'2','app<?php echo $i1;?>','<?php echo $row['category']?>');" value="<?php echo $reject; ?>" <?php echo $disable; ?>></button>
					</td>
					<?php 
					/*<td>
					    <?php if($adstats==2) {
