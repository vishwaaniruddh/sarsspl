...
<header id="header-layout" class="header-v2">
 ...
 <!--<ul class="breadcrumb">
    ...
    <div id="content" class="col-sm-12" style="left: -23px;width: 1018px;">   
    <h2>My Wish List</h2>
        ...
        <table class="table table-bordered table-hover">
          ...
          <tbody style="background-color: white;">
             ...
             $getpro=mysqli_query($con1,"SELECT `code`, `name`, `ccode`, `description`, `category`, `photo`,  `price`, `others`, `discount`, `discount_type`, `total_amt` FROM `Productviewtable` WHERE code='".$slctwishftch['product_id']."' and category='".$slctwishftch['categories_id']."' ");     
              ...
              <td class="text-center"><?php echo $getprof['code'];?></td>
              <td class="text-center">In Stock</td>
              <td class="text-center">                <div class="price">
                                    <?php echo $getprof['total_amt'];?>                                  </div>
                </td>
                <style>     
      
                       
.trash{transition:opacity 200ms;width:128px;height:0px;opacity:0.8;position:relative;right:0;top:3px;vertical-align:middle;cursor:pointer;margin:-44px auto -17px}  
 ...
 .trash:focus > .lid,.trash:hover > .lid{transform:rotate(-45deg);transition:transform 250ms;-webkit-transform:rotate(-45deg);-webkit-transition:-webkit-transform 250ms}  </style>
              <td class=""style="width: 155px;">
               <!--   <button type="button" onclick="parent.addcart('<?php echo $slctwishftch['product_id']; ?>','<?php echo $slctwishftch['categories_id']; ?>');" data-toggle="tooltip" title="Add to Cart" class="btn btn-primary"><i class="fa fa-shopping-cart"></i></button>
                <a href="javascript:void(0);" data-toggle="tooltip" title="Remove" class="btn btn-danger" onclick="removewishlist('<?php echo $slctwishftch['wishlist_id']; ?>');"><span class="glyphicon" style="top:3px;font-size: 12px;">&#xe020;</span> </a>
               -->
               	<img src="image/cartdemo2.gif" style="width: 47px;height: 39px;margin-top: 18px;" data-toggle="tooltip" title="Add to Cart" onclick="parent.addcart('<?php echo $slctwishftch['product_id']; ?>','<?php echo $slctwishftch['categories_id']; ?>');"/>
                <div data-toggle="tooltip" title="Remove" class="" onclick="parent.removewishlist('<?php echo $slctwishftch['wishlist_id']; ?>');"  style="margin-left: 15px;padding-top: -17px;" ><span class="glyphicon" style="top:-15px;font-size: 12px;"> <div class="trash" title="Remove" style="right:21px"><span class="lid" style="width:26px"></span><span class="can" style="width:26px"></span></div></span> </a>
