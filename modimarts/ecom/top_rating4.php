...
include("config.php");
$sliderid="12";
$slidername="topratingslider4";
$slidertablename="top_right_slider4";
...
<div class="widget panel ">
	<div class="widget-heading panel-heading block-borderbox"><h4 class="panel-title">  &nbsp; &nbsp;</h4>Current Offers
</div>
...
<?php
    ...
    for($ca=0;$ca<=8;$ca++)
    {
      ...
      // echo "okhere".$_POST["latitude"];
        ...
        $arraycid=array();
            ...
            {
                $qrgetds=mysqli_query("SELECT * FROM `clients` where code='".$rfrws['merchant_id']."'");
    ...
    if($latstprnrws>0)
    {
        ...
        //echo "select * from Productviewimg where product_id='".$lat1['pid']."' and category='".$lat1['cat']."' ";
        $prdrwsimg=mysqli_fetch_array($getprdetsimg);
        $detsarr[]=['code'=>$prdrws['code'],'name'=>$prdrws['name'],'pname'=>$prdrws['product_model'],'ccode'=>$prdrws['ccode'],'description'=>$prdrws['descr'],'category'=>$prdrws['category_id'],'photo'=>$prdrwsimg['img'],'price'=>$prdrws['price'],'discount'=>$prdrws['discount'],'discount_type'=>$prdrws['discount_type'],'total_amt'=>$prdrws['total_amt'],'updtid'=>$lat1['id'],'avail'=>$avilfrupdt];
...
{  ?>	
    ...
    <div class="product-img img">
        ...
        <img class="img-responsive" src="<?php echo $prodimgpth.$detsarr[$itr]['photo'];?>" title="<?php echo $detsarr[$itr]['pname'];?>" style="height:150px; width: 100%;
