...
else if($Maincate==190)
{
    ...
    */
    $View="select p.* from (select pm.product_model,pm.category_id,pd.code,pd.ccode,pd.price,pd.total_amt,pd.name from product_model as pm join electronics as pd on pm.id=pd.name where pm.category_id ='".$mdi."' and pm.status=1 and total_amt>='".$min_total_amt."'  and  total_amt<='".$max_total_amt."' order by pd.price ) as p group by p.product_model";
 ...
 else if($Maincate==218)
{
    ...
    */
    $View="select p.* from (select pm.product_model,pm.category_id,pd.code,pd.ccode,pd.price,pd.total_amt,pd.name from product_model as pm join grocery as pd on pm.id=pd.name where pm.category_id ='".$mdi."' and pm.status=1 and total_amt>='".$min_total_amt."'  and  total_amt<='".$max_total_amt."' order by pd.price ) as p group by p.product_model";
} 
 else 
{
...
if($Maincate==1){
$slting=mysqli_query($con1,"SELECT img FROM `fashion_img` where product_id='".$rwss['code']."' limit 0,1");
}
else if($Maincate==190)
{
    $slting=mysqli_query($con1,"SELECT img FROM `electronics_img` where product_id='".$rwss['code']."' limit 0,1");
}
else if($Maincate==218)
{
    $slting=mysqli_query($con1,"SELECT img FROM `grocery_img` where product_id='".$rwss['code']."' limit 0,1");
}
else
{
    $slting=mysqli_query($con1,"SELECT img FROM `product_img` where product_id='".$rwss['code']."' limit 0,1");
}
