...
<main class="main-content">
  ...
  <div class="wrapper">
    ...
    $allmart_commission = $view_result['allmart_commission'];
     ...
     // if ($view_result['shipping_in_area']) {
     //          $shipping         = 'shipping_in_area';
     //          $shipping_charges = $view_result['shipping_in_area'];
     //      } else if ($view_result['shipping_out_state']) {
     //          $shipping         = 'shipping_out_state';
     //          $shipping_charges = $view_result['shipping_out_state'];
     //      } else {
     //          $shipping         = '';
     //          $shipping_charges = 0;
     //      }
     //            $minqty=$view_result['minqty'];
...
if($cat =='205'){
   ...
   //  $image = $rws3['img'];
    ...
    $status =getstatus($view_result['name']);
     $prodata=getproductprice($view_result['category_id'],$view_result['code']);
...
<li class="grid__item item-row wide--one-fifth post-large--one-fifth large--one-third medium--one-half small--one-half"
                            ...
                            >
                              ...
                              <div class="products product-hover-11">
                                ...
                                <a href="/<?=strurl($name)?>/P/<?=strcode($view_result['code'])?>/<?=strcode($view_result['category_id'])?>/<?=strcode($view_result['name'])?>"> 
                                ...
                                <div class="product-detail">
                                  ...
                                  <div class="grid-link__meta">
                                    ...
                                    <a
                                      ...
                                      title="Add to Cart"                                      
                                      onclick="addtocart('<?=$view_result['category_id']?>','<?=$view_result['code']?>','<?=$finalPrice?>','<?=$getimgdata?>','<? echo $name;?>','<?=$view_result['name']?>','<?=$shipping?>',<?=$shipping_charges?>,<?=$minqty?>)"
...
$is_search= '&s_title='.$search;

 
$addurl="https://thebrandtadka.com/api/index.php?mod=ApiMobile&api_key=VarifyTADKA7563&company_id=448&action=getProductList&token=02c276c93ce8508fc578476b99ab4c80".$is_search;
...
if($total_rows<$nextpage)
{
$nextpage=$total_rows;
}

$addurl="https://thebrandtadka.com/api/index.php?mod=ApiMobile&api_key=VarifyTADKA7563&company_id=448&action=getProductList&start_point=".$offset."&end_point=".$nextpage."&token=02c276c93ce8508fc578476b99ab4c80&category_id=".$is_search;
...
$products=$resultres->Records;
   ...
   for ($j=0; $j < count($products) ; $j++) { 
    $mrp=$products[$j]->Price;
    if($_SESSION['mem_id']!=''){ 
        $price=$products[$j]->Price/2;
    }else{
 ...
 <div class="products product-hover-11">
    <div class="product-container">
        <a href="/Product-Details/<?=$products[$j]->Product_id?>"> 
            <div class="ImageOverlayCa"></div>
                <a href="/Product-Details/<?=$products[$j]->Product_id?>">
                  <img src="https://thebrandtadka.com/images_inventory_products/front_images/<?=$products[$j]->Product_image?>" class="featured-image" alt="" style="width:200px;height:200px;"/></a></a>
                                  <?php if($price<$mrp){?>
                                   <div class="product_right_tag offer_exist">
                                    <span class="offer-price">
                                      <b><?php echo round((1 - ($price / $mrp)) * 100,0);?> % Off
                                    </b></span>
                                  </div>
                                  <?php } ?>
                                  ...
                                  <div class="ImageWrapper">
                                    <div class="product-button">
                                      <div class="add-to-wishlist">
                                        <div class="show">
                                          <div class="default-wishbutton-black-tea loading" >
                                            <a title="Add to wishlist" class="add-in-wishlist-js" href="black-tea"><i class="far fa-heart"></i><span class="tooltip-label">Add to wishlist</span></a>
                                          </div>
                                          <div class="loading-wishbutton-black-tea loading" style="display: none; pointer-events: none;"><a class="add_to_wishlist"><i class="fas fa-spinner"></i></a>
                                          </div>
                                          <div class="added-wishbutton-black-tea loading" style="display: none"><a title="View Wishlist" class="added-wishlist add_to_wishlist" href="#"><i class="fas fa-heart"></i><span class="tooltip-label">View Wishlist</span></a>
                                          </div>
                                        </div>
                                      </div>
                                      <a href="javascript:void(0)" title="Quick View" class="quickview-button quick-view-text product_link"><i class="fa fa-search"></i></a>
                                    </div>
                                  </div>
                                </div>
                                <div class="product-detail">
                                 
                                  <a href="/Product-Details/<?=$products[$j]->Product_id?>"><?=substr($products[$j]->Title,0,30)?></a>
                                  <div class="grid-link__meta">
                                    <div class="product_price">
                                        <?php if($_SESSION['mem_id']!=''){?>
                                      <div class="grid-link__org_price" id="ProductPrice"><span>MRP - ₹ <?=$products[$j]->Price?></span>
                                      </div>
                                      <div class="grid-link__org_price" id="ProductPrice"><span>Offer Price - ₹ <?php  echo $products[$j]->Price/2;  ?></span>
                                      </div>
                                      <div class="grid-link__org_price" id="ProductPrice"><span> Discount - <?php echo round((1 - ($price / $mrp)) * 100,0);?> % </span>
                                      </div>
                                  <?php } else{ ?>
                                      <div class="grid-link__org_price" id="ProductPrice"><span>Price - ₹ <?php if($_SESSION['mem_id']!=''){ echo $products[$j]->Price/2; }else{?> <?=$products[$j]->Price?><?php } ?></span>
                                      </div>
                                  <?php } ?>
                                    </div>
                                  </div>
...
<hr>
  ...
  <?php
    if(isset($_GET['search'])){
      $catlink="&search=".urlencode($_GET['search']);
      }
      else
      {
      $catlink="";
