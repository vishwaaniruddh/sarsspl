...
$catg_id    = mysqli_real_escape_string($con1, $_GET['catid']);
...
<nav class="breadcrumb" aria-label="breadcrumbs">
        <div class="container-bg">
		  ...
		  <?php
$sqlbrdcr = mysqli_query($con1, "select * from main_cat where id ='" . $catg_id . "'");
...
while ($row = mysqli_fetch_assoc($sql_category)) {
    ...
    ?>
              <div class="grid__item grid__item wide--one-quarter post-large--one-quarter large--one-quarter medium--one-half small--one-whole text-center pickgradient-products" >
                <a
                  href="/list/<?=strurl($row['name'])?>/<?=strcode($row['id'])?>"
                  title="Browse our <?=ucwords($row['name'])?> collection"
                  class="pickgradient grid-link"
                >
                  <img
                    src="/ecom/adminpanel/<?php echo $row['cat_img']; ?>"
                ...
                <div class="collection-detail">
                  ...
                  <a href="/list/<?=strurl($row['name'])?>/<?=strcode($row['id'])?>" class="btn">View All</a>
                </div>
              </div>
              <?php
              }
