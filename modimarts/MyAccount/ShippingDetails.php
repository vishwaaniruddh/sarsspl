      ...
      <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12" style="margin-top: 100px;position: unset">
        <div class="container">
            <div class="container">
              ...
              <?php
              $sql = mysqli_query($con1, "SELECT * FROM `order_shipping` WHERE oid='" . $order_id . "'");
                 $i=1;
              while($sql_result = mysqli_fetch_assoc($sql)) {
                  $item_id       = $sql_result['item_id'];
                  $channel_id=$sql_result['channel_id'];
                  $shipment_id=$sql_result['shipment_id'];
                  $order_ids=$sql_result['order_ids'];
                  $awb_code=$sql_result['awb_code'];
                  $generate_awb=$sql_result['generate_awb'];
                  $generate_pickup=$sql_result['generate_pickup'];
                  $generateManifest=$sql_result['generateManifest'];
                  $printManifest=$sql_result['printManifest'];
                  $generateLabel=$sql_result['generateLabel'];
                  $printInvoice=$sql_result['printInvoice'];
                  $gettrackdetails=$sql_result['gettrackdetails'];

                  // $decodepickup= json_decode($generate_pickup,true);
                  // var_dump($generate_pickup);
                  echo "<br/>";
                
                  $checkLogin = $generate_pickup;
                  // var_dump($checkLogin);
                  ...
                  $prodata = mysqli_query($con1, "SELECT * FROM `order_details` WHERE id='" . $item_id . "'");
                  $dataResult=mysqli_fetch_assoc($prodata);
         ...
         <div class="row form-group" style="background: #e2e2e2;border-radius: 4px;margin: 1px;padding: 2%;">
          <div class="col-md-3"> <img src="<?=$image?>" alt="<?=$product_name?>" width="100" height="100"></div>
          <div class="col-md-6 form-group">
           
          <h4><strong><?=$product_name?></strong></h4>
          <h5><strong>Track Id : #<?=$awb_code?></strong></h5>         
          <p>Order Date : <?=date('D ,d-m-Y',strtotime($date))?></p>
        </div>
        <div class="col-md-3 form-group">
        <!--   <button data-toggle="collapse" class="btn btn-info" style="float: right" data-target="#Product_<?=$i?>">Show More</button> -->
        </div>
      </div>
      <br/>
