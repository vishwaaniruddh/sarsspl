...
// $garment =mysqli_query($con1,"select * from  `garment_product` and  gproduct_id in(select gproduct_id from product_images_new where gproduct_id>0) and status=1  order by id desc limit 10");
 ...
 if (mysqli_num_rows($view)) {
  ...
  while ($view_result = mysqli_fetch_assoc($view)) {
   ...
   //$discount_price = $view_result["total_amt"];
   ...
   $getimgdata = Getimg($view_result['code'], $view_result['category_id'], $view_result['name']);
     ...
     $prodata=getprice($view_result['category_id'],$view_result['code']);
    ...
    <li class="products">
                          ...
                          <div class="grid__item  large--one-whole ">
                            ...
                            <div class="product-container" style="height: 19.5em; width:13em ">
                              ...
                              <a class="grid__item" href="/<?=strurl($productname)?>/P/<?=strcode($view_result['code'])?>/<?=strcode($view_result['category_id'])?>/<?=strcode($view_result['name'])?>">
                          ...
                          <div class="grid__item wide--two-thirds post-large--two-thirds large--two-thirds medium--two-thirds small--grid__item">
                            ...
                            <div class="top-products-detail product-detail grid__item">
                              ...
                              <a class="grid-link__title"  href="/<?=strurl($productname)?>/P/<?=strcode($view_result['code'])?>/<?=strcode($view_result['category_id'])?>/<?=strcode($view_result['name'])?>">
