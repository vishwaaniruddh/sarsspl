...
//  	}
    ...
    public function checkSecurityKey() {
        ...
        $allowed_url = array('scanQrcode', 'qrcodedetail', 'sendotpviewpage', 'sendotpqrcode', 'verifyotpsdqrcode', 'qrcodeuserdetail', 'qrcodedetailbycode', 'send_NotificationToUser', 'checkReturningToLocation', 'getDeviceToRing', 'updateTrackerDeviceFinder',
            'addmissingalert', 'addQrCodeScanAlert', 'sendNotificationAdminQrCodeScan', 'addTrackerData', 'getTrackerData', 'getTestFirebaseData', 'getTestData','getTestDataDDevice','get_PanicRequestByID','getLastLocationData','batteryNotifications',
            'updateTrackerDeviceOnOffSettings', 'getDeviceToOnOffSettings','updateTrackerDeviceUpdateTime','getDeviceToUpdateTime','updateTrackerDevicePowerSavingTimer','getDeviceToPowerSavingTimer','updateTrackerDevicePowerSaving', 'getDeviceToPowerSaving',
...
///ini_set('display_errors', '1');
        ...
        if ($result) {
            ...
            $finresult = array('status' => 'Success','check'=>'checking', 'result' => $res);
            print json_encode($finresult);
        } else {
    ...
    public function setTracking() {
        ...
        if ($_POST['tracking_type'] == 2) {
            ...
            // 24-aug-2022
            $this->firebase->send_notification('Captain India is following you sucessfully until you reach your destination safely.', $user_device_token, 'Captain India', null, 'followme');
        }
        ...
        if ($result) {
            ...
            if ($panic_info != 0) {
                if ($panic_info->module_type == '1') {
                    ...
                    $user_panic_time = $panic_time;
                    $user_latitude = $panic_info->user_lat;
                    $user_longitude = $panic_info->user_long;      
                  //  $send_notification_to_emergency_contacts = send_notification_to_emergency_contacts($user_name, $user_panic_time, );  
                    $this->load->library('firebase');    
                    $response = $this->api_model->sendMessage($message, $loc, $notify_url);
                        ...
                        foreach ($array as $k => $v) {
                            ...
                            if ($euid) {
                                ...
                                if ($mobile_no) {
                                    ...
                                    $curl = curl_init();
                                    curl_setopt($curl, CURLOPT_URL, 'https://meraqr.co/api/SOSMessage');
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                for ($i = 0; $i < count($_FILES['image']['name']); $i++) {
                    ...
                    if (!move_uploaded_file($_FILES['image']['tmp_name'][$i], $target_path_new)) {
                        ...
                        // $target_thumbnail_path_url_main = $image_url_path.'thumbnail/'.$newthumbfilename;
                        // 7-feb-2023
                        $target_path_url_main = '/multimedia/' . $newfilename;
                        $target_thumbnail_path_url_main = '/multimedia/thumbnail/' . $newthumbfilename;
...
// 		if($finresults){
    ...
    public function getLocationAddressFromLatLong() {
        error_reporting(E_ALL);
        ini_set('display_errors', '1');
        $input_data = $_POST;
                    ...
                    if ($json_result) {
                        if (isset($json_result['results'][0]['formatted_address'])) {
                           // return ($json_result['results'][0]['formatted_address']);
                            $formatted_address = $json_result['results'][0]['formatted_address'];
                            $result = array('status' => 'Success', 'message' => $formatted_address);
                            print json_encode($result);
                        } else {
                            $result = array('status' => 'Fail', 'message' => "No record found.",);
                            print json_encode($result);
...
// die;
    ...
    //ATM   //  curl_setopt($curl, CURLOPT_URL, 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' . $latitude . ',' . $longitude . '&radius=10000&types=' . $location_type . '&key=AIzaSyDIJerlIQ5DZUqXmAndOExCfLQ6lNnyU1c');
                ...
                if (strtolower($location_type) == 'atm' || strtolower($location_type) == 'hospital' || strtolower($location_type) == 'police' || strtolower($location_type) == 'gas_station') {
                    curl_setopt($curl, CURLOPT_URL, 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' . $latitude . ',' . $longitude . '&radius=10000&types=' . $location_type . '&key=AIzaSyDIJerlIQ5DZUqXmAndOExCfLQ6lNnyU1c');
                } else {
                    curl_setopt($curl, CURLOPT_URL, 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?keyword=' . $location_type . '&location=' . $latitude . ',' . $longitude . '&radius=10000&types=' . $location_type . '&key=AIzaSyDIJerlIQ5DZUqXmAndOExCfLQ6lNnyU1c');
                }
    ...
    // 17-nov-2022
    public function getBulletinTag_Backup() {
        $data = $_POST;
        if (isset($data['user_id'])) {
...
//                                echo json_encode($return);
    ...
    public function addTrackerData() {
        ...
        if ($info) {
            ...
            if($is_battery_issue>0 && $imei != null){
            ...
            if ($lat != null && $lng != null) {
                ...
                if ($final_result && $imei != null) {
                    $this->checkTrackerDevicesData($imei);
                }
                if ($final_result && $imei != null) { 
                    ...
                    if($query->num_rows() > 0){
                      // if(result_tracker_devices_check){
                        $arr_tracker_devices_to_update = array('tracker_info' => $tracker_info, 'power'=>$power,'last_communication' => date('Y-m-d H:i:s'));
    ...
    public function getTrackerData($id = '') {
        ...
        if ($result) {
            ...
            if($user_id!='' && $date!=''){
              ...
              $total_count = 0; $final_total_count = 0;
                if ($imei != '') {
            ...
            $result = array('status' => 'Success', 'tracker_name' => $tracker_name, 'total_count' => $final_total_count, 'result_log' => $result_track_point,
            'user_id'=>$user_id, 'date'=>$date,'imei'=>$imei,'result' => $result);
            print json_encode($result);
        } else {
            $error_info = $this->api_model->getErrorCode('13');
            $result = array('status' => 'Fail', 'message' => $error_info->message, 'code' => $error_info->code);
    ...
    public function addTrackerDevice() {
        ...
        if ($data) {
            ...
            if ($result_tracker_devices_check) {
                ...
                $result = array('status' => 'Fail', 'message' => 'Tracker device invalid.','res' => $result_tracker_devices_check);
                print json_encode($result);
            }
        } else {
            $error_info = $this->api_model->getErrorCode('13');
                ...
                if ($result) {
                    ...
                    if ($device_imei != null) {
                        ...
                        if ($result_tracker_device_location) {
                            ...
                            $result = array('status' => 'Success', 'result' => $result_tracker_device_location, 'message' => '','formattedaddress' => $res_location_name, 'track_last_comm' => $_track_last_comm, 'totaltimediff' => $totaltimediff );
                            print json_encode($result);
                        } else {
    ...
    public function checkFirebaseData($firebase_key, $tracker_devices_id) {
    ...
    public function addDeviceFirebaseData($firebase_key, $arr) {
        // $handle = curl_init();
        // // $url = "https://device-850c6-default-rtdb.firebaseio.com/raw-locations/" . $firebase_key . "/location.json";
        // $url = "https://captain-india-1fd08-default-rtdb.asia-southeast1.firebasedatabase.app/raw-locations/" . $firebase_key . "/location.json";
        // curl_setopt($handle, CURLOPT_URL, $url);
    ...
    public function checkTrackerDevicesData($device_imei) {
        $this->db->select('*');
        $this->db->from('tracker_devices');
        $this->db->where('is_deleted', '0');
        $this->db->where('device_imei', $device_imei);
    ...
    // public function getCalulatedDistance() {
    //     ini_set('display_errors', 1);
    ...
    //     if (!$geo_fence_data) {
    //         $result = array('status' => 'Error', 'code' => 404, 'message' => 'No geofence data found');
    //         print json_encode($result);
    //         return;
    //     }
    ...
    //     if (!$last_location) {
    //         $result = array('status' => 'Error', 'code' => 404, 'message' => 'No last location data found');
    //         print json_encode($result);
    //         return;
    //     }
    
    //     $last_lat = $last_location['lat'];
    //     $last_lng = $last_location['lng'];
    ...
    //     $distances = array(
    //         'loc1' => haversine_distance($last_lat, $last_lng, $geo_fence_data['loc1_lat'], $geo_fence_data['loc1_lng']),
    //         'loc2' => haversine_distance($last_lat, $last_lng, $geo_fence_data['loc2_lat'], $geo_fence_data['loc2_lng']),
    //         'loc3' => haversine_distance($last_lat, $last_lng, $geo_fence_data['loc3_lat'], $geo_fence_data['loc3_lng']),
    //     );
    
    //     // Check if distances are within the specified ranges
    //     $within_range = array(
    //         'loc1' => $distances['loc1'] <= $geo_fence_data['loc1_range'],
    //         'loc2' => $distances['loc2'] <= $geo_fence_data['loc2_range'],
    //         'loc3' => $distances['loc3'] <= $geo_fence_data['loc3_range'],
    //     );
    ...
    // private function getLastLocationData($user_id) {
    //     // Joining the user_tracker_devices and tracker_devices tables
...
// error_reporting(E_ALL);
        ...
        if ($is_insert == 1) {
            ...
            if ($pnquery->num_rows() > 0) {
                if($push_notification_result['is_start']=='1'){
                    $notify_response = $this->send_NotificationToUser($user_id, $nearest_location_remark, 'CaptainIndia - Geofence Security Scope');
   ...
   /* Insert & Update SOS Contacts GPS Tracker 
      Author BY : Prabir */
                ...
                if ($result) {
                    ...
                    if ($device_imei != null) {
                        ...
                        if ($result_tracker_device_location) {
                            ...
                            $result = array('status' => 'Success', 'result' => $result_tracker_device_location, 'message' => '','formattedaddress' => $res_location_name, 'track_last_comm' => $_track_last_comm, 'totaltimediff' => $totaltimediff, 'image_url' => $image_url_link, 'device_state' => $device_state, 'battery_power' => $_battery_power );
                            print json_encode($result);
                        } else {
                            $error_info = 'No record found';
    ...
    public function getTrackerNewData($id = '') {
        ...
        if ($result) {
            ...
            'user_id'=>$user_id, 'date'=>$date,'imei'=>$imei,'result' => $result,'start_lat'=>$_strt_lat,'start_lng'=>$_strt_lng,'end_lat'=>$_end_lat,'end_lng'=>$_end_lng, 'tot_count' => $total_count);
            print json_encode($result);
        } else {
            $error_info = $this->api_model->getErrorCode('13');
            $result = array('status' => 'Fail', 'message' => $error_info->message, 'code' => $error_info->code, 'image_url' => $image_url_link);
      ...
      //  $imei = $_POST['imei'];
       ...
       if($result){
          ...
          //  print_r($response);
            ...
            $device_token = $result['last_name'];
            $send_message = $first_name . ' ' . $last_name . ' Welcome to Captain India.';
            $msg_response = $this->firebase->send_notification($send_message, $device_token, 'Captain India'); */
                $result_array = array('Code' => 200, 'result' => $json_result);
                print json_encode($result_array);
            }
        }
        else{
        ...
        if($user_id > 0){
         ...
         //   print json_encode($result);
            ...
            if($fcmresult){
                ...
                $curl = curl_init();
                curl_setopt($curl, CURLOPT_URL, 'https://meraqr.co/api/send_firebase_msg');
                curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($curl, CURLOPT_POST, true);
                curl_setopt($curl, CURLOPT_ENCODING, '');
                curl_setopt($curl, CURLOPT_MAXREDIRS, 10);
        ...
        foreach($users as $user){
                ...
                if ($newresult) {
                    ...
                    if ($tr_query->num_rows() > 0) {
                        ...
                        if ($pnquery->num_rows() > 0) {
                            if($push_notification_result['is_start']=='1'){
                                $notify_response = $this->send_NotificationToUser($user_id, $notification_msg,'CaptainIndia - Battery Notification');
                                if ($notify_response) {
    ...
    public function addNewTrackerData_123() {
        date_default_timezone_set('Asia/Kolkata');
        $info = $_GET;
        //   $result = array('status' => 'Fail', 'message' => $info);
        //   print json_encode($result);
            ...
            if ($lat != null && $lng != null) {
              ...
              //      print json_encode($result);
                ...
                if ($final_result) { 
                    ...
                    if($query->num_rows() > 0){
                      // if(result_tracker_devices_check){
                        $arr_tracker_devices_to_update = array('last_communication' => date('Y-m-d H:i:s'));
                        $this->db->where('imei', $imei);
                        $res = $this->db->update('tracker_device_location_update', $arr_tracker_devices_to_update); 
                    }else{
                        $insert_loc_data = array(
