...
// 			$result = $this->login->getUsers($status,$plan);
                ...
                foreach ($result as $key => $value) {
                    ...
                    $result[$key]['email_val'] = $value['email'];
                    unset($result[$key]['middle_name']);
                    unset($result[$key]['last_name']);
                    ...
                    if ($value['plan_id'] != null) {
                        ...
                        if (!empty($plan_res)) {
                            ...
                            $plan_title_value = $plan_res['title'];
                        }
                    }
                    ...
                    if ($value['tenant_licence_id'] != null) {
                        ...
                        if (!empty($tenant_licence_res)) {
                            ...
                            $licence_key_value = $tenant_licence_res['licence_key'];
                            $start_date_value = $tenant_licence_res['start_date'];
                            $end_date_value = $tenant_licence_res['end_date'];
                        }
                    }
                    ...
                    $result[$key]['email_val'] = base64_encode($result[$key]['email_val']);
                    $result[$key]['tenant_name'] = base64_encode($result[$key]['tenant_first_name'] . ' ' . $result[$key]['tenant_last_name']);
                    $result[$key]['address'] = base64_encode($value['address']);
                    $result[$key]['plan_title_value'] = base64_encode($plan_title_value);
                    $result[$key]['licence_key_value'] = base64_encode($licence_key_value);
                    $result[$key]['start_date_value'] = base64_encode($start_date_value);
                    $result[$key]['end_date_value'] = base64_encode($end_date_value);
                    $emergency_contacts = $this->login->getEmergencyContact($value['uid']);
                    $result[$key]['em_1'] = "";
                    $result[$key]['em_2'] = "";
                    $result[$key]['em_3'] = "";
                    $result[$key]['em_4'] = "";
                    if ($emergency_contacts) {
                        if (isset($emergency_contacts[0])) {
                            $result[$key]['em_1'] = base64_encode($emergency_contacts[0]['mobile_no'] . '-' . $emergency_contacts[0]['first_name'] . ' ' . $emergency_contacts[0]['last_name']);
                        }
                        if (isset($emergency_contacts[1])) {
                            $result[$key]['em_2'] = base64_encode($emergency_contacts[1]['mobile_no'] . '-' . $emergency_contacts[1]['first_name'] . ' ' . $emergency_contacts[1]['last_name']);
                        }
                        if (isset($emergency_contacts[2])) {
                            $result[$key]['em_3'] = base64_encode($emergency_contacts[2]['mobile_no'] . '-' . $emergency_contacts[2]['first_name'] . ' ' . $emergency_contacts[2]['last_name']);
                        }
                        if (isset($emergency_contacts[3])) {
                            $result[$key]['em_4'] = base64_encode($emergency_contacts[3]['mobile_no'] . '-' . $emergency_contacts[3]['first_name'] . ' ' . $emergency_contacts[3]['last_name']);
                        }
                    }
                    $i++;
                }
        ...
        switch ($_SERVER['REQUEST_METHOD']) {
            ...
            case 'GET':
                ...
                foreach ($result as $key => $value) {
                    ...
                    // 23-aug-2023
                    if ($result[$key]['tracker_device_id'] != null) {
                        $tracker_device_res = $this->login->getTrackerDeviceNameById($result[$key]['tracker_device_id']);
                        $result[$key]['trackee_id'] = $tracker_device_res['device_name'] . ' (' . $tracker_device_res['tracker_device_name'] . '-' . $tracker_device_res['device_imei'] . ')';
                    }
    ...
    public function getPlans($tid = '') {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $datas = json_decode(file_get_contents("php://input"));
                //	echo "<pre>";print_r($_POST['session_id']);exit;
                $result = $this->login->getPlans($tid);
                if (!empty($result)) {
                    foreach ($result as $key => $value) {
                        ...
                        if ($value['tenant_id'] != null) {
                            $tenant_id_res = $this->login->getAdminDetailById($value['tenant_id']);
                            if ($tenant_id_res) {
                                $tenant_id_name = $tenant_id_res->first_name . ' ' . $tenant_id_res->last_name;
                            }
                        }
                        $result[$key]['tenant_name'] = $tenant_id_name;
                    }
                }
                echo json_encode($result);
                break;
...
//                get user device
                $result->user_tracker_device = $this->login->getUserDeviceByUserId($id);
                echo json_encode($result);
                break;
        }
    }
...
//die;
    ...
    public function getUserPoshEvidenceTest($tid = '') {
    ...
    public function getTrackingMapById($id = null, $updated = null) {
        ...
        if ($id) {
            ...
            $output = curl_exec($handle);
            curl_close($handle);
            $output = $this->login->getFirebaseLocationByFirebaseId($firebase_key);
    ...
    public function getTodayTracking($tid = '') {
                        ...
                        if ($result[$key]['status'] == 1) {
                            if (isset($result[$key]['firebase_key']) && $result[$key]['firebase_key'] != null) {
                                ...
                                //$url = "https://captain-india-1fd08-default-rtdb.asia-southeast1.firebasedatabase.app/raw-locations/".$firebase_key."/everyMinute.json";
                                                            ...
                                                            $setting_url_res = $this->login->getSettingById(6);
        $setting_url_res_value = '';
        if ($setting_url_res) {
            $setting_url_res_value = $setting_url_res['value'];
        }
        $setting_auth_key_res = $this->login->getSettingById(3);
        ...
        $url = $setting_url_res_value . $firebase_key . "/location.json?auth=" . $setting_auth_key_value;
                            ...
                            $info = $this->db->get_where('users', array('id' => $result[$key]['trackee_id']))->row(); //$value['first_name'].' '.$value['last_name'].'    ';
                            // $result[$key]['trackee_id'] = $info->row()->first_name . ' ' . $info->row()->last_name;
                            // $last_arr['name'] = $info->row()->first_name . ' ' . $info->row()->last_name;

                            $trackee_id_first_name = '';
                            if (isset($info->first_name) && $info->first_name != null) {
                                $trackee_id_first_name = $info->first_name ? $info->first_name : '';
                            }
                            $trackee_id_last_name = '';
                            if (isset($info->last_name) && $info->last_name) {
                                $trackee_id_last_name = $info->last_name ? $info->last_name : '';
                            }
                            $result[$key]['trackee_id'] = $trackee_id_first_name . ' ' . $trackee_id_last_name;
                            $last_arr['name'] = $trackee_id_first_name . ' ' . $trackee_id_last_name;

                            $info = $this->db->get_where('users', array('id' => $result[$key]['tracker_id']))->row(); //$value['first_name'].' '.$value['last_name'].'    ';
                            // $result[$key]['tracker_id'] = $info->row()->first_name . ' ' . $info->row()->last_name;
                            $tracker_id_first_name = '';
                            if (isset($info->last_name) && $info->first_name) {
                                $tracker_id_first_name = $info->first_name ? $info->first_name : '';
                            }
                            $tracker_id_last_name = '';
                            if (isset($info->last_name) && $info->last_name) {
                                $tracker_id_last_name = $info->last_name ? $info->last_name : '';
                            }
                            $result[$key]['tracker_id'] = $tracker_id_first_name . ' ' . $tracker_id_last_name;
                            ...
                            $result[$key]['trackee_id'] = $tracker_id_first_name . ' ' . $tracker_id_last_name;
                            $last_arr['name'] = $tracker_id_first_name . ' ' . $tracker_id_last_name;
                            $lat_long_arr[] = $last_arr;
                        }
                    }
                    $result = $lat_long_arr;
            ...
            case 'GET':
                ...
                foreach ($result as $key => $value) {
                    if (!in_array($result[$key]['trackee_id'], $id_arr)) {
                        ...
                        $trackee_id_name_first_name = '';
                        if (isset($info->first_name) && $info->first_name) {
                            $trackee_id_name_first_name = $info->first_name ? $info->first_name : '';
                        }
                        $trackee_id_name_last_name = '';
                        if (isset($info->last_name) && $info->last_name) {
                            $trackee_id_name_last_name = $info->last_name ? $info->last_name : '';
                        }
                        $result[$key]['trackee_id_name'] = $trackee_id_name_first_name . ' ' . $trackee_id_name_last_name;

                        // 23-aug-2023
                        if ($result[$key]['tracker_device_id'] != null) {
                            $tracker_device_res = $this->login->getTrackerDeviceNameById($result[$key]['tracker_device_id']);
                            $result[$key]['trackee_id_name'] = $tracker_device_res['device_name'] . ' (' . $tracker_device_res['tracker_device_name'] . '-' . $tracker_device_res['device_imei'] . ')';
                        }

                        $info = $this->db->get_where('users', array('id' => $result[$key]['tracker_id']))->row(); //$value['first_name'].' '.$value['last_name'].'    ';
                        // $result[$key]['tracker_id'] = $info->row()->first_name . ' ' . $info->row()->last_name;
                        $tracker_id_name_first_name = '';
                        if (isset($info->first_name) && $info->first_name) {
                            $tracker_id_name_first_name = $info->first_name ? $info->first_name : '';
                        }
                        $tracker_id_name_last_name = '';
                        if (isset($info->last_name) && $info->last_name) {
                            $tracker_id_name_last_name = $info->last_name ? $info->last_name : '';
                        }
                        $result[$key]['tracker_id'] = $tracker_id_name_first_name . ' ' . $tracker_id_name_last_name;
    ...
    public function registerUserHealthysureDigitgrp() {
        ...
        $this->db->where('dashboard_menu_id', 12);
        $this->db->where('is_checked', '1');
        $query_tenant_privileges = $this->db->get();
        $res_arr = $query_tenant_privileges->result_array();
        $tenant_arr = array_column($res_arr, 'tenant_id');
        ...
        $this->db->where_in('tenant_id', $tenant_arr);
    ...
    public function getAllPlans($tenant_id = '') {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $datas = json_decode(file_get_contents("php://input"));
                $result = $this->login->getAllPlans($tenant_id);
                echo json_encode($result);
                break;
        }
    }
    ...
    public function getTenantLicenceCount($id = null) {
        error_reporting(1);
        error_reporting(E_ALL);
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $datas = json_decode(file_get_contents("php://input"));
                $result_admin = $this->login->getAdmin($id);
...
// }
        ...
        if ($result_user_plan) {
            foreach ($result_user_plan as $row) {
                ...
                if ($query->num_rows() == 1) {
                    ...
                    print_R(" licence_key_id => " . $licence_key_id);
                    echo "<br>"; //die;
                    $this->db->where('id', $row['id']);
                    $this->db->where('user_id', $user_id);
    ...
    public function getSettingById($id) {
        if ($id) {
            $finresults = $this->login->getSettingById($id, '1');
            if ($finresults != "") {
                $result = array('status' => 'Success', 'result' => $finresults);
                print json_encode($result);
            } else {
                $error_info = $this->login->getErrorCode('13');
...
public function getUserEvidence_TEST($tid = '', $page = '') {
        error_reporting(E_ALL);
         ini_set('display_errors', '1');
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
  ...
  echo $this->db->last_query();
                    ...
                    $result[$key]['sr_no'] = $sr_no;
                      print_r($result[$key]['followme_id']);
                      print_r($followme_id_arr );
    ...
    public function addTenantKeyword() {
        $datas = array(
            'tenant_id' => $this->input->post('admin_select_id') ? $this->input->post('admin_select_id') : null,
            'keyword' => $this->input->post('keyword') ? $this->input->post('keyword') : null,
        );
        $result = $this->login->addTenantKeyword($datas);
        echo json_encode($result);
    }

    public function viewTenantKeywordByTenantId($id) {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $datas = json_decode(file_get_contents("php://input"));
                $admin_result = $this->login->getAdminDetailById($id);
                $return_arr = array();
                $keyword_title = "";
                $tenant_keyword_result = $this->login->getTenantKeyword($id);
                if (!empty($tenant_keyword_result)) {
                    $bulletin_news_tag_title_arr = array_column($tenant_keyword_result, 'keyword');
                    $keyword_title = implode(", ", $bulletin_news_tag_title_arr);
                }
                $admin_result->keyword = $keyword_title;
                $return_arr['admin_result'] = $admin_result;
                $return_arr['tenant_keyword_result'] = $tenant_keyword_result;
                echo json_encode($return_arr);
                break;
        }
    }
    ...
    public function getTenantKeyword() {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $admin_result = $this->login->getAdmin();
                if (!empty($admin_result)) {
                    foreach ($admin_result as $key => $row) {
                        $admin_result[$key]['tenant_name'] = $row['first_name'] . " " . $row['last_name'];
                        $tenant_id = $row['uid'];
                        $result_tenant_keyword = $this->login->getTenantKeyword($tenant_id);
                        $keyword_arr_val = "";
                        if (!empty($result_tenant_keyword)) {
                            $keyword_arr = $critical_illness_id_array = array_column($result_tenant_keyword, 'keyword');
                            $keyword_arr_val = implode(", ", $keyword_arr);
                        }
                        if (strlen($keyword_arr_val) > 70) {
                            $admin_result[$key]['keyword'] = mb_substr($keyword_arr_val, 0, 70) . '...';
                        } else {
                            $admin_result[$key]['keyword'] = $keyword_arr_val;
                        }
                    }
                }
                echo json_encode($admin_result);
                break;
        }
    }
    
    public function checkTenantKeyword() {
        $datas = json_decode(file_get_contents("php://input"));
        $result = $this->login->checkTenantKeyword($datas);
        echo json_encode($result);
    }

    public function deleteTenantKeyword() {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'POST':
                $datas = json_decode(file_get_contents("php://input"));
                $result = $this->login->deleteTenantKeyword($datas);
                echo json_encode($result);
                break;
        }
    }
	public function getUserRecordForExcel() {
	    error_reporting(E_ALL);
        ini_set('display_errors', '1');
	    $this->db->select('users.id, users.first_name, users.middle_name, users.last_name, users.date_of_birth , users.mobile_no ,users.email , 
	    users.status as user_status, users.tenant_id,
	  ,users.is_deleted as user_is_deleted,  user_admin.first_name as tenant_fn,   user_admin.last_name as tenant_ln,  tenant_licence.licence_key as tenant_licence_licence_key,
	    user_plan.plan_id,   user_plan.start_date,   user_plan.end_date,  plan_master.title as plan_title, plan_master.price as plan_price
	    ');
          $this->db->join('user_admin', 'user_admin.id = users.tenant_id', 'left');
          $this->db->join('tenant_licence', 'tenant_licence.id = users.tenant_licence_id', 'left');
        ...
        // $this->db->limit('10');
	    $res = $this->db->get();
        $user_result = $res->result_array();
        
        echo 'Sr No.,User Full Name,date of birth,plan price,start date,end date,plan title,mobile_no,email,licence_key,Tenant name,user status,user deleted' . '<br>';
            
        foreach ($user_result as $k => $row) {
            $sr_no = ++$k;
            $plan_price_val = "";
            if ($row['plan_price'] == 0) {
                $plan_price_val = "Free";
            } else if ($row['plan_price'] > 1) {
                $plan_price_val = "Paid";
            }
            $user_status_val = "";
            if ($row['user_status'] == 1) {
                ...
                $user_status_val = "In-Active";
            } else if ($row['user_status'] == 3) {
                $user_status_val = "Suspended";
            }
            $user_is_deleted_val = "(Active)";
            if ($row['user_is_deleted'] == 1) {
                $user_is_deleted_val = "(Deleted)";
            }
            
            echo $sr_no . ',' . $row['first_name'] . ' ' . $row['middle_name'] . ' ' . $row['last_name'] . ',' . $row['date_of_birth'] 
             . ',' . $row['plan_price']
             . ',' . $row['start_date']
             . ',' . $row['end_date']
             . ',' . $row['plan_title']
            . ',' . $row['mobile_no'] . ',' . $row['email'] 
            . ',' . $user_status_val 
            . ',' . $row['tenant_licence_licence_key'] 
              . ',' . $row['tenant_fn'] . ' ' . $row['tenant_ln']  
              . ',' . $user_is_deleted_val
            . '<br>';
        }
        // print_r($user_result);
	...
	public function getScanQrCodeUnlinkedRecordForExcel() {
	    error_reporting(E_ALL);
        ini_set('display_errors', '1');
        
           $this->db->select('qr_code_scan_alert.id, qr_code_scan_alert.qr_code_id, qr_code_scan_alert.latitude, qr_code_scan_alert.longitude, qr_code_scan_alert.created_at, qr_code.code,  qr_code.user_id,users.first_name, users.middle_name, users.last_name, users.email ,users.status as user_status,users.is_deleted as user_is_deleted');
        ...
        // $this->db->limit('10');
        $res = $this->db->get();
       $qr_code_scan_alert_result = $res->result_array();
        // print_r($qr_code_scan_alert_result);     // die;
        
        echo 'Sr No.,qr_code_id,code,user_id,User Full Name,email,user status,user deleted' . '<br>';
            
        foreach ($qr_code_scan_alert_result as $k => $row) {
            $sr_no = ++$k;
            $user_status_val = "";
            if ($row['user_status'] == 1) {
                $user_status_val = "Active";
	...
	public function getTenantUserRecordForExcel() {
	    error_reporting(E_ALL);
        ini_set('display_errors', '1');
	    $this->db->select('users.id, users.first_name, users.middle_name, users.last_name,   users.mobile_no ,users.email , 
	    users.status as user_status, users.tenant_id,users.member_id, users.is_deleted as user_is_deleted,  users.auth_key 
	    ');
           $this->db->where('tenant_id', 26);
		 $this->db->from('users');
        //   $this->db->limit('10');
	    $res = $this->db->get();
        $user_result = $res->result_array();
        
        // echo 'Sr No.,id,Tenant Id,Member id,User Full Name,Email,Mobile_no,User status,User deleted'.'<br>';
        echo 'Sr No.,Tenant Id,Member id,User Full Name,Email,Mobile_no,User status,User deleted' . '<br>';
        foreach ($user_result as $k => $row) {
            $sr_no = ++$k;
            $user_status_val = "";
            if ($row['user_status'] == 1) {
	...
	public function getLicenceStartEndDateRecord() {
	    error_reporting(E_ALL);
	    ini_set('display_errors', '1');
	    $status = 1;
	    $tid = "";
        ...
        $result = $this->db->get('users')->result_array();
	       ...
	       foreach ($result as $value) {
	            if ($value['tenant_licence_id'] != null) {
                    ...
                    $this->db->select('user_plan.id, user_plan.user_id, user_plan.plan_id, plan_master.title,  user_plan.start_date, user_plan.end_date');
                    $this->db->from('user_plan');
                    $this->db->where('user_plan.user_id', $user_id_value);
                    $this->db->where('user_plan.is_deleted', '0');
                    $this->db->where('user_plan.status', 1);
                    $this->db->where('user_plan.licence_key_id is not null');
                    $this->db->join('plan_master', 'plan_master.id = user_plan.plan_id', 'left');
                    $this->db->order_by('user_plan.id', 'DESC');
                    $q = $this->db->get();
                    // echo $this->db->last_query();die;
                    ...
                    if ($result_user_plan) {
                        ...
                        $plan_id = $result_user_plan['plan_id'];
                        $plan_title = $result_user_plan['title'];
                        $start_date = $result_user_plan['start_date'];
                        $end_date = $result_user_plan['end_date'];
                        
                        if ($value['tenant_licence_id'] != null && $end_date_value != "") {
                            if ($end_date_value != $end_date) {
                                
                                if ($end_date == '2050-01-01') {
                                    echo "<br>";
                  
                    print_r("User Id - " . $user_id_value);
                    echo "<br>";
                    print_R("Licence Key - " . $licence_key_value);
                    echo "<br>";
                    print_R("Start date - " . $start_date_value);
                    print_R(" End date - " . $end_date_value);
                    
                      echo "<br>";
                    print_R(" result_user_plan - " . $result_user_plan['id']);
                    // print_R(" end_date_value - " .$end_date_value);
                                    //   die;
                                    $this->db->where('id', $result_user_plan['id']);
                                    $this->db->where('user_id', $user_id_value);
                                    $this->db->update('user_plan', array('end_date' => $end_date_value));
                                     echo $this->db->last_query();
                                    //   die;
                                echo "<br>";
                             
                    echo "<br>";
                    print_R("Plan id - " . $plan_id);
                    echo "<br>";
	...
	public function deleteMultimediaFile() {
        ...
        $this->db->select('user_multimedia.id, user_multimedia.panic_id, user_multimedia.file_path,  user_multimedia.created_at');
        $this->db->from('user_multimedia');
//         $this->db->where('user_multimedia.created_at',  $date);
        // $this->db->where("DATE_FORMAT(user_multimedia.created_at,'%Y-%m-%d')", $date);
            //   $this->db->where('user_multimedia.panic_id', $id);
               $this->db->where('user_multimedia.panic_id <= ', 3726);
//        $this->db->limit(1 );
        $this->db->order_by('id', 'ASC');
        $query = $this->db->get();
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $target_path = base_url() . 'uploads' . $file_path;
                $target_path = 'uploads' . $file_path;
            //   $target_path =  "..".$file_path ;
             
                // if (file_exists($target_path)) {echo "File available <br>";
                     
                     if (unlink($target_path)) {
                        echo ("$target_path has been deleted");
                           echo "<br>";
                    } else {
                        echo ("$target_path cannot be deleted due to an error");
                           echo "<br>";
                    }
        ...
        echo $this->db->last_query();
        echo "<br>";
        //https://uat.captainindia.anekalabs.com/backend//uploads//multimedia/4917074718301576005174.jpg
    }
