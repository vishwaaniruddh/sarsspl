...
//  	}
    ...
    public function checkSecurityKey() {
        ...
        $allowed_url = array('scanQrcode', 'qrcodedetail', 'sendotpviewpage', 'sendotpqrcode', 'verifyotpsdqrcode', 'qrcodeuserdetail', 'qrcodedetailbycode', 'send_NotificationToUser', 'checkReturningToLocation', 'getDeviceToRing', 'updateTrackerDeviceFinder',
            'addmissingalert', 'addQrCodeScanAlert', 'sendNotificationAdminQrCodeScan', 'addTrackerData', 'getTrackerData', 'getTestFirebaseData', 'getTestData','getTestDataDDevice', 'get_PanicRequestById','checkBattery','newget_PanicRequestByID'
            ,'testgupshup','testfirebase','sendPOSEmail'
            );
...
///ini_set('display_errors', '1');
    ...
    public function setTracking_backupcode() {
        $datas = $_POST;
        date_default_timezone_set('Asia/Kolkata');
                        ...
                        foreach ($array as $k => $v) {
                            ...
                            if ($euid) {
                                ...
                                if ($mobile_no) {
                                    ...
                                    $curl = curl_init();
                                    curl_setopt($curl, CURLOPT_URL, 'https://www.meraqr.co/api/SOSMessage');
                            ...
                            if ($user_admin_res->email != null || $user_admin_res->email != "") {
                                ...
                                if (!empty($user_multimedia_array)) {
                                    foreach ($user_multimedia_array as $row) {
                                        $_setpath = "https://uat.zimaxxtech.com/backend/uploads/". $row['file_path'];
                                        $message .= "<br>" . '<a href="' . $_setpath . '">' . $_setpath . '</a>' . "<br>";
                                    }
                                }
...
//                                echo json_encode($return);
    ...
    public function getLastLocation_22082024() {
        $data = $_POST;
        $user_id = $_POST['user_id'];
    ...
    public function getTrackerNotificationsUnreadCount(){
     ...
     public function newget_PanicRequestByID() {
        $data = $_POST;
        if($_POST){
            $panic_id = $_POST['panic_id'];
    ...
    public function get_PanicRequestByID() {
        $data = $_POST;
        $result = array('status' => 'Fail', 'message' => $data);
        print json_encode($result);
    }
    
    public function get_PanicRequestByID_backup() {
        $data = $_POST;
        if($_POST){
            $panic_id = $_POST['panic_id'];
            $this->db->select('*');
    ...
    public function testfirebase(){
       $device_token = "ep1rv-6BS2qIvjqzbGVgnY:APA91bF_Q4nZob2iyW4gDOfRZp69OOQdvG9R6QnG0qVLuIQKAEuaq2gihIGFldhS-u863l7FF6NZM29OzjVc5mIOBte8pV2Uxt5svrNfrfwCLwpP26X8emtYJfoub7-G5ftmEMAlf17o";
        // $device_token = 'ccovWy_eTdyiir44pgyfPU:APA91bGnILOpwboh4LHZRKETw9c24iZBu2EQ2vn2DL7BXfmR2-BgMCKhp8N3zDw64lPDqQgyLHdqILwLBtKH8iPdV2IzYrWjZKP6fwQq4tWpOGhX3Zmj6FU3TaFemKFM3J3LDQ2PyQ2M';
        $this->load->library('firebase');
            //$this->firebase->send_notification($trackee_info->first_name.' '.$trackee_info->last_name.' shared Live location. Click here to track.',$device_token,'Captain India','tracking' );
            //24-aug-2022
        $res = $this->firebase->sendNotify(' shared Live location. Click here to view the location.', $device_token, 'Captain India');
        $result = array('status' => 'Fail','res' => $res);
        print json_encode($result);
    }

    public function testgupshup(){
        $mobile_no = '919131185942';
        $username = '2000232718'; // Replace with your Gupshup username
                                    $password = 'NtyqmU0Qg'; // Replace with your Gupshup password
                                    $apiUrl = 'https://api.gupshup.io/sm/api/v1/msg'; // Gupshup SMS API endpoint

                                    // SMS details
                                    $phoneNumber = $mobile_no; // Replace with the recipient's phone number
                                    $message = 'Hello, this is a test message from Gupshup!'; // Replace with your message

                                    // Create the request payload
                                    $data = [
                                        'channel' => 'SMS',
                                        'source' => 'YOUR_SOURCE_NUMBER', // Replace with your Gupshup source number
    ...
    public function user_ambulance_call_list() {
        date_default_timezone_set('Asia/Kolkata');
        $data = $_POST;
        if($_POST){
            $user_id = $_POST['user_id'];
          ...
          //  $this->db->where('imei',$imei);
            $query = $this->db->get();
            $ambresult = $query->result_array();
            if($ambresult){
                
                $result = array('status' => 'Success', 'result' => $ambresult);  
            }else{
     ...
     public function user_ambulance_call_details() {
        date_default_timezone_set('Asia/Kolkata');
        $data = $_POST;
        if($_POST){
            $user_id = $_POST['user_id'];
            ...
            $this->db->from('call_ambulance');
            $this->db->where('user_id', $user_id);
            $this->db->where('crn',$request_id);
            $query = $this->db->get();
            $ambresult = $query->row_array();
            if($ambresult){
    ...
    //    $this->email->message($message);
     ...
     public function saveMultimediaNew() {
        ...
        if ($finresults && in_array($ext, $allowed)) {
            ...
            if ($_FILES['image']['name'] != '') {
                $target_path = './uploads/multimedia/new';
                    ...
                    if (!move_uploaded_file($_FILES['image']['tmp_name'][$i], $target_path_new)) {
                        ...
                        if ($_POST['content_type'] == '1') {
                            ...
                            $configer = array(
                                ...
                                'width' => 950,
                                'height' => 950,
                                'new_image' => FCPATH . '/uploads/multimedia/new/thumbnail/'
                            );
                        ...
                        // $target_thumbnail_path_url_main = $image_url_path.'thumbnail/'.$newthumbfilename;
                        // 7-feb-2023
                        $target_path_url_main = '/multimedia/new/' . $newfilename;
                        $target_thumbnail_path_url_main = '/multimedia/new/thumbnail/' . $newthumbfilename;
    ...
    public function personalCounsellingReq() {
                date_default_timezone_set('Asia/Kolkata');
                $data = $_POST;
                $user_id = $_POST['user_id'];
        // send mail to user tenant admin
                           ...
                           // $to = $user_admin_res->email;
                            $to = 'prabir.d06@gmail.com';
                            $subject = 'Personal Counselling Alert';
                            //$message = 'message';
                            $message = "Hello, " . "<br>" . "<br>";
                          //  $message .= $user_info->first_name . ' ' . $user_info->last_name . ' has raised Posh request.';
                            $message .=  'Prabir Dutta has raised Personal Counselling request.';
                         //   $message .= "<br>" . "<br>" . 'Reporter Name: ' . $_POST['reporter_name'];
                          //  $message .= "<br>" . "<br>" . 'Complainter Name: ' . $_POST['complaint_against_name'];
                        //    $message .= "<br>" . "<br>" . 'This is evidence' . "<br>";
