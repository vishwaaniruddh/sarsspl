...
class Birthday_greetings extends CI_Controller {
...
//        ini_set('display_errors', '1');
        ...
        $this->load->model('Birthday_greetings_message_model', 'birthday_greetings_message'); // Loading model 
    }
    ...
    public function getBirthdayGreetings($uid='') {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                ...
                $result = $this->birthday_greetings_message->getBirthdayGreetings($uid);
                $i = 1;
    ...
    public function deleteBirthdayGreetings() {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'POST':
                $datas = json_decode(file_get_contents("php://input"));
                $result = $this->birthday_greetings_message->deleteBirthdayGreetings($datas);
                echo json_encode($result);
                break;
        }
    }

    public function getUpdateBirthdayGreetingsInfo($id) {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $datas = json_decode(file_get_contents("php://input"));
                $result = $this->birthday_greetings_message->getBirthdayGreetingsById($id);
                if ($result) {
    ...
    public function UpdateBirthdayGreetingsInfo($id = '') {
        $datas = json_decode(file_get_contents("php://input"));
        $result = $this->birthday_greetings_message->UpdateBirthdayGreetingsInfo($datas, $id);
        echo json_encode($result);
    }

    public function getBirthdayGreetingsView($id) {
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                $datas = json_decode(file_get_contents("php://input"));
                $result = $this->birthday_greetings_message->getBirthdayGreetingsById($id);
    ...
    public function AddBirthdayGreetingsInfo() {
        $datas = json_decode(file_get_contents("php://input"));
        $result = $this->birthday_greetings_message->AddBirthdayGreetingsInfo($datas);
        echo json_encode($result);
    }

    public function downloadexcelsheetBirthdayGreetings() {
        ...
        $sheet->setCellValue('B1', 'Message Title');
        $sheet->setCellValue('C1', 'Message Description');
        ...
        $fileName = "Sample_birthday_greetings_" . time() . ".xlsx";
...
//        die;
    ...
    public function uploadexcelsheetBirthdayGreetings() {
        require 'vendor/autoload.php';
...
//  Loop through each row of the worksheet in turn
                ...
                for ($row = 1; $row <= $highestRow; $row++) {
                    ...
                    if ($row == 1) {
                        ...
                        $insert_sheet->setCellValue('B' . $row_ct, $title_col);
                        $insert_sheet->setCellValue('C' . $row_ct, $message_col);
                        if (empty($err_msg)) {
                            $result = $this->birthday_greetings_message->addBirthdayGreetingsMsg($insert_excel_data);
                            if ($result) {
                                $insert_sheet->SetCellValue($err_column . $row_ct, "");
                            }
                        } else {
                            $err_msg = implode(",", $err_msg);
                            $insert_sheet->setCellValue('D' . $row_ct, $err_msg);
                        }
                        $row_ct++;
                    }
                }
