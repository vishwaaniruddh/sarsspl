...
//  	}
 	...
 	public function checkSecurityKey() {
 		...
 		'addmissingalert', 'addQrCodeScanAlert', 'sendNotificationAdminQrCodeScan');
	...
	public function saveMultimedia_old(){ 
		...
		if($finresults  && in_array($ext,$allowed)){
           ...
           for($i=0;$i < count($_FILES['image']['name']);$i++){
	        ...
	        $notify_url = "http://captainindia.anekalabs.com/forms/evidence_view/".$_POST['panic_id'];
...
*/
    ...
    public function getTracking(){
       ...
       // echo 'date_default_timezone_set: ' . date_default_timezone_get() . '<br />';exit;
		...
		$trackee_list=array();$tracker_list=array();$key = '';$trigger_list = array();
		if($result){
			...
			foreach($result as $k=>$v){
		        ...
		        'tracker_id'=>$v->tracker_id,'start_time'=>$v->start_time,'end_time'=>$v->end_time,'share_time'=>$v->share_time,'mobile_no'=>$v->mobile_no,'user_name'=>$v->first_name.' '.$v->last_name,'user_dp'=>'','firebase_key'=>$key,'current_time'=>date('Y-m-d H:i:s'),'mode'=>$v->mode);
		...
		if($result){
    		    ...
    		    if($panic_info  != 0) {
    		        if( $panic_info->module_type == '1' ) {
        	            ...
        	            $notify_url = "http://captainindia.anekalabs.com/forms/evidence_view/".$result;
         	            $response = $this->api_model->sendMessage($message,$loc,$notify_url);
    ...
    public function addFollowmeSafe(){
		...
		if($finresults){
            ...
            if( $data['status'] =='1') {
    	        ...
    	        $loc = "https://www.google.com/maps/search/?api=1&query=".$_POST['user_lat'].",".$_POST['user_long'];
    	        $notify_url = "http://captainindia.anekalabs.com/";
...
// die;
   ...
   //29-aug-2022
    ...
    public function addQrCode() {
        ...
        if ($_POST['user_id']) {
            // check Qr Code used
            $final_result_check_code_exists = $this->api_model->checkQrCodeExists($data);
            ...
            if ($final_result) {
                ...
                if (isset($final_result['status']) && $final_result['status'] == false) {
                    ...
                    if (!empty($result_get_qrcode['mobile_no'])) {
                        ...
                        $emergency_contacts_html .= '&nbsp;<button class="button call_btn"><a href="'.$result_get_qrcode['mobile_no'].'"><i class="fa fa-phone color_white"></i></a></button>';
                    ...
                    if (!empty($result_get_qrcode['emergency_contacts'])) {
                        foreach ($result_get_qrcode['emergency_contacts'] as $row) {
                            ...
                            $emergency_contacts_html .= '&nbsp;<button class="button call_btn"><a href="'.$row['mobile_no'].'"><i class="fa fa-phone color_white"></i></a></button>';
                            $emergency_contacts_html .= '</div></div>';
                        }
                    } else {
