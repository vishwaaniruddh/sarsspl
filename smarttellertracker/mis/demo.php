...
$statement = "select a.lho, a.remarks,a.reference_code,a.id,a.bank,a.customer,a.location,a.zone,a.state,a.city,a.branch,a.created_by,a.bm,b.mis_id,b.atmid,
...
if ($assignedLho) {
  ...
  $statement .= " and a.LHO like '" . $assignedLho . "'";
  $sqlappCount .= " and a.LHO like '" . $assignedLho . "'";
} else if ($_SESSION['isVendor'] == 1) {

  $statement .= " and a.vendorId like '" . $_GLOBAL_VENDOR_ID . "'";
  $sqlappCount .= " and a.vendorId like '" . $_GLOBAL_VENDOR_ID . "'";
...
<div class="row">
  <div class="col-sm-12 grid-margin">
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
          ...
          <div class="row">
            <div class="grid-margin col-md-3">
              <label>ATMID</label>
              <input type="text" name="atmid" class="form-control" value="<?phpecho $_REQUEST['atmid']; ?>">
            </div>

            <div class="grid-margin col-md-3">
              <label>From Call Login Date</label>
              <input type="date" name="fromdt" class="form-control" value="<?phpif ($_REQUEST['fromdt']) {
            ...
            <div class="grid-margin col-md-3">
              ...
              <input type="date" name="todt" class="form-control" value="<?phpif ($_REQUEST['todt']) {
                echo $_REQUEST['todt'];
              } else {
                echo date('Y-m-d');
              } ?>">
            ...
            <div class="grid-margin col-md-3">
              ...
              <select name="call_receive_from" id="call_receive_from" class="form-control">
                ...
                <option value="Customer / Bank" <?phpif ($_REQUEST['call_receive_from'] == 'Customer / Bank') {
                  echo 'selected';
                } ?>>Customer / Bank</option>
                <option value="Internal" <?phpif ($_REQUEST['call_receive_from'] == 'Internal') {
                  echo 'selected';
                } ?>>Internal</option>
                <option value="Auto Email Call" <?phpif ($_REQUEST['call_receive_from'] == 'Auto Email Call') {
                  echo 'selected';
            ...
            <div class="col-md-12 ">
              ...
              while ($status_sql_result = mysqli_fetch_assoc($status_sql)) {
                ...
                ?>
                <input type="checkbox" name="status[]" value="<?phpecho $status_sql_result['status_code']; ?>" <?phpif (isset($_REQUEST['status'])) {
    ...
    <?phpecho $status_sql_result['status_name']; ?> &nbsp;&nbsp;
              <?
              }
              ?>
  ...
  <div class="col-sm-12 grid-margin">
    ...
    <form action="./exportMis.php" method="POST">
      <input type="hidden" name="exportSql" value="<?phpecho $statement; ?>">
      ...
      <div class="box <?php if ($detailsStatus == 'close') {
        ...
        <div class="row">
          <div class="col-sm-7 callinfo">
            <div class="title">
              <a href="viewticket.php?ticket=<?phpecho $reference_code; ?>" target="_blank">
                <?php echo $atmid; ?>
              </a>
            ...
            <?phpecho '<p style="text-align:right;"><small>' . $ProblemOccurs . '</small></p>'; ?>
            ...
            <p>
              <?phpecho $sql_result['city'] . ', ' . $sql_result['state']; ?> <br>
              <?php
...
for ($i = $start_window; $i <= $end_window; $i++) {
  ?>
  <li class="<?phpif ($i == $current_page) {
    echo 'active';
  } ?>">
    <a
      href="?page=<?phpecho $i; ?>&&atmid=<?phpecho $atmid; ?>&&<?phpecho $customer; ?>&&fromdt=<?phpecho $fromdt; ?>&&todt=<?phpecho $todt; ?>&&call_type=<?phpecho $call_type; ?>&&<?phpecho $statusQuery; ?>&&call_receive_from=<?= $call_receive_from; ?>">
      <?phpecho $i; ?>
    </a>
  </li>
