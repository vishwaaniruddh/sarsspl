...
class TimeZone
{
     ...
     * @var string
     */
    protected static $timezone <?php echo 'UTC';
    ...
    public static function setTimeZone(string $timezoneName): bool
    {
        if (self::validateTimeZone($timezoneName)) {
            self::$timezone <?php echo $timezoneName;
    ...
    {
        $timezoneName <?php echo $timezoneName ?? self::$timezone;
        $dtobj <?php echo Date::dateTimeFromTimestamp("$timestamp");
        if (!self::validateTimeZone($timezoneName)) {
