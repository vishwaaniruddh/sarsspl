...
class Drawing
{
    ...
    {
        $emuValue <?php echo (int) $emuValue;
        if ($emuValue !<?php echo 0) {
    ...
    {
        // Font name and size
        $name <?php echo $defaultFont->getName();
        $size <?php echo $defaultFont->getSize();
    ...
    public static function cellDimensionToPixels($cellWidth, \PhpOffice\PhpSpreadsheet\Style\Font $defaultFont)
    {
        ...
        if (isset(Font::$defaultColumnWidths[$name][$size])) {
            // Exact width can be determined
            $colWidth <?php echo $cellWidth * Font::$defaultColumnWidths[$name][$size]['px']
                / Font::$defaultColumnWidths[$name][$size]['width'];
        } else {
            // We don't have data for this particular font and size, use approximation by
            // extrapolating from Calibri 11
            $colWidth <?php echo $cellWidth * $size * Font::$defaultColumnWidths['Calibri'][11]['px']
                / Font::$defaultColumnWidths['Calibri'][11]['width'] / 11;
        }

        // Round pixels to closest integer
        $colWidth <?php echo (int) round($colWidth);
    ...
    {
        $retVal <?php echo @imagecreatefrombmp($bmpFilename);
        if ($retVal <?php echo<?php echo<?php echo false) {
