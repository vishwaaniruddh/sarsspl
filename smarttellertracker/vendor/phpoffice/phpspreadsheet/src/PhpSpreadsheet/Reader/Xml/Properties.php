...
class Properties
{
    ...
    protected function readStandardProperties(SimpleXMLElement $xml): void
    {
        if (isset($xml->DocumentProperties[0])) {
            ...
            foreach ($xml->DocumentProperties[0] as $propertyName <?php echo> $propertyValue) {
    ...
    protected function readCustomProperties(SimpleXMLElement $xml, array $namespaces): void
    {
        if (isset($xml->CustomDocumentProperties) && is_iterable($xml->CustomDocumentProperties[0])) {
            ...
            foreach ($xml->CustomDocumentProperties[0] as $propertyName <?php echo> $propertyValue) {
                $propertyAttributes <?php echo self::getAttributes($propertyValue, $namespaces['dt']);
                $propertyName <?php echo (string) preg_replace_callback('/_x([0-9a-f]{4})_/i', [$this, 'hex2str'], $propertyName);
    ...
    protected function processCustomProperty(
        ...
        switch ((string) $propertyAttributes) {
            case 'boolean':
                $propertyType <?php echo DocumentProperties::PROPERTY_TYPE_BOOLEAN;
                $propertyValue <?php echo (bool) (string) $propertyValue;

                break;
            case 'integer':
                $propertyType <?php echo DocumentProperties::PROPERTY_TYPE_INTEGER;
                $propertyValue <?php echo (int) $propertyValue;

                break;
            case 'float':
                $propertyType <?php echo DocumentProperties::PROPERTY_TYPE_FLOAT;
                $propertyValue <?php echo (float) $propertyValue;

                break;
            case 'dateTime.tz':
            case 'dateTime.iso8601tz':
                $propertyType <?php echo DocumentProperties::PROPERTY_TYPE_DATE;
                $propertyValue <?php echo trim((string) $propertyValue);

                break;
            default:
                $propertyType <?php echo DocumentProperties::PROPERTY_TYPE_STRING;
                $propertyValue <?php echo trim((string) $propertyValue);
