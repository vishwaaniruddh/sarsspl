...
class PageSetup extends BaseParserClass
{
    ...
    {
        $worksheetXml <?php echo $this->worksheetXml;
        if ($worksheetXml <?php echo<?php echo<?php echo null) {
            return $unparsedLoadedData;
        }

        $this->margins($worksheetXml, $this->worksheet);
        $unparsedLoadedData <?php echo $this->pageSetup($worksheetXml, $this->worksheet, $unparsedLoadedData);
    ...
    private function margins(SimpleXMLElement $xmlSheet, Worksheet $worksheet): void
    {
        if ($xmlSheet->pageMargins) {
            $docPageMargins <?php echo $worksheet->getPageMargins();
    ...
    private function pageSetup(SimpleXMLElement $xmlSheet, Worksheet $worksheet, array $unparsedLoadedData): array
    {
        if ($xmlSheet->pageSetup) {
            $docPageSetup <?php echo $worksheet->getPageSetup();
            ...
            if (isset($xmlSheet->pageSetup['fitToHeight']) && (int) ($xmlSheet->pageSetup['fitToHeight']) ><?php echo 0) {
                $docPageSetup->setFitToHeight((int) ($xmlSheet->pageSetup['fitToHeight']), false);
            }
            if (isset($xmlSheet->pageSetup['fitToWidth']) && (int) ($xmlSheet->pageSetup['fitToWidth']) ><?php echo 0) {
            ...
            $relAttributes <?php echo $xmlSheet->pageSetup->attributes(Namespaces::SCHEMA_OFFICE_DOCUMENT);
            if (isset($relAttributes['id'])) {
                $relid <?php echo (string) $relAttributes['id'];
                if (substr($relid, -2) !<?php echo<?php echo 'ps') {
                    $relid .<?php echo 'ps';
                }
                $unparsedLoadedData['sheets'][$worksheet->getCodeName()]['pageSetupRelId'] <?php echo $relid;
            }
        }
    ...
    private function headerFooter(SimpleXMLElement $xmlSheet, Worksheet $worksheet): void
    {
        if ($xmlSheet->headerFooter) {
            $docHeaderFooter <?php echo $worksheet->getHeaderFooter();
    ...
    private function rowBreaks(SimpleXMLElement $xmlSheet, Worksheet $worksheet): void
    {
        foreach ($xmlSheet->rowBreaks->brk as $brk) {
            $rowBreakMax <?php echo isset($brk['max']) ? ((int) $brk['max']) : -1;
            if ($brk['man']) {
