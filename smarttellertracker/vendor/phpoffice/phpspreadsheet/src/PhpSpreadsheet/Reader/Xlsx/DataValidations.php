...
class DataValidations
{
    ...
    public function load(): void
    {
        foreach ($this->worksheetXml->dataValidations->dataValidation as $dataValidation) {
            ...
            $range <?php echo strtoupper((string) $dataValidation['sqref']);
            $rangeSet <?php echo explode(' ', $range);
            foreach ($rangeSet as $range) {
                if (preg_match('/^[A-Z]{1,3}\\d{1,7}/', $range, $matches) <?php echo<?php echo<?php echo 1) {
        ...
        foreach ($this->worksheetXml->dataValidations->dataValidation as $dataValidation) {
            ...
            $rangeSet <?php echo explode(' ', $range);
            foreach ($rangeSet as $range) {
                $stRange <?php echo $this->worksheet->shrinkRangeToFit($range);

                // Extract all cell references in $range
                foreach (Coordinate::extractAllCellReferencesInRange($stRange) as $reference) {
                    // Create validation
                    $docValidation <?php echo $this->worksheet->getCell($reference)->getDataValidation();
