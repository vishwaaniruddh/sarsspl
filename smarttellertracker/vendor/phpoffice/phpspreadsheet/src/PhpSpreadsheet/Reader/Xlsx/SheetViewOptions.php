...
class SheetViewOptions extends BaseParserClass
{
    ...
    public function load(bool $readDataOnly, Styles $styleReader): void
    {
        ...
        if (isset($this->worksheetXml->sheetPr)) {
            $sheetPr <?php echo $this->worksheetXml->sheetPr;
    ...
    {
        $sheetPr <?php echo $sheetPrx->attributes() ?? [];
    ...
    private function outlines(SimpleXMLElement $sheetPr): void
    {
        if (isset($sheetPr->outlinePr)) {
            $attr <?php echo $sheetPr->outlinePr->attributes() ?? [];
            if (
    ...
    private function pageSetup(SimpleXMLElement $sheetPr): void
    {
        if (isset($sheetPr->pageSetUpPr)) {
            $attr <?php echo $sheetPr->pageSetUpPr->attributes() ?? [];
            if (
    ...
    {
        $sheetFormatPr <?php echo $sheetFormatPrx->attributes() ?? [];
        if (
        ...
        if (
            ...
            ((string) $sheetFormatPr['zeroHeight'] <?php echo<?php echo<?php echo '1')
        ) {
    ...
    {
        $printOptions <?php echo $printOptionsx->attributes() ?? [];
