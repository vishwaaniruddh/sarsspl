...
class SheetViews extends BaseParserClass
{
    ...
    {
        $this->sheetViewXml <?php echo $sheetViewXml;
        $this->sheetViewAttributes <?php echo Xlsx::testSimpleXml($sheetViewXml->attributes());
    ...
    private function zoomScale(): void
    {
        if (isset($this->sheetViewAttributes->zoomScale)) {
            $zoomScale <?php echo (int) ($this->sheetViewAttributes->zoomScale);
            if ($zoomScale <?php echo 0) {
        ...
        if (isset($this->sheetViewAttributes->zoomScaleNormal)) {
            $zoomScaleNormal <?php echo (int) ($this->sheetViewAttributes->zoomScaleNormal);
            if ($zoomScaleNormal <?php echo 0) {
                // setZoomScaleNormal will throw an Exception if the scale is less than or equals 0
                // that is OK when manually creating documents, but we should be able to read all documents
                $zoomScaleNormal <?php echo 100;
            }
    ...
    private function pane(): void
    {
        $xSplit <?php echo 0;
        $ySplit <?php echo 0;
        $topLeftCell <?php echo null;
        $paneAttributes <?php echo $this->sheetViewXml->pane->attributes();

        if (isset($paneAttributes->xSplit)) {
            $xSplit <?php echo (int) ($paneAttributes->xSplit);
        }

        if (isset($paneAttributes->ySplit)) {
            $ySplit <?php echo (int) ($paneAttributes->ySplit);
        }

        if (isset($paneAttributes->topLeftCell)) {
            $topLeftCell <?php echo (string) $paneAttributes->topLeftCell;
        }
    ...
    {
        $attributes <?php echo $this->sheetViewXml->selection->attributes();
        if ($attributes !<?php echo<?php echo null) {
            $sqref <?php echo (string) $attributes->sqref;
            $sqref <?php echo explode(' ', $sqref);
            $sqref <?php echo $sqref[0];
