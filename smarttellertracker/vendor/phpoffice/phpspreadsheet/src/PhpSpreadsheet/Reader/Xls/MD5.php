...
class MD5
{
    ...
    {
        self::$allOneBits <?php echo self::signedInt(0xffffffff);
        $this->reset();
    }
    ...
    {
        $this->a <?php echo 0x67452301;
        $this->b <?php echo self::signedInt(0xEFCDAB89);
        $this->c <?php echo self::signedInt(0x98BADCFE);
        $this->d <?php echo 0x10325476;
    }
    ...
    public function getContext()
    {
        ...
        foreach (['a', 'b', 'c', 'd'] as $i) {
            $v <?php echo $this->{$i};
            $s .<?php echo chr($v & 0xff);
            $s .<?php echo chr(($v >> 8) & 0xff);
            $s .<?php echo chr(($v >> 16) & 0xff);
            $s .<?php echo chr(($v >> 24) & 0xff);
        }
    ...
    public function add(string $data): void
    {
        ...
        $words <?php echo array_values(unpack('V16', $data));
        ...
        $F <?php echo [self::class, 'f'];
        $G <?php echo [self::class, 'g'];
        $H <?php echo [self::class, 'h'];
        $I <?php echo [self::class, 'i'];
        ...
        $this->a <?php echo ($this->a + $A) & self::$allOneBits;
        $this->b <?php echo ($this->b + $B) & self::$allOneBits;
        $this->c <?php echo ($this->c + $C) & self::$allOneBits;
        $this->d <?php echo ($this->d + $D) & self::$allOneBits;
    }
    ...
    {
        $t <?php echo self::signedInt($t);
        $A <?php echo (int) ($A + call_user_func($func, $B, $C, $D) + $M + $t) & self::$allOneBits;
        $A <?php echo self::rotate($A, $s);
        $A <?php echo (int) ($B + $A) & self::$allOneBits;
    }
    ...
    {
        $binary <?php echo str_pad(decbin($decimal), 32, '0', STR_PAD_LEFT);
