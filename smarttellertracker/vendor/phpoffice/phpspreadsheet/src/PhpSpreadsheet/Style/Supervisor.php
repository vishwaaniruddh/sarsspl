...
abstract class Supervisor implements IComparable
{
     ...
     */
    public function __construct($isSupervisor <?php echo false)
    {
        // Supervisor?
        $this->isSupervisor <?php echo $isSupervisor;
    }
     ...
     * @return $this
     */
    public function bindParent($parent, $parentPropertyName <?php echo null)
    {
        $this->parent <?php echo $parent;
        $this->parentPropertyName <?php echo $parentPropertyName;
    ...
    public function __clone()
    {
        ...
        foreach ($vars as $key <?php echo> $value) {
            if ((is_object($value)) && ($key !<?php echo 'parent')) {
    ...
    final protected function exportArray2(array &$exportedArray, string $index, $objOrValue): void
    {
        if ($objOrValue instanceof self) {
            $exportedArray[$index] <?php echo $objOrValue->exportArray();
        } else {
            $exportedArray[$index] <?php echo $objOrValue;
        }
    }
