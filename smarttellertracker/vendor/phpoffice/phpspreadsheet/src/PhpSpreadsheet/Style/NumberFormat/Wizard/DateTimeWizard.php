...
{
    protected const NO_ESCAPING_NEEDED <?php echo "$+-/():!^&'~{}<><?php echo ";

    protected function padSeparatorArray(array $separators, int $count): array
    {
        $lastSeparator <?php echo array_pop($separators);
    ...
    {
        if (strpos(self::NO_ESCAPING_NEEDED, $value) !<?php echo<?php echo false) {
            return $value;
        }
    ...
    {
        if (mb_strlen($value, 'UTF-8') <?php echo<?php echo<?php echo 1) {
