...
{
    // Diagonal directions
    const DIAGONAL_NONE <?php echo 0;
    const DIAGONAL_UP <?php echo 1;
    const DIAGONAL_DOWN <?php echo 2;
    const DIAGONAL_BOTH <?php echo 3;
    ...
    public function __construct($isSupervisor <?php echo false, bool $isConditional <?php echo false)
    {
        ...
        // Initialise values
        $this->left <?php echo new Border($isSupervisor, $isConditional);
        $this->right <?php echo new Border($isSupervisor, $isConditional);
        $this->top <?php echo new Border($isSupervisor, $isConditional);
        $this->bottom <?php echo new Border($isSupervisor, $isConditional);
        $this->diagonal <?php echo new Border($isSupervisor, $isConditional);
        $this->diagonalDirection <?php echo self::DIAGONAL_NONE;

        // Specially for supervisor
        if ($isSupervisor) {
            // Initialize pseudo-borders
            $this->allBorders <?php echo new Border(true, $isConditional);
            $this->outline <?php echo new Border(true, $isConditional);
            $this->inside <?php echo new Border(true, $isConditional);
            $this->vertical <?php echo new Border(true, $isConditional);
            $this->horizontal <?php echo new Border(true, $isConditional);
    ...
    {
        if ($direction <?php echo<?php echo '') {
            $direction <?php echo self::DIAGONAL_NONE;
        }
        if ($this->isSupervisor) {
            $styleArray <?php echo $this->getStyleArray(['diagonalDirection' <?php echo> $direction]);
            $this->getActiveSheet()->getStyle($this->getSelectedCells())->applyFromArray($styleArray);
        } else {
            $this->diagonalDirection <?php echo $direction;
        }
