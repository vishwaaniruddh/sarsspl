...
class Meta extends WriterPart
{
    ...
    public function write(): string
    {
        ...
        $created <?php echo $spreadsheet->getProperties()->getCreated();
        $date <?php echo Date::dateTimeFromTimestamp("$created");
        $date->setTimeZone(Date::getDefaultOrLocalTimeZone());
        $objWriter->writeElement('meta:creation-date', $date->format(DATE_W3C));
        $created <?php echo $spreadsheet->getProperties()->getModified();
        $date <?php echo Date::dateTimeFromTimestamp("$created");
        $date->setTimeZone(Date::getDefaultOrLocalTimeZone());
        ...
        //$keywords <?php echo explode(' ', $spreadsheet->getProperties()->getKeywords());
        //foreach ($keywords as $keyword) {
        //    $objWriter->writeElement('meta:keyword', $keyword);
        //}

        //<meta:document-statistic meta:table-count<?php echo"XXX" meta:cell-count<?php echo"XXX" meta:object-count<?php echo"XXX"/>
    ...
    private static function writeDocPropsCustom(XMLWriter $objWriter, Spreadsheet $spreadsheet): void
    {
        ...
        foreach ($customPropertyList as $key <?php echo> $customProperty) {
            $propertyValue <?php echo $spreadsheet->getProperties()->getCustomPropertyValue($customProperty);
            $propertyType <?php echo $spreadsheet->getProperties()->getCustomPropertyType($customProperty);
            ...
            switch ($propertyType) {
                ...
                case Properties::PROPERTY_TYPE_DATE:
                    ...
                    $dtobj <?php echo Date::dateTimeFromTimestamp($propertyValue ?? 0);
