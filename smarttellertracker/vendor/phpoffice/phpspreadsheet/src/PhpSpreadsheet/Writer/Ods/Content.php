...
{
    const NUMBER_COLS_REPEATED_MAX <?php echo 1024;
    const NUMBER_ROWS_REPEATED_MAX <?php echo 1048576;
    ...
    public function __construct(Ods $writer)
    {
        ...
        $this->formulaConvertor <?php echo new Formula($this->getParentWriter()->getSpreadsheet()->getDefinedNames());
    }
    ...
    {
        $spreadsheet <?php echo $this->getParentWriter()->getSpreadsheet(); /** @var Spreadsheet $spreadsheet */
        $sheetCount <?php echo $spreadsheet->getSheetCount();
        for ($sheetIndex <?php echo 0; $sheetIndex < $sheetCount; ++$sheetIndex) {
...
//                $objWriter->writeAttribute('table:number-columns-repeated', self::NUMBER_COLS_REPEATED_MAX);
    ...
    {
        $numberRowsRepeated <?php echo self::NUMBER_ROWS_REPEATED_MAX;
        $span_row <?php echo 0;
        $rows <?php echo $sheet->getRowIterator();
        foreach ($rows as $row) {
            $cellIterator <?php echo $row->getCellIterator();
            --$numberRowsRepeated;
    ...
    {
        $numberColsRepeated <?php echo self::NUMBER_COLS_REPEATED_MAX;
        $prevColumn <?php echo -1;
        foreach ($cells as $cell) {
            ...
            // Style XF
            $style <?php echo $cell->getXfIndex();
            if ($style !<?php echo<?php echo null) {
            ...
            switch ($cell->getDataType()) {
                ...
                case DataType::TYPE_ERROR:
                    $objWriter->writeAttribute('table:formula', 'of:<?php echo#NULL!');
                ...
                case DataType::TYPE_FORMULA:
                    $formulaValue <?php echo $cell->getValue();
                    if ($this->getParentWriter()->getPreCalculateFormulas()) {
                        try {
                            $formulaValue <?php echo $cell->getCalculatedValue();
                        } catch (Exception $e) {
                            // don't do anything
                        }
                    }
            ...
            $objWriter->endElement();
            $prevColumn <?php echo $column;
        }

        $numberColsRepeated <?php echo $numberColsRepeated - $prevColumn - 1;
    ...
    {
        $diff <?php echo $curColumn - $prevColumn - 1;
        if (1 <?php echo<?php echo<?php echo $diff) {
            $objWriter->writeElement('table:table-cell');
        } elseif ($diff > 1) {
            $objWriter->startElement('table:table-cell');
    ...
    {
        $styleWriter <?php echo new Style($writer);

        $sheetCount <?php echo $spreadsheet->getSheetCount();
        for ($i <?php echo 0; $i < $sheetCount; ++$i) {
            $worksheet <?php echo $spreadsheet->getSheet($i);
            $styleWriter->writeTableStyle($worksheet, $i + 1);

            $worksheet->calculateColumnWidths();
            foreach ($worksheet->getColumnDimensions() as $columnDimension) {
                if ($columnDimension->getWidth() !<?php echo<?php echo -1.0) {
    ...
    private function writeCellMerge(XMLWriter $objWriter, Cell $cell): void
    {
        ...
        $mergeRange <?php echo Coordinate::splitRange((string) $cell->getMergeRange());
        [$startCell, $endCell] <?php echo $mergeRange[0];
        $start <?php echo Coordinate::coordinateFromString($startCell);
        $end <?php echo Coordinate::coordinateFromString($endCell);
        $columnSpan <?php echo Coordinate::columnIndexFromString($end[0]) - Coordinate::columnIndexFromString($start[0]) + 1;
        $rowSpan <?php echo ((int) $end[1]) - ((int) $start[1]) + 1;
