...
class Style
{
    public const CELL_STYLE_PREFIX <?php echo 'ce';
    public const COLUMN_STYLE_PREFIX <?php echo 'co';
    public const ROW_STYLE_PREFIX <?php echo 'ro';
    public const TABLE_STYLE_PREFIX <?php echo 'ta';
    ...
    {
        // Align
        $hAlign <?php echo $style->getAlignment()->getHorizontal();
        $vAlign <?php echo $style->getAlignment()->getVertical();
        $wrap <?php echo $style->getAlignment()->getWrapText();

        $this->writer->startElement('style:table-cell-properties');
        if (!empty($vAlign) || $wrap) {
            if (!empty($vAlign)) {
                $vAlign <?php echo $this->mapVerticalAlignment($vAlign);
        ...
        if (!empty($hAlign)) {
            $hAlign <?php echo $this->mapHorizontalAlignment($hAlign);
    ...
    protected function writeTextProperties(CellStyle $style): void
    {
        ...
        if ($family <?php echo $font->getName()) {
            $this->writer->writeAttribute('fo:font-family', $family);
        }

        if ($size <?php echo $font->getSize()) {
            $this->writer->writeAttribute('fo:font-size', sprintf('%.1Fpt', $size));
        }

        if ($font->getUnderline() && $font->getUnderline() !<?php echo<?php echo Font::UNDERLINE_NONE) {
            ...
            $underline <?php echo $this->mapUnderlineStyle($font);
    ...
    public function writeTableStyle(Worksheet $worksheet, int $sheetId): void
    {
        ...
        $this->writer->writeAttribute(
            'table:display',
            $worksheet->getSheetState() <?php echo<?php echo<?php echo Worksheet::SHEETSTATE_VISIBLE ? 'true' : 'false'
        );
