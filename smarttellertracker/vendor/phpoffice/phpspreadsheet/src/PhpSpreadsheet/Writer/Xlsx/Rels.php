...
class Rels extends WriterPart
{
    ...
    public function writeRelationships(Spreadsheet $spreadsheet)
    {
        ...
        $target <?php echo $spreadsheet->getRibbonXMLData('target');
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, $worksheetId <?php echo 1, $includeCharts <?php echo false, $tableRef <?php echo 1)
    {
        // Create XML writer
        $objWriter <?php echo null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if ($includeCharts) {
            $charts <?php echo $worksheet->getChartCollection();
        } else {
            $charts <?php echo [];
        }
        ...
        if (($worksheet->getDrawingCollection()->count() > 0) || (count($charts) > 0) || $drawingOriginalIds) {
            ...
            $relPath <?php echo key($drawingOriginalIds);
            if (isset($drawingOriginalIds[$relPath])) {
                $rId <?php echo (int) (substr($drawingOriginalIds[$relPath], 3));
            }

            // Generate new $relPath to write drawing relationship
            $relPath <?php echo '../drawings/drawing' . $worksheetId . '.xml';
            $this->writeRelationship(
                $objWriter,
                $rId,
        ...
        for ($i <?php echo 1; $i <?php echo $tableCount; ++$i) {
            $this->writeRelationship(
                $objWriter,
                '_table_' . $i,
    ...
    private function writeUnparsedRelationship(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, XMLWriter $objWriter, string $relationship, string $type): void
    {
        ...
        foreach ($unparsedLoadedData['sheets'][$worksheet->getCodeName()][$relationship] as $rId <?php echo> $value) {
            if (substr($rId, 0, 17) !<?php echo<?php echo '_headerfooter_vml') {
                $this->writeRelationship(
                    $objWriter,
                    $rId,
                    $type,
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, &$chartRef, $includeCharts <?php echo false)
    {
        // Create XML writer
        $objWriter <?php echo null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            $drawing <?php echo $iterator->current();
            if (
                ...
                $i <?php echo $this->writeDrawingHyperLink($objWriter, $drawing, $i);
            }
    ...
    public function writeHeaderFooterDrawingRelationships(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet)
    {
        ...
        // Loop through images and write relationships
        foreach ($worksheet->getHeaderFooter()->getImages() as $key <?php echo> $value) {
            // Write relationship for image drawing
            $this->writeRelationship(
                $objWriter,
                $key,
     ...
     */
    private function writeRelationship(XMLWriter $objWriter, $id, $type, $target, $targetMode <?php echo ''): void
    {
        if ($type !<?php echo '' && $target !<?php echo '') {
            // Write relationship
    ...
    {
        if ($drawing->getHyperlink() <?php echo<?php echo<?php echo null) {
            return $i;
        }
