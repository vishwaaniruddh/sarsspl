...
class Comments extends WriterPart
{
    ...
    public function writeComments(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet)
    {
        ...
        // Comments cache
        $comments <?php echo $worksheet->getComments();
        ...
        foreach ($comments as $comment) {
            if (!isset($authors[$comment->getAuthor()])) {
                $authors[$comment->getAuthor()] <?php echo $authorId++;
            }
        }
        ...
        foreach ($authors as $author <?php echo> $index) {
        ...
        foreach ($comments as $key <?php echo> $value) {
    ...
    {
        // Metadata
        [$column, $row] <?php echo Coordinate::indexesFromString($cellReference);
        $id <?php echo 1024 + $column + $row;
        $id <?php echo substr("$id", 0, 4);
