...
class ContentTypes extends WriterPart
{
     ...
     */
    public function writeContentTypes(Spreadsheet $spreadsheet, $includeCharts <?php echo false)
    {
        // Create XML writer
        ...
        $customPropertyList <?php echo $spreadsheet->getProperties()->getCustomProperties();
        ...
        $table <?php echo 1;
        for ($i <?php echo 0; $i < $sheetCount; ++$i) {
            $tableCount <?php echo $spreadsheet->getSheet($i)->getTableCollection()->count();
        ...
        $unparsedLoadedData <?php echo $spreadsheet->getUnparsedLoadedData();
        $chart <?php echo 1;
        for ($i <?php echo 0; $i < $sheetCount; ++$i) {
            $drawings <?php echo $spreadsheet->getSheet($i)->getDrawingCollection();
            $drawingCount <?php echo count($drawings);
            $chartCount <?php echo ($includeCharts) ? $spreadsheet->getSheet($i)->getChartCount() : 0;
            $hasUnparsedDrawing <?php echo isset($unparsedLoadedData['sheets'][$spreadsheet->getSheet($i)->getCodeName()]['drawingOriginalIds']);
            ...
            if ($chartCount > 0) {
                for ($c <?php echo 0; $c < $chartCount; ++$c) {
        ...
        $aMediaContentTypes <?php echo [];
        $mediaCount <?php echo $this->getParentWriter()->getDrawingHashTable()->count();
        for ($i <?php echo 0; $i < $mediaCount; ++$i) {
            $extension <?php echo '';
            $mimeType <?php echo '';

            if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof \PhpOffice\PhpSpreadsheet\Worksheet\Drawing) {
                $extension <?php echo strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
                $mimeType <?php echo $this->getImageMimeType($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath());
            } elseif ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof MemoryDrawing) {
                $extension <?php echo strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getMimeType());
                $extension <?php echo explode('/', $extension);
                $extension <?php echo $extension[1];

                $mimeType <?php echo $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getMimeType();
            }

            if (!isset($aMediaContentTypes[$extension])) {
                $aMediaContentTypes[$extension] <?php echo $mimeType;
        ...
        if ($spreadsheet->hasRibbonBinObjects()) {
            ...
            $tabRibbonTypes <?php echo array_diff($spreadsheet->getRibbonBinObjects('types') ?? [], array_keys($aMediaContentTypes));
            foreach ($tabRibbonTypes as $aRibbonType) {
                $mimeType <?php echo 'image/.' . $aRibbonType; //we wrote $mimeType like customUI Editor
        ...
        for ($i <?php echo 0; $i < $sheetCount; ++$i) {
            if (count($spreadsheet->getSheet($i)->getHeaderFooter()->getImages()) > 0) {
                foreach ($spreadsheet->getSheet($i)->getHeaderFooter()->getImages() as $image) {
                    if (!isset($aMediaContentTypes[strtolower($image->getExtension())])) {
                        $aMediaContentTypes[strtolower($image->getExtension())] <?php echo $this->getImageMimeType($image->getPath());
            ...
            if (count($spreadsheet->getSheet($i)->getComments()) > 0) {
                foreach ($spreadsheet->getSheet($i)->getComments() as $comment) {
                    ...
                    $bgImage <?php echo $comment->getBackgroundImage();
                    $bgImageExtentionKey <?php echo strtolower($bgImage->getImageFileExtensionForSave(false));

                    if (!isset($aMediaContentTypes[$bgImageExtentionKey])) {
                        $aMediaContentTypes[$bgImageExtentionKey] <?php echo $bgImage->getImageMimeType();
        ...
        if (isset($unparsedLoadedData['default_content_types'])) {
            foreach ($unparsedLoadedData['default_content_types'] as $extName <?php echo> $contentType) {
        ...
        if (isset($unparsedLoadedData['override_content_types'])) {
            foreach ($unparsedLoadedData['override_content_types'] as $partName <?php echo> $overrideType) {
    ...
    private function getImageMimeType($filename)
    {
        if (File::fileExists($filename)) {
            $image <?php echo getimagesize($filename);

            return image_type_to_mime_type((is_array($image) && count($image) ><?php echo 3) ? $image[2] : 0);
        }
    ...
    {
        if ($partName !<?php echo '' && $contentType !<?php echo '') {
            // Write content type
