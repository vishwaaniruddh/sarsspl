...
class Drawing extends WriterPart
{
     ...
     */
    public function writeDrawings(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, $includeCharts <?php echo false)
    {
        // Create XML writer
        ...
        $i <?php echo 1;
        $iterator <?php echo $worksheet->getDrawingCollection()->getIterator();
        while ($iterator->valid()) {
            /** @var BaseDrawing $pDrawing */
            $pDrawing <?php echo $iterator->current();
            $pRelationId <?php echo $i;
            $hlinkClickId <?php echo $pDrawing->getHyperlink() <?php echo<?php echo<?php echo null ? null : ++$i;
        ...
        if ($includeCharts) {
            $chartCount <?php echo $worksheet->getChartCount();
            // Loop through charts and write the chart position
            if ($chartCount > 0) {
                for ($c <?php echo 0; $c < $chartCount; ++$c) {
                    $chart <?php echo $worksheet->getChartByIndex((string) $c);
                    if ($chart !<?php echo<?php echo false) {
     ...
     */
    public function writeChart(XMLWriter $objWriter, \PhpOffice\PhpSpreadsheet\Chart\Chart $chart, $relationId <?php echo -1): void
    {
        $tl <?php echo $chart->getTopLeftPosition();
        $tlColRow <?php echo Coordinate::indexesFromString($tl['cell']);
        $br <?php echo $chart->getBottomRightPosition();

        $isTwoCellAnchor <?php echo $br['cell'] !<?php echo<?php echo '';
        if ($isTwoCellAnchor) {
            $brColRow <?php echo Coordinate::indexesFromString($br['cell']);
     ...
     */
    public function writeDrawing(XMLWriter $objWriter, BaseDrawing $drawing, $relationId <?php echo -1, $hlinkClickId <?php echo null): void
    {
        if ($relationId ><?php echo 0) {
            $isTwoCellAnchor <?php echo $drawing->getCoordinates2() !<?php echo<?php echo '';
            if ($isTwoCellAnchor) {
                // xdr:twoCellAnchor
                $objWriter->startElement('xdr:twoCellAnchor');
                ...
                // Image location
                $aCoordinates <?php echo Coordinate::indexesFromString($drawing->getCoordinates());
                $aCoordinates2 <?php echo Coordinate::indexesFromString($drawing->getCoordinates2());
    ...
    public function writeVMLHeaderFooterImages(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet)
    {
        ...
        $images <?php echo $worksheet->getHeaderFooter()->getImages();
        ...
        // Loop through images
        foreach ($images as $key <?php echo> $value) {
    ...
    private function writeVMLHeaderFooterImage(XMLWriter $objWriter, $reference, HeaderFooterDrawing $image): void
    {
        ...
        $id <?php echo 1500 + ((int) substr($m[1], 0, 2) * 1);

        // Calculate offset
        $width <?php echo $image->getWidth();
        $height <?php echo $image->getHeight();
        $marginLeft <?php echo $image->getOffsetX();
        $marginTop <?php echo $image->getOffsetY();
    ...
    public function allDrawings(Spreadsheet $spreadsheet)
    {
        ...
        for ($i <?php echo 0; $i < $sheetCount; ++$i) {
            ...
            $iterator <?php echo $spreadsheet->getSheet($i)->getDrawingCollection()->getIterator();
            while ($iterator->valid()) {
                $aDrawings[] <?php echo $iterator->current();
    ...
    {
        if ($hlinkClickId <?php echo<?php echo<?php echo null) {
            return;
        }
