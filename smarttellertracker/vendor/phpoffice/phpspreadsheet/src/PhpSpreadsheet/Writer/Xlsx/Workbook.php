...
class Workbook extends WriterPart
{
     ...
     */
    public function writeWorkbook(Spreadsheet $spreadsheet, $recalcRequired <?php echo false)
    {
        // Create XML writer
        ...
        // bookViews
        if ($this->getParentWriter()->getOffice2003Compatibility() <?php echo<?php echo<?php echo false) {
    ...
    private function writeWorkbookPr(XMLWriter $objWriter): void
    {
        ...
        if (Date::getExcelCalendar() <?php echo<?php echo<?php echo Date::CALENDAR_MAC_1904) {
    ...
    private function writeWorkbookProtection(XMLWriter $objWriter, Spreadsheet $spreadsheet): void
    {
        if ($spreadsheet->getSecurity()->isSecurityEnabled()) {
            ...
            if ($spreadsheet->getSecurity()->getRevisionsPassword() !<?php echo '') {
                $objWriter->writeAttribute('revisionsPassword', $spreadsheet->getSecurity()->getRevisionsPassword());
            }

            if ($spreadsheet->getSecurity()->getWorkbookPassword() !<?php echo '') {
     ...
     * @param bool $recalcRequired Indicate whether formulas should be recalculated before writing
     */
    private function writeCalcPr(XMLWriter $objWriter, $recalcRequired <?php echo true): void
    {
     ...
     */
    private function writeSheet(XMLWriter $objWriter, $worksheetName, $worksheetId <?php echo 1, $relId <?php echo 1, $sheetState <?php echo 'visible'): void
    {
        if ($worksheetName !<?php echo '') {
            // Write sheet
            $objWriter->startElement('sheet');
            $objWriter->writeAttribute('name', $worksheetName);
            $objWriter->writeAttribute('sheetId', (string) $worksheetId);
            if ($sheetState !<?php echo<?php echo 'visible' && $sheetState !<?php echo '') {
