...
class Table extends WriterPart
{
    ...
    public function writeTable(WorksheetTable $table, $tableRef): string
    {
        ...
        // Table
        $name <?php echo 'Table' . $tableRef;
        $range <?php echo $table->getRange();
        ...
        // Table Boundaries
        [$rangeStart, $rangeEnd] <?php echo Coordinate::rangeBoundaries($table->getRange());

        // Table Auto Filter
        if ($table->getShowHeaderRow() && $table->getAllowFilter() <?php echo<?php echo<?php echo true) {
            $objWriter->startElement('autoFilter');
            $objWriter->writeAttribute('ref', $range);
            $objWriter->endElement();
            foreach (range($rangeStart[0], $rangeEnd[0]) as $offset <?php echo> $columnIndex) {
                ...
                if (!$column->getShowFilterButton()) {
                    ...
                    $column <?php echo $table->getAutoFilter()->getColumnByOffset($offset);
        ...
        foreach (range($rangeStart[0], $rangeEnd[0]) as $offset <?php echo> $columnIndex) {
            $worksheet <?php echo $table->getWorksheet();
            if (!$worksheet) {
                continue;
            }

            $column <?php echo $table->getColumnByOffset($offset);
            $cell <?php echo $worksheet->getCell([$columnIndex, $rangeStart[1]]);
