...
$sql.="select atmid,reqby, entrydate,bill_date,unit, amount,start_date, end_date, due_date,opening_reading,  closing_reading, req_no, print,   cust_id, status, memo, trackerid,req_no from ebillfundrequests where   pstat='0' and reqstatus='100'";
...
$sql.=" order by  print DESC LIMIT $Page_Start , $Per_Page";
//echo $sql;
...
while($row = mysqli_fetch_row($result))
	...
	<?php if($sitero[7]!='0000-00-00'){ echo date("d/m/Y",strtotime($sitero[7])); }else{ echo "NA"; } ?></td>	 
			...
			<div id="edited<?php echo $cnt; ?>"> <input type="button" value="Edit" id="editt<?php echo $cnt; ?>" onclick="Editdata('<?php echo $cnt; ?>','<?php echo $row[17]; ?>');"></div>-->
             ...
             <td><?php echo $ro[0]; ?></td>
		<td><div id="arc<?php echo $row[11]; ?>"><a href='#' onclick="Remarc('<?php echo $row[11]; ?>')">Remove from Archieve</a></div></td>
