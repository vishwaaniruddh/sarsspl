...
else
{
	...
	if($errors==0)
	{
		...
		$scan=mysqli_query($con,"INSERT INTO `invscncpy` (`detail_id`, `filename`,`entryby`,`entrydt`) VALUES ('".$_REQUEST['detail_id']."', '".$image_name."','".$srno[0]."','".$dt."')");
		if($scan)
		{
			$_SESSION['success']="Attached successfully.";
		}
		else
		{
			unlink($newname);
			$_SESSION['success']="Problem in attaching file please try again.";
		}
		echo "<script type='text/javascript'>alert('".$_SESSION['success']."');window.location='scan_viewoldebinv.php?invid=".$_REQUEST['send_id']."';</script>";
	}
	else
	{
		echo "<br/><button><a href=\"scan_viewoldebinv.php?invid=".send_id."\">Back</a></button>";
	}
}
