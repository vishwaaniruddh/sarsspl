...
$contents.="Sr No. \t Atm Id \t Site Id \t Electricity Board \t Consumer No \t Consumer   \t Bill Date \t Due Date \t Unit Consumed \t Month \tAmount \t Invoice No \t Paid Date \t Last Sub-meter Reading \t Rate of the Sub-meter Unit \t supervisor \n";	
...
$fund=mysqli_query($con,"select opening_reading,closing_reading,unit,extrachrg,supervisor  from ebillfundrequests where req_no='".$detro[20]."'");
$fundr=mysqli_fetch_row($fund);
$tot=$tot+$detro[12]+$detro['extrachrg']+$detro['recon_chrg']+$detro['discon_chrg']+$detro['sd']+$detro['after_duedt_chrg'];
$cnt=$cnt+1;
$contents.=$cnt."\t";
$contents.=$sitero[2]."\t";
$contents.=$sitero[1]."\t";
...
$contents.=$detro[8]."\t";
$contents.=$detro[11]."\t";
$contents.=($detro[12]+$detro['extrachrg']+$detro['recon_chrg']+$detro['discon_chrg']+$detro['sd']+$detro['after_duedt_chrg'])."\t";
$contents.=$row[5]."\t";
$contents.=$detro[13]."\t";
$contents.=$fundr[2]."\t";
$contents.=$fundr[4]."\t";
