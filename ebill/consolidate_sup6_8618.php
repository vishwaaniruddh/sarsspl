...
<?php	
	...
	$aid=$opening_fund[1];
	 ...
	 // $accsop=mysqli_query($con,"select * from fundaccounts where aid='".$aid."'");
	  $accsop=mysqli_query($con,"select * from fundaccounts_opbal where aid='".$aid."'");
...
$rev_qry=mysqli_query($con,"SELECT sum(pamount) AS TotalItemsOrdered FROM onaccount_reversal where accid='".$opening_fund['aid']."' and pdate>=STR_TO_DATE('".$_POST['frmdt']."','%d/%m/%Y') AND  pdate<=STR_TO_DATE('".$_POST['todt']." ','%d/%m/%Y')");
