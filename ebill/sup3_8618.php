...
function consolidet()
{
	if(document.getElementById('report_type').value=="consolidate")
	{
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		...
		xmlhttp.open("POST","consolidate_sup3_8618.php",true);
...
if(strtotime(date("Y-m-d",strtotime($ebr[2])))>strtotime(date("Y-m-d",strtotime("2016-03-31")))) { echo "0"; }else
{
...
if($req=="")
{
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.supervisor=a.hname and a.aid='".$_POST['sup']."') and upby!='' and Paid_Date>='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['frmdt'])))." 00:00:00' AND  Paid_Date<='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['todt'])))." 23:59:59'");

echo "select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.supervisor=a.hname and a.aid='".$_POST['sup']."') and upby!='' and Paid_Date>='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['frmdt'])))." 00:00:00' AND  Paid_Date<='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['todt'])))." 23:59:59'"."<br>";

}else{
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.req_no not in($req) and r.supervisor=a.hname and a.aid='".$_POST['sup']."') and upby!=''  and Paid_Date>='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['frmdt'])))." 00:00:00' AND  Paid_Date<='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['todt'])))." 23:59:59'");
