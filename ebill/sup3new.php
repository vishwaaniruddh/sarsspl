...
function consolidet()
{
	if(document.getElementById('report_type').value=="consolidate")
	{
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		...
		xmlhttp.open("POST","consolidate_sup3new.php",true);
...
$reqnoinebillfundreqs=array();
$sendidbetweendates=array();
$reqidbetweendates=array();


$sndidbetdates=mysqli_query($con,"select send_id from send_bill where date>=STR_TO_DATE('".$_POST['frmdt']."','%d/%m/%Y') and date<=STR_TO_DATE('".$_POST['todt']."','%d/%m/%Y')");
while($getsndidbtdates=mysqli_fetch_array($sndidbetdates))
{
$sendidbetweendates[]=$getsndidbtdates[0];
}
//print_r($sendidbetweendates);
$sndidstrr=implode(",", $sendidbetweendates);

//echo $sndidstrr;
//get the req id

//echo "select reqid from send_bill_detail where send_id in($sndidstrr)";
$getreqidfrmsbd=mysqli_query($con,"select reqid from send_bill_detail where send_id in($sndidstrr) and status!=1");

while($gefrsbdrws=mysqli_fetch_array($getreqidfrmsbd))
{
$reqidbetweendates[]=$gefrsbdrws[0];
}

$fnreqid=implode(",", $reqidbetweendates);

$sqlqfeq="select req_no from ebillfundrequests where req_no in ($fnreqid)";


//echo $sqlqfeq;
$str1=mysqli_query($con,$sqlqfeq);
while($sr1rwsft=mysqli_fetch_array($str1))
{

$reqnoinebillfundreqs[]=$sr1rwsft[0];

}
$strinfofreqnofr=implode(",", $reqnoinebillfundreqs);
//print_r($strinfofreqnofr);
...
<?php //if($_SESSION['designation']=='7' || $_SESSION['designation']=='2'){ 
$ebp=mysqli_query($con,"select paid_amount,Paid_Date,entrydt  from ebpayment where Bill_No='".$app[1]."'");
...
if($invro[1]>"2016-03-31" || $invro[0]==""){ echo "0"; }else
{
...
<td>
<?php
$rev_qry1=mysqli_query($con,"SELECT * FROM `reversal` where reqid='".$app[1]."' and  accid='".$_POST['sup']."' and status=1");
if(mysqli_num_rows($rev_qry1)>0)
{
$rev_row1=mysqli_fetch_array($rev_qry1);
echo "R";
}
?>
</td>
...
<td width="75"><?php if($row[17]!=0){ echo $row[17]; } ?></td>
...
<td width="75"><?php echo date('d/m/Y',strtotime($ebr[2])); ?></td>
<td width="75"><?php  echo date('d/m/Y',strtotime($invro[1])); ?></td>
...
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.supervisor=a.hname and a.aid='".$_POST['sup']."' and r.req_no in($fnreqid)) and upby!=''");
else
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.req_no not in($req) and r.supervisor=a.hname and a.aid='".$_POST['sup']."' and r.req_no in($fnreqid)) and upby!=''");



//echo "select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.supervisor=a.hname and a.aid='".$_POST['sup']."' and r.req_no in($fnreqid)) and upby!=''";
...
if(round($row2[16])==0 || ($row2[15] < 8 & $row2[15]!=0) )
{
$qry=mysqli_query($con,"select paid_amount from ebpayment where Bill_No='".$ebpayro[0]."' ");
$rowqr=mysqli_fetch_row($qry);
?>
<tr>
...
while($ebpayro=mysqli_fetch_array($ebpay))
{
$dt=date('Y-m-d', strtotime('2016-03-31'));;
$dt2=date('Y-m-d', strtotime($ebpayro[3]));

//echo $dt."/".$dt2;
if(strtotime($dt2)<=strtotime($dt))
{
$accs2=mysqli_query($con,"select * from fundaccounts where aid='".$_POST['sup']."'");
$rx2=mysqli_fetch_row($accs2);
...
<td></td>
<td></td>
<td colspan=7 ><?php echo $onaccro[5]?></td><td><?php echo $ebpayro[6]; ?></td>
...
//echo $paidamt+$paidamt2+$paidamt3+$paidamt4+$paidamt5+$paidamt6+$opening_fund[2];
  ?>
  </td><td></td><td colspan=8 align='right' >&nbsp;</td></tr>
...
<?php
$to=$total+$total2+$total3+$total4+$total5+$total6;
$netam=$to-$paidamt6;
?>
<tr><td colspan=2 align='right' >NET AMOUNT</td><td align='CENTER' ><?php echo $netam; ?></td><td> </td><td colspan=8 align='right' >&nbsp;</td></tr>
<tr><td colspan=2 align='right' >NET PAID</td><td align='CENTER' ></td><td> <?php echo $paidamt+$paidamt2+$paidamt3+$paidamt4+$paidamt5; ?></td><td colspan=8 align='right' >&nbsp;</td></tr>

<tr><td colspan=2 align='right' >Difference Amount </td><td align='CENTER' ><?php echo $netam-($paidamt+$paidamt2+$paidamt3+$paidamt4+$paidamt5); ?></td><td></td><td colspan=8 align='right' >&nbsp;</td></tr>
<?php
}
