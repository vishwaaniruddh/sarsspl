...
document.getElementById("search").innerHTML ="<center><img src=loader.gif></center>";
		...
		var desig=document.getElementById('desig').value;
			...
			var url = 'searchRmn.php';
	var pmeters="";
	if(a!="Loading"){ 
			 pmeters = 'cid='+cid+'&Page='+b+'&perpg='+ppg+'&sdate='+sdate+'&edate='+edate+'&desig='+desig+'&service='+serv+'&dept='+dept+'&bank='+bank+'&state='+state;
			}//alert("gg");
			else
			{
...
//alert(xmlhttp.responseText);
 ...
 var rem=document.getElementById("rem"+cnt).value;
//alert("approvequot.php?id="+id+"&stat="+stat+"&rem="+rem+'&amt='+amt);
...
//alert(xmlhttp.responseText);
 ...
 // alert("approvefund.php?id="+id+"&stat="+stat+"&rem="+rem+'&amt='+amt+"&chk="+chkno);
xmlhttp.open("GET","getrmnbank.php?cid="+cid,true);
xmlhttp.send();
}
...
<body onload="searchById('Listing','1','');<?php if(isset($_GET['cid'])){ ?>getbank('<?php echo $_GET['cid']; ?>');<?php  } ?>" >
...
//$qry=mysqli_query($con,"Select short_name,contact_first from contacts where type='c' and short_name='".$_SESSION['custid']."'");
 ...
 <select  name="cid" id="cid" onchange="getbank(this.value);"><option value="">Select Client</option>

<?php
while($clro=mysqli_fetch_row($qry))
{
?>
<option value="<?php echo $clro[0]; ?>" <?php if($_GET['cid']==$clro[0]){ echo "selected";  } ?>><?php echo $clro[1]; ?></option>
<?php
}
?>
</select> </th><th>
<select name="bank" id="bank" onchange="">
<option value="">Select Bank</option>
</select></th>

<th width="75">
<?php
$stater=mysqli_query($con,"select * from `state` order by `state` ASC");
?>
<select name="state" id="state" onchange=""><option value="">Select State</option>
<?php

while($stro=mysqli_fetch_array($stater))
{
?>
...
<th width="75"><input type="text" name="sdate" id="sdate" value="<?php if(isset($_GET['stdt'])){ echo $_GET['stdt'];  } ?>" onclick="displayDatePicker('sdate');" readonly="readonly" placeholder="From Date"/></th>
<th width="75"><input type="text" name="edate" id="edate" value="<?php if(isset($_GET['todt'])){ echo $_GET['todt'];  } ?>"  onclick="displayDatePicker('edate');" readonly="readonly" placeholder="To Date"/></th>
