...
<head>
    ...
    <script>
        ...
        function func(strpg, perpg) {
            ...
            $.ajax({
                type: 'POST',
                url: 'getquotdfr_test.php',
                beforeSend: function () {

                    document.getElementById("search").innerHTML = "<center><img src=loader.gif></center>";
                },
                data: 'sup=' + sup + '&strdt=' + strdt + '&endt=' + endt + '&perpg=' + perp + '&Page=' + Page +
                    '&accname=' + accname + '&custr=' + custr + '&atm=' + atm + '&fdtyp=' + fdtyp,
                success: function (msg) {
        ...
        <center>
            ...
            <table>
                ...
                <th>
                    ...
                    ?>
                    <select name="cust1[]" id="cust1" multiple="multiple" onchange="getaccm($(this).val());">
                        <option value="">Select Client</option>
                        <?php
                        while ($clro = mysqli_fetch_row($qry)) {
                            ?>
                            <option value="<?php echo $clro[0]; ?>" <?php if ($custr == $clro[0]) {
                                   echo "selected";
                               } ?>><?php echo $clro[1]; ?></option>
                            <?php
                        }
                        ?>
                    </select>
                ...
                <th><select id="accname" name="accname">
                        ...
                        $qrs = "select srno,username from login  where srno in(select reqby from quotation1) ";
                ...
                <th>From Date:- <input type="text" name="frmdt" id="frmdt" value="<?php if (isset($_POST['frmdt'])) {
                    echo $_POST['frmdt'];
                } else {
                    echo date('d/m/Y', strtotime('-2 days'));
                } ?>"
                        onclick="displayDatePicker('frmdt');"></th>
                <th>
                    To Date:- <input type="text" name="todt" id="todt" value="<?php if (isset($_POST['todt'])) {
                        echo $_POST['todt'];
                    } else {
                        echo date('d/m/Y');
                    } ?>"
                        onclick="displayDatePicker('todt');">
                </th>
