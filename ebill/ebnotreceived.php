...
<center>
	<form name="myform" method="post" action="<?php $_SERVER['PHP-SELF']; ?>">
        ...
        <select name="cust" onchange="getproject(this.value,'projectid');">
        		...
        		if(isset($_SESSION['custid']) && $_SESSION['custid']!="all" && $_SESSION['custid']!="")
			{
        		...
        		while($cust_row=mysqli_fetch_array($cust_qry))
        		{
        			if(trim($cust_row[0])!="")
        			{
        				$cust_name_qry = mysqli_query($con,"SELECT contact_first FROM contacts where short_name ='".$cust_row[0]."'");
					$cust_name_row=mysqli_fetch_array($cust_name_qry);
        	?>
        		<option value="<?php echo $cust_row[0]; ?>" <?php if(isset($_REQUEST['cust']) && $_REQUEST['cust']!='' && $_REQUEST['cust']==$cust_row[0]) echo "selected"?>><?php echo $cust_name_row[0]; ?></option>
        	<?php
        			}
        		}
        	?>
        ...
        <select name="supervisor">
        	...
        	<?php
        		$supervisor_qry=mysqli_query($con,"select distinct(hname) from fundaccounts order by hname");
