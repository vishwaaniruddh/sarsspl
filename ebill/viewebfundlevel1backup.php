...
{
if($sdate!=$edate)
$sql.=" and req_no in (select reqid from ebillfundapp where apptime between STR_TO_DATE('".$_POST['sdate']."','%d/%m/%Y') and STR_TO_DATE('".$_POST['edate']."','%d/%m/%Y') order by apptime DESC ) ";
else
$sql.=" and req_no in (select reqid from ebillfundapp where apptime like '".$edate."%' order by apptime DESC )";
}
else{
if($_POST['pstat']=='8' || $_POST['pstat']=='0'){
if($sdate!=$edate)
$sql.=" and req_no in (select reqid from ebillfundapp where apptime between STR_TO_DATE('".$_POST['sdate']."','%d/%m/%Y') and STR_TO_DATE('".$_POST['edate']."','%d/%m/%Y') order by apptime DESC limit 1) ";
else
$sql.=" and req_no in (select reqid from ebillfundapp where apptime like '".$edate."%' order by apptime DESC limit 1)";
}
else
{
if($sdate!=$edate)
...
$sql.=" and supervisor like '%".trim($_POST['superv'])."%'";
...
<td width="75"><a href="#" onclick="newwin('getebhistory.php?custid=<?php echo $row[12]; ?>&trackid=<?php echo $row[14]; ?>&atmid=<?php echo $row[1]; ?>','display')"><?php echo $row[1]; ?></a></td>
...
if($desig=="6" && $service=='1' && $dept=='5' && $row[17]!='0' && $row[17]!=''){
