...
$queryamt=mysqli_query($con,"Select sum(tamount) from quotation1ftransfers where tid='".$tid."' and status!=0");
$qrrow=mysqli_fetch_array($queryamt);


$query=mysqli_query($con,"Select qid from quotation1ftransfers where tid='".$tid."' and status!=0");
$reqarr=array();
...
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
	...
	<table  border="2" align="center" cellspacing="10" cellpadding="10">
				...
				<?php
				$cnt=0;
				$qrydata=mysqli_query($con,"Select * from quotation1 where id in($reqid)");
				while($rowdata=mysqli_fetch_row($qrydata))
				{
				$cnt=$cnt+1;
				
				
				$qrys=mysqli_query($con,"Select tamount,chqno,accid,chq_name from quotation1ftransfers where qid='".$rowdata[0]."'");
