...
// 	echo $qry;	
	...
	<input type="button" id="btnExport" value="Export Excel" onclick="expex();"/>
    ...
    <th align="center">Expected Approval Amount</th>
	...
	<?php
	    ...
	    <td  align="center" width="50">
	     ...
	     <td  align="center" width="150">
	       <input type="text" style='width:75px;' id="qid<?php echo $srno?>" name="qid" value="<?php echo  $row[0]; ?>"  readonly="readonly">
...
<?php
// || $row[2]=='kotak'
if($row[2]=='ICICI' || $row[2]=='RATNAKAR' || $row[2]=='ICICI_Direct' || $row[2]=='Knight_Frank' || $row[2]=='BajajFinance' || $row[2]=='Bandhan_Branch' )
{ 
...
<?php
 }else if($_POST['accname']=='563' && $row[2]=='kotak'){ ?>
...
<?php
 ...
 echo  round($tamt[0]); $totamt=$totamt+round($tamt[0]);
	   ...
	   <?php 
	      ...
	      {
	      $amtqry=mysqli_query($con,"select app_amt,filename,app_by,approved_date,req_amt,ticket_no,remark,expectedApprovalAmt from quotation_approve_details where qid='".$row[0]."'");
	   ...
	   ?> 
        ...
        <?php
                echo round($rowamt[7]);
               $expectedquotamt=$expectedquotamt+round($rowamt[7]);
            ?>
               </td>
...
if($_POST['stat']=="1" )
	 ...
	 ?>  
           ...
           <td><input type="checkbox" name="pay[]" id="pay<?php echo $srno;?>" value="<?php echo $row[0]; ?>" onclick="addamt('<?php echo round($reqamtw[0]); ?>',this.id);" amount = "<?php echo round($reqamtw[0]); ?>" checked> 
...
<?php } ?>
	...
	<td align="center"><b><?php echo round($expectedquotamt); ?><b></td>
...

{
        ...
        if ($_POST['stat']=="1" )
           {
    ?>
           <td></td><td><b>Selected Total </b></td> <td ><b>
               <input type="text" name="seltot" id="seltot" value="<?php echo round($reqtotamt); ?>" readonly>
               </b></td><td colspan="5" align='center' ><input type="button" value="Payments" onclick="subm();" /></td>
