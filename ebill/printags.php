...
$send=mysql_query("select * from send_bill where send_id='".$sendid."'");

$sendro=mysql_fetch_row($send);
...
mysqli_
       mysqli_  {
mysqli_
          mysqli_   $w .= int_to_words(floor($x/1000)) .' Thousand';
...
//echo "select * from send_bill_detail where send_id='".$sendid."'";

$qr=mysql_query("select * from send_bill_detail where send_id='".$sendid."'");

$bills=array();

while($qrro=mysql_fetch_array($qr))
...
$qry=mysql_query("select cust_name from sites where cust_id='$cid'");

                $r1=mysql_fetch_row($qry); 

      mysqli_    $cust_name=$r1[0];

           $rowcompmysqli__fetch_row($rescomp); 

$qry1=mysql_query("select * from ebillcharges where Cid='$cid' and Bid='$bid'");  

//echo mysql_num_rows($qry1); 
mysqli_
if(mysql_num_rows($qrymysqli_        
mysqli_
...
<meta http-eqmysqli_ontent-Type" content="text/html; charset=iso-8859-1" />

<title>Umysqli_d Document</title>
...
//$res4=mysql_query("insert into send_bill(customer_name,bank,date,invoice_no,comp) values('$cust_name','$bid','$currentdate','$newinvoice_no','$comp')");
mysqli_
mysqli_
...
/EB/<?php echo $newinvoice_no; ?>/2013-14</td>
  ...
  <tr>
   ...
   <th width="76" scope="col" ><div align="center">CONSUMER NAME</div></th>
    ...
    <th width="60" scope="col"><div align="center">AMOUNT</div></th>  
      ...
      mysqli_th width="65" scope="col"><div align="center"> Last Sub-meter Reading</div></th>
mysqli_
  ...
  $qr=mysql_query("select * from send_bill_detail where send_id='".$sendid."'");
       ...
       while($qrro=mysql_fetch_array($qr))///$i=0

             {
			  ...
			  $num=mysql_num_rows($result1);
                ...
                $nsql = "select * from ebill where ATM_ID='$qrro[2]'"; //echo $nsql;
	...
	$ress = mysql_query($sqll);
		    ...
		    $res=mysql_query("select location,site_id from sites where atm_id1='$qrro[2]'");
...
$qry=mysql_query("select * from send_bill_detail where detail_id='".$bills[$i]."'");

$qryro=mysql_fetch_row($qry);



$res=mysql_query("select location,site_id from sites where atm_id1='$qryro[2]' or atm_id2='$qryro[2]'");
...
<td><?php echo $qryro[6]; ?></td>

<td><?php echo $qryro[7]; ?></td>

<td><?php echo $qryro[8]; ?></td>

<td><?php echo $qryro[11]; ?></td>
...
$atm=$row[3];	
	...
	/*$result3=mysql_query("select cust_id,atmsite_address from sites where atm_id1='$atm'");
    ...
    // echo "select cust_id,atmsite_address from sites where cust_name='$cid'";

	  $resul3=mysql_query("select cust_id,atmsite_address from sites where cust_name='$cid'");
