...
mysqli_
<title>Feedmysqli_title>

<linkmysqli_"style.css" rel="stylesheet" type="text/css" />
mysqli_
</head>
...
$conf=mysql_query("select * from  reversal_update where reqid='".$reqid."' order by id DESC");

while($row=mysql_fetch_array($conf))

{

	$sr=mysql_query("select username from login where srno='".$row['upby']."'");
