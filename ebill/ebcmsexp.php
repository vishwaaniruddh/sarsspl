...
$qry=mysqli_query($con,"select distinct(accid),email_body,mail_by,dbtaccno,pdate from ebonacctransfers where tid='".$tid."' ");
...
$qrdet=mysqli_query($con,"select");

$qrnm=mysqli_query($con,"select hname,accno,branch,ifsc_code from fundaccounts where aid='".$rowd[0]."'");
$qrnrow=mysqli_fetch_array($qrnm);

$qry2=mysqli_query($con,"select sum(approvedamt) from onacctransfer where reqid in(select reqid from ebonacctransfers where tid='".$tid."' and accid='".$rowd[0]."')");
$qrrow2=mysqli_fetch_array($qry2);

$pdt="";
if($rowd[4]!="0000-00-00")
{
$pdt=date('m Y',strtotime($rowd[4]));
}

$objSheet->setCellValueByColumnAndRow(1, $row, $qrrow2[0]);
...
$objSheet->setCellValueByColumnAndRow(6, $row,$rowd[1]);
$objSheet->setCellValueByColumnAndRow(7, $row,$rowd[3]);
...
$objSheet->setCellValueByColumnAndRow(12, $row, $rowd[2]);
$objSheet->setCellValueByColumnAndRow(13, $row,"EBILL WORK");
