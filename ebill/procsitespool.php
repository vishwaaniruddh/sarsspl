...
$prov=mysqli_query($con,"select provider from eserviceprovider where code='".$stro[9]."' and state='".$stro[8]."'");
 ...
 $contents.="\n".strtoupper(preg_replace('/\s+/', ' ',$clientro[0]))."\t";
 //$contents.=$stro[0]."\t";
 $contents.=strtoupper(preg_replace('/\s+/', ' ',$stro[1]))."\t";
 $contents.=strtoupper(preg_replace('/\s+/', ' ',$stro[2]))."\t";
 $contents.=strtoupper(preg_replace('/\s+/', ' ',$stro[3]))."\t";
 $contents.=strtoupper(preg_replace('/\s+/', ' ',$stro[4]))."\t";
...
//echo "select max(p.Paid_Date),e.paytype from ebpayment p,ebillfundrequests e where e.trackerid='".$row[15]."' and e.req_no=p.Bill_No<br>";
		...
		$contents.=strtoupper(preg_replace('/\s+/', ' ',$stro[13]))."\t";
		$contents.=strtoupper(preg_replace('/\s+/', ' ',$stro[14]))."\t";
