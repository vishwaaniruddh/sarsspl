...
<table>
	...
	<tr>
		...
		$qry=mysql_query("select short_name, contact_first from contacts where type='c' order by contact_first ASC");
		...
		?>
			...
			while($cl=mysql_fetch_array($qry))
   ...
   <tr>
		...
		$tp=mysql_query("select `deptid`, `desc` from department where deptid<>'1' AND deptid<>'4'");
        ...
        <select name="department" id="department"><option value="">Select Concerned Department</option>
			...
			while($tpro=mysql_fetch_array($tp))
