...
<?php	
        $total=0;
$qry1=mysqli_query($con,"select max(tid) from ebfundtransfers_tis");        
$qrrow=mysqli_fetch_row($qry1);
	...
	for($x=0;$x<count($reqs);$x++){
		...
		if($accname[$x]!='-1'){
			...
			$sql="insert into ebfundtransfers_tis(tid,reqid,accid,pdate,chqname,chqno,dbtaccno,narration,entrydt) values('$tid','$reqs[$x]','$accname[$x]','$pdt','$chqname','$chqno','".$_POST['dbtacc']."','".$_POST['narr'][$x]."','$dt')";		
			$table=mysqli_query($con,$sql); 
			/*if(!$table)
				echo mysqli_error();*/
			$sql="insert into ebillfundapp_tis(reqid,appby,apptime,level) values('$reqs[$x]','$userid','$dt','8')";		
			$table=mysqli_query($con,$sql); 
			$qry=mysqli_query($con,"select iFund_status from ebillfundrequests_tis where req_no='$reqs[$x]'");
			$row=mysqli_fetch_array($qry);
			if($row[0]==7)
			{
				$sql="update ebillfundrequests_tis set chqno='$chqno',iFund_status='8' where req_no='$reqs[$x]'";
				mysqli_query($con,"Update oldebreq_tis set status='8' where reqid='$reqs[$x]'");
			}
			else
				$sql="update ebillfundrequests_tis set reqstatus='8',chqno='$chqno' where req_no='$reqs[$x]'";		
			$table=mysqli_query($con,$sql);
		}  
    }  
                
$qry1=mysqli_query($con,"select distinct(accid) from ebfundtransfers_tis where tid='$tid' and accid<>90");
$ct=1;
...
$qry2=mysqli_query($con,"select sum(approvedamt) from ebillfundrequests_tis where req_no in(select reqid from ebfundtransfers_tis where tid='$tid' and accid='$qrrow[0]')");
...
<?php
 ...
 //for cheque DD
 $qry2=mysqli_query($con,"select accid,reqid,narration from ebfundtransfers_tis where tid='$tid' and accid=90");
//$ct=1;
while($qrrow2=mysqli_fetch_array($qry2))
{
$branch2=mysqli_query($con,"select * from fundaccounts where aid='$qrrow2[0]'");
$brro2=mysqli_fetch_row($branch2);
//$deptde=mysqli_query($con,"select `desc` from department where deptid='2'");
//$dtro=mysqli_fetch_row($deptde);
//$crow=mysqli_fetch_row($qry1);	
//echo "select approvedamt from ebillfundrequests where req_no ='".$qrrow2[1]."'";
$qry3=mysqli_query($con,"select approvedamt from ebillfundrequests_tis where req_no ='".$qrrow2[1]."'");
$qrrow3=mysqli_fetch_array($qry3);
?><div class=article>
<div class=title><tr>
<td width="10" align='center'><?php echo $ct++; ?></td>
