...
$panel_ID = $sql_result['panel_ID'];
$panel_IP = $sql_result['panel_IP'];
$router_IP = $sql_result['router_IP'];
$router_ID = $sql_result['router_ID'];
$panel_Make = $sql_result['panel_Make'];
$dvr_Model = $sql_result['dvr_Model'];
$dvr_IP = $sql_result['dvr_IP'];

                                    
                                    $insql = "insert into mis_newsite(activity,customer,bank,atmid,atmid2,atmid3,trackerno,address,city,state,zone,branch,bm_name,bm_number,created_by,created_at,status,engineer_user_id,panel_ID,panel_IP,router_IP,router_ID,panel_Make,dvr_Model,dvr_IP) 
                                    values('".$activity."','".$customer."','".$bank."','".$atmid."','".$atmid2."','".$atmid3."','".$trackerno."','".$address."','".$city."','".$state."','".$zone."','".$branch."','".$bm_name."',
                                    '".$bm_number."','','".$date."',1,'".$engineer."','".$panel_ID."','".$panel_IP."','".$router_IP."','".$router_ID."','".$panel_Make."','".$dvr_Model."','".$dvr_IP."')" ;
