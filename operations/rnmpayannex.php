...
?><html><head><title>Paid RNM Fund</title>
...
//echo $cl;
        ...
        $total=0;$x=0;
        $str="SELECT * FROM `rnmfundtransfers` where reqid in (select quotid from quotation WHERE  `chqno` !=  ''
AND  `chqno` !=0) and pdate>='2014-04-01'";
...
//echo $app[18];
if($row[18]=="sites")
$site="select bank,atmsite_address,location,atm_id1,atm_id2,atm_id3,site_id,state,city from ".$row[3]."_sites where trackerid='".$row[4]."'";
if($row[18]=="rnmsites")
$site="select bank,atmsite_address,location,atm_id1,atm_id2,atm_id3,site_id,state,city from rnmsites where id='".$row[4]."'";
...
<td width="75"><?php echo $row[21];$total=$total+$row[21]; ?></td>
<td width="75">
<?php echo $rx[1]; ?>
</td>
<td width="75">
...
<?php echo date('d/m/Y',strtotime($row[23])); ?></td>
</tr></div></div>
<?php
}
}
