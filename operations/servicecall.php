...
document.getElementById('trg').disabled=true;

}
else 
{
document.getElementById('trg').disabled=false;
}
}
else
{
if(document.getElementById('trg').checked)
{
...
val=escape(document.getElementById(id).value);
if(document.getElementById(id2).value!='-1')
val2=escape(document.getElementById(id2).value);
...
<form name="requestamt" method="post" action="process_new_alert.php" onSubmit="return validate(this)">
...
<td>ATM ID</td>
    ...
    <tr>
    	...
    	<td>CSS Area<select name="cssarea1" id="cssarea1"> 
    ...
    <tr>
    	...
    	<td>Address</td>
        <td>
        	<textarea name="address" id="address"  ></textarea><!--<input type="checkbox" name="updet" id="updet">-->
...
<td colspan=2><table><tr><td>Email ID</td><td><input type="text" name="email" id="email" tabindex="5"></td></tr>
    ...
    <td>Select Supervisor&nbsp;&nbsp;&nbsp;<br><br><!--<br>Call delegated to<br>--><br>Transfer To Geeta<br><br>Branch Manager Name</td>
<td><?php
...
?>
<select name="super" id="sup" tabindex="6" onchange="disfunc('s');">

<option value="">Select Supervisor</option>
<?php
			 $sup=mysqli_query($con,"select distinct(hname),branch,aid from fundaccounts where status=0 order by hname ASC");
				 while($supro=mysqli_fetch_array($sup))
				{ ?>
				   <option value="<?php echo $supro[2]; ?>" <?php if(($_SESSION['user']==$supro[0])){ echo "selected";}else{ if($quotro[19]==$supro[0]){ echo "selected";  }  }  ?> ><?php echo $supro[0]." ".$supro[1]; ?></option>
			   <?php } 

?></select><br><br>
<!--<input type="text" name="del" id="del"><br><br>-->

<input type="checkbox" name="trg" id="trg" onchange="disfunc('g');"/><br><br>

        	<input type="text" name="csslocalbr" id="csslocalbr" >
</td>

<tr>
<td>Alert Date</td><td><input type="text" name="aldt" id="aldt" value=<?php echo date('d/m/Y'); ?> onclick="displayDatePicker('aldt');" readonly/></td>
