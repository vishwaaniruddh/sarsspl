...
<?php
	...
	if($_SESSION['designation']==9 && $_SESSION['branch']!="all" && $_SESSION['branch']!="")
		$branchmgr=1;
	$rev_str="SELECT * FROM `transfer` where 1 ";
	if($accmgr || $branchmgr)
	{
		if(isset($_REQUEST['from_supv']) && $_REQUEST['from_supv']!="")
			$rev_str.="and from_accid='".$_REQUEST['from_supv']."' ";
	}
	else
	{
		...
		$rev_str.="and from_accid='".$fundacc['aid']."' ";
	}
	...
	if(isset($_REQUEST['sup']) && $_REQUEST['sup']!="")
		$rev_str.="and to_accid='".$_REQUEST['sup']."' ";
...
if($showpaging)
{
	...
	while($rev=mysqli_fetch_array($rev_qry))
	{
		...
		{
			$fundacc_qry=mysqli_query($con,"SELECT hname FROM `fundaccounts` WHERE `aid` = '".$rev['to_accid']."'");
			$fundacc=mysqli_fetch_array($fundacc_qry);
			$fundacc_qry1=mysqli_query($con,"SELECT hname FROM `fundaccounts` WHERE `aid` = '".$rev['from_accid']."'");
			$fundacc1=mysqli_fetch_array($fundacc_qry1);
