...
<form name="requestamt" method="post" action="procesditquote.php" onSubmit="return validate(this)" enctype="multipart/form-data">
...
<?php
			 $sup=mysqli_query($con,"select distinct(hname),branch from rnmfundaccounts order by hname ASC");
				 while($supro=mysqli_fetch_array($sup))
				{ ?>
				   <option value="<?php echo $supro[0]; ?>" <?php if(($quotro[19]==$supro[0])){ echo "selected";}else{ if($quotro[19]==$supro[0]){ echo "selected";  }  }  ?> ><?php echo $supro[0]." ".$supro[1]; ?></option>
			   <?php } 
