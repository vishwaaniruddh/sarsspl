...
<form method="post" action="process_transfer_req_sv.php"> 
	<table>
    	<?php
			if($accmgr || $branchmgr)
			{
				...
				$sup_qry=mysqli_query($con,"select * from fundaccounts where hname like '$row[0]'");
				$supro1=mysqli_fetch_array($sup_qry);
		?>
        <tr>
        	<td>Transfer From</td>
            <th><?php echo $supro1[1]."/ ".$supro1[4]."/ ".$supro1[2]; ?></td>
    	</tr> 
		<?php
			}
		?>
    	<tr>        
            ...
            <td>
                ...
                ?>
                <select name="to_accid" id="to_accid" style="width:150px"><option value="">Select Supervisor</option>
                <?php
                while($supro=mysqli_fetch_array($sup))
                {
					if($accmgr || $branchmgr)
					{
						...
						{
				?>
					<option value="<?php echo $supro[0]; ?>"><?php echo $supro[1]."/ ".$supro[4]."/ ".$supro[2]; ?></option>
				<?php
						}
					}
					else
        ...
        <tr>        
            <td>Amount : </td><?php
				...
				$ebfundreq_qry=mysqli_query($con,"select approvedamt from ebillfundrequests where req_no='".$_REQUEST['reqid']."'");
        ...
        <tr>
        	<td colspan="2"><input type="text" name="reqid" value="<?php echo $_REQUEST['reqid']; ?>"/><input type="submit" value="Submit"/></td>
        </tr>
    </table>
</form>
</center>
