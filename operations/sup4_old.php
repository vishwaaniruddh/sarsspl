...
{
$login_qry=mysqli_query($con,"select srno from login where username='".$_SESSION['user']."'");
$login_row=mysqli_fetch_row($login_qry);
$login_str=" WHERE `srno` = '".$login_row[0]."'";
}
$sup=mysqli_query($con,"select * from fundaccounts ".$login_str." order by hname ASC");
...
?>
<div class=article>
<div class=title><tr style="display:none">
<td width="25">
<?php 
//echo $cnt."<br/>".$app[1];
echo $app[1];
...
?>
<tr style="display:none"><td colspan=2 align='right' >Total Amount Transfer through Software & Receipt Received (A) </td><td align='CENTER' ><?php echo $total; ?></td><td><?php echo $paidamt; ?></td><td colspan=8 align='right' >&nbsp;</td></tr>
<?php
...
<?php
	...
	echo $row2[16]; $total2=$total2+$row2[16]; 
?>
</td>
<?php //if($_SESSION['designation']=='7' || $_SESSION['designation']=='2'){ 

$paidamt2=$paidamt2+$ebpayro[1];
$inv2=mysqli_query($con,"select s.send_id from send_bill s,send_bill_detail d where s.status='0' and s.send_id=d.send_id and d.status=0 and d.reqid='".$row2[0]."'");
...
<tr><td colspan=2 align='right' >NET PAID</td><td align='CENTER' ></td><td align="center"> <?php echo $paidamt+$paidamt2+$paidamt3+$paidamt4+$paidamt5+$podtotal+$disputeamt; ?></td><td colspan=10 align='right' >&nbsp;</td></tr>
