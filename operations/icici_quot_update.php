...
$qry=mysqli_query($con,"select * from icici_quot_details where id='".$detailid."' and qid='".$qid."'");
...
$insqry=mysqli_query($con,"Insert into `quotation_iciciedit_history`(det_id,`qid`, `mat_grp`, `serv_no`, `mat_text`, `gprice`, `qnty`, `unit`, `amt`, `quot_tot_amt`, `remark`, `reqby`, `entrydt`,update_remark,status,filename)

values('".$row[0]."','".$row[1]."','".$row[2]."','".$row[3]."','".$row[4]."','".$row[5]."','".$row[6]."','".$row[7]."','".$row[8]."','".$tqtamt."',
'".$row[9]."','".$srno[0]."','".$dt."','".$updremark."','u','".$image_name."')");
if(!$insqry)
{
...
$upqry=mysqli_query ("Update icici_quot_details SET `qid`='".$qid."',`material_group`='".$matg."',`service_no`='".$servno."',`material_text`='".$matext."',`gprice`='".$gprice."',`qnty`='".$qnty."',`unit`='".$unit."',`amt`='".$amt."',`remark`='".$rem."' where id='".$detailid."' and qid='".$qid."' ");
if(!$upqry)
{
