...
<table width="800" border="1">
  ...
  <?php
		...
		$sql.=" and (status='0' or status='1')";
		      ...
		      if($_SESSION['custid']!='' && $_SESSION['custid']!='all')  
		$sql.=" and cid='".$_SESSION['custid']."'";
...
//echo $sql;
	$cnt=0;		 
		...
		{	
		$cnt=$cnt+1;
		$custname=mysqli_query($con,"select contact_first from contacts where short_name='".$row[15]."'");
		$cname=mysqli_fetch_row($custname);
		?>
		  <tr>
    ...
    <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="atm" id="atm<?php echo $cnt-1; ?>" value="<?php echo $row[1]; ?>" readonly="readonly" /><?php }else{  echo $row[1];  } ?><?php //echo $row[1]; ?></div></td> 
     <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="cons" id="cons<?php echo $cnt-1; ?>" value="<?php echo $row[2]; ?>" readonly="readonly" /><?php }else{  echo $row[2];  } ?><?php //echo $row[2]; ?> </div></td>
      <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="fdt<?php echo $cnt-1; ?>" id="fdt<?php echo $cnt-1; ?>" onclick="displayDatePicker('fdt<?php echo $cnt-1; ?>');" value="<?php echo date("d/m/Y",strtotime($row[3])); ?>" readonly="readonly" /><?php }else{  echo $row[3];  } ?><?php //echo $row[3]; ?> </div></td>
      <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="tdt<?php echo $cnt-1; ?>" id="tdt<?php echo $cnt-1; ?>" onclick="displayDatePicker('tdt<?php echo $cnt-1; ?>');" value="<?php echo date("d/m/Y",strtotime($row[4])); ?>" readonly="readonly" /><?php }else{  echo $row[4];  } ?><?php //echo $row[4]; ?></div></td>
      <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="bdt<?php echo $cnt-1; ?>" id="bdt<?php echo $cnt-1; ?>" onclick="displayDatePicker('bdt<?php echo $cnt-1; ?>');" value="<?php echo date("d/m/Y",strtotime($row[5])); ?>" readonly="readonly" /><?php }else{  echo $row[5];  } ?><?php //echo $row[5]; ?> </div></td>
     	

       <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="ddt<?php echo $cnt-1; ?>" id="ddt<?php echo $cnt-1; ?>" onclick="displayDatePicker('ddt<?php echo $cnt-1; ?>');" value="<?php echo date("d/m/Y",strtotime($row[6])); ?>" readonly="readonly" /><?php }else{  echo $row[6];  } ?><?php //echo $row[6]; ?> </div></td>
       <td scope="col"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="text" name="pdt<?php echo $cnt-1; ?>" id="pdt<?php echo $cnt-1; ?>" onclick="displayDatePicker('pdt<?php echo $cnt-1; ?>');" value="<?php echo date("d/m/Y",strtotime($row[7])); ?>" readonly="readonly" /><?php }else{  echo $row[7];  } ?><?php //echo $row[7]; ?></div></td>
   ...
   <td scope="col" id="edt<?php echo $cnt-1; ?>"><div align="center"><?php if($desg=='4' && $row[17]<'2'){ ?><input type="button" value="Edit" onclick="Showedt('<?php echo $cnt-1; ?>');" id="show<?php echo $cnt-1; ?>" /><?php } ?>
  <?php 
  //for Account Manager
  if($desg=='4'){ ?> <input type="button" value="Save" onclick="saveedt('<?php echo $cnt-1; ?>','<?php echo $row[0]; ?>','1');" id="save<?php echo $cnt-1; ?>" style=" <?php if($desg=='4' && $row[17]<'2'){ ?>display:none; <?php }else {?>display:block; <?php } ?>" /><?php }
  else
  {
  
  if($row[17]!='2'){
  ?> <input type="button" value="Approve" onclick="approve('<?php echo $cnt-1; ?>','<?php echo $row[0]; ?>','2');" id="save<?php echo $cnt-1; ?>" style=" <?php if($desg=='4' && $row[17]<'2'){ ?>display:none; <?php }else {?>display:block; <?php } ?>" /><?php  }
  }
   ?>
