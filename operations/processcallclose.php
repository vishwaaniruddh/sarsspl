...
if($rtime!='')
{
$sql2=mysqli_query($con,"select po,atm_id,cust_id,responsetime from alert where alert_id='$id'");
$row2=mysqli_fetch_row($sql2);
$qry2=mysqli_query($con,"Update alert set close_date='".$rtime."', call_status='Done' where alert_id='".$id."'");
 $time=strtotime($rtime);

if($row2[3]=='0000-00-00 00:00:00')
$qry2=mysqli_query($con,"Update alert set responsetime='".$rtime."' where alert_id='".$id."'");
$sql5=mysqli_query($con,"select Ref_id from atm where atm_id='".$row2[1]."'");
$row5=mysqli_fetch_row($sql5);
//echo "select servicetype from atm where po='$row2[0]' and atm_id LIKE '".$row2[1]."%' Limit 1";
$sql3=mysqli_query($con,"select servicetype from atm where po='$row2[0]' and atm_id LIKE '".$row2[1]."%' Limit 1");
$row3=mysqli_fetch_row($sql3);
//echo "select * from site_assets where po IN ('".$row2[0]."') ";
$sql=mysqli_query($con,"select * from site_assets where po IN ('".$row2[0]."') ");
...
$sql4=mysqli_query($con,"INSERT INTO `satyavan_accounts`.`installed_sites` (`id`, `custid`, `po`, `atmid`, `Ref_id`, `assdescid`, `startdt`, `expdt`, `status`) VALUES (NULL, '".$row2[2]."', '".$row[0]."', '".$row2[1]."', '".$row5[0]."', '".$row[4]."', '".$rtime."', '".date("Y-m-d",strtotime("+".$str[0]." months", $time))."', '1')");
...
$sql6=mysqli_query($con,"Insert into servicemonth(`po`,`date`,`type`) Values('".$row2[0]."','".date("Y-m-d",strtotime("+".$j." months", $time))."','new')");
}
//$qry=mysqli_query($con,"Insert into installed_sites()");
}
