...
<?php
	...
	if($accmgr || $branchmgr)
	{
		if(isset($_REQUEST['from_supv']) && $_REQUEST['from_supv']!="")
			$rev_str.="and accid='".$_REQUEST['from_supv']."' ";
	}
	else
	{
	...
	if(isset($_REQUEST['sup']) && $_REQUEST['sup']!="")
		$rev_str.="and dbtacc='".$_REQUEST['sup']."' ";
...
if($showpaging)
{
	...
	while($rev=mysqli_fetch_array($rev_qry))
	{
		...
		//echo "<br/>".mysqli_num_rows($ebfundreq_qry)."<br/>";
		if(mysqli_num_rows($ebfundreq_qry)>0)
		{
			//echo "SELECT hname FROM `fundaccounts` WHERE `aid` = '".$rev['accid']."'";
...
?>
	<tr>
        ...
        <td>
            ...
            <a href="javascript:void(0);" id='feefbk<?php echo $count; ?>' onclick="newwin('viewefeed_reversal.php?reqid=<?php echo $ebfundreq['req_no']; ?>','display','500',400)">Click to view all Feedbacks</a>
        </td>
