...
//echo $custs;
        ...
        if($atm=="" & $qid=="" & $reqby=="-1" & $category=="-1"  & $strdt=="" & $endt=="" & $qtst=="" & ($fundtyp=="" || $fundtyp=="-1") & $calltyp=='')
        {
           $srdt=date('Y-m-01');
           $erdt=date('Y-m-t');
...
//	$qry.=" and DATE(entrydate) >='".$start."' and DATE(entrydate) <='".$end."'";
     	...
     	$qry.=" and CAST(entrydate AS DATE) >='".$start."' and CAST(entrydate AS DATE) <='".$end."'";
	}else
	{
...
//	$qry.=" and DATE(entrydate)>='".$srdt."' and DATE(entrydate)<='".$erdt."'   ";
         ...
         $qry.=" and CAST(entrydate AS DATE)>='".$srdt."' and CAST(entrydate AS DATE)<='".$erdt."'   ";
	}
...
if($tik_id!=""){
    // echo "SELECT qid FROM `quotation_approve_details` where (wbs='".$tik_id."' or vpr='".$tik_id."' or prime='".$tik_id."' or  ticket_no='".$tik_id."' or ref_no='".$tik_id."' or job='".$tik_id."')";
...
$fetchTik=mysqli_fetch_assoc($tik);
while($fetchTik=mysqli_fetch_assoc($tik)){
    $fetchtikqid[] = $fetchTik['qid'];
}

// $fetchtikqidarray = json_encode($fetchtikqid);
// $fetchtikqidarray = str_replace(array('[',']','"'),);


 $fetchtikqid=json_encode($fetchtikqid);
		$fetchtikqid=str_replace( array('[',']','"') , ''  , $fetchtikqid);
		$fetchtikqid=explode(',',$fetchtikqid);
		$fetchtikqid = "'" . implode ( "', '", $fetchtikqid )."'";

// echo mysqli_num_rows($tik);


// echo '<pre>';print_r($fetchtikqid);echo '</pre>';


    $qry.=" and id in (".$fetchtikqid.")";
}
...
$Num_Rows = mysqli_num_rows ($table);
	...
	$getcalldetcntt.="and reqby='".$reqby."'";
      ...
      /* if($_SESSION['custid']!="all" && $_SESSION['custid']!=null )
          {
           $getcalldetcntt.="and cust='".$_SESSION['custid']."'";
        $getcalldetcntt1.="and cust='".$_SESSION['custid']."'";
        $totcalldets.="and cust='".$_SESSION['custid']."'";
...
if($atm=="" & $qid=="" & $reqby=="-1" & $category=="-1" & $strdt=="" & $endt=="" & ($fundtyp=="" || $fundtyp=="-1") & $calltyp=='')
        {
           $srdtt=date('Y-m-01');
           $erdtt=date('Y-m-t');
	$getcalldetcntt.=" and CAST(entrydate AS DATE)>='".$srdtt."' and CAST(entrydate AS DATE)<='".$erdtt."' ";
        $getcalldetcntt1.=" and CAST(entrydate AS DATE)>='".$srdtt."' and CAST(entrydate AS DATE)<='".$erdtt."'";
         $totcalldets.=" and CAST(entrydate AS DATE)>='".$srdtt."' and CAST(entrydate AS DATE)<='".$erdtt."'";
	...
	$endtt=date('Y-m-d', strtotime($dtt2));
         ...
         if($strdt!="" & $endt!="")
        {
            ...
            $getcalldetcntt.=" and CAST(entrydate AS DATE)>='".$startt."' and CAST(entrydate AS DATE)<='".$endtt."' ";
        $getcalldetcntt1.=" and CAST(entrydate AS DATE)>='".$startt."' and CAST(entrydate AS DATE)<='".$endtt."'"; 
         $totcalldets.=" and CAST(entrydate AS DATE)>='".$startt."' and CAST(entrydate AS DATE)<='".$endtt."'";  
...
$qry.=" ORDER BY cust ASC ";
	...
	$qry.=" LIMIT $Page_Start , $Per_Page"; // echo $qry;
...
<?php }?>
	...
	while($row=mysqli_fetch_array($qrys))
	{
	  ...
	  $trdrow=mysqli_fetch_array($trdet);
        ...
        $reject_remark_sql = mysqli_query($con,"select remark from quotation1_reject where qid='".$row[0]."'");
        $reject_remark_res = mysqli_fetch_array($reject_remark_sql);
	...
	<td  align="center" width="80">
	    ...
	    <td  align="center" width="150">
	       ...
	       <td  align="left" width="150">
	           <? //echo "select distinct(particular) from quotation_details where qid='".$row[0]."'"; ?>
	         <table  >
<?php
