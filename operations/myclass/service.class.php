...
Class Service
{
	...
	public function __construct()
	{
		...
		$this->servicetp=$_POST['servicetp'];
...
//echo			$i."- ".$_POST['assets'][$i];
	...
	public function Isvalid()
	{
		...
		if(empty($this->servicetp))
		$this->errors[]="Please Select Service type";
		for($i=0;$i<$this->cnt;$i++)
		{
...
//	echo $num3;
	...
	//echo "<br>INSERT INTO `alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `state`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`, `close_date`, `standby`, `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`serviceid`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->state."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$this->alertdt."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', 'service', '', '', '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$qry2ro[0]."_".date("Ymd").$num3."','".$this->servicetp."')";
	$sql = mysql_query("INSERT INTO `alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `state`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`, `close_date`, `standby`, `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`serviceid`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->state."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$this->alertdt."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', 'service', '', '', '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$qry2ro[0]."_".date("Ymd").$num3."','".$this->servicetp."')");
	...
	for($i=0;$i<count($this->assid);$i++)
		{
			...
			if($this->assid[$i]!="")
			{
			$qry2=mysql_query("select * from assets_specification where ass_spc_id='".$this->assid[$i]."'");
