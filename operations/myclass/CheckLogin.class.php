...
Class CheckLogin
{
	...
	$this->logchk($con);
	 ...
	 {
	     $ch = "select * from login where username='$this->user' and password='$this->pwd' and status=1 and designation>7 ";
	    // echo $ch;die;
	    // echo $this->user;die;
		 //include("config.php");
		// echo "select * from login where username='$this->user' and password='$this->pwd' and status=1 ";
		 $result = mysqli_query($con,"select * from login where username='$this->user' and password='$this->pwd' and status=1 and designation>7 ");
