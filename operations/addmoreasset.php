...

{
	...
	//echo "select po,cid,servicetype from Amc where amcid='".$_POST['id']."'";
		...
		$qry2=mysqli_query($con,"select po,cid,servicetype from Amc where amcid='".$_POST['id']."'");

		elseif($_POST['atm']=='atm')

		$qry2=mysqli_query($con,"select po,cust_id from atm where track_id='".$_POST['id']."'");

		 $qry2row=mysqli_fetch_row($qry2);

		 $qry4=mysqli_query($con,"select assets_name from assets where assets_id='".$_POST['assets']."'");
		...
		$qry3=mysqli_query($con,"Insert into amcassets(`assets_name`,`assetspecid`,`amcid`,`amcpoid`,`siteid`,`quantity`) Values('".$qryrow4[0]."','".$_POST['spec']."','".$qry2row[1]."','".$qry2row[0]."','".$_POST['id']."','".$_POST['quan']."')");
...

{
	...
	$q=mysqli_query($con,"Insert into servicemonth(`po`,`date`,`type`,`siteid`) Values('".$po2."','".date("Y-m-d",strtotime("+".$j." months", $start))."','AMC','".$_POST['id']."')");
...
$qry=mysqli_query($con,"select * from assets");
