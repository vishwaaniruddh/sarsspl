...
$insqry=mysqli_query($con,"Insert into quotation_edit_history(`qid`, `quotationdetails_id`, `particular`, `description`, `quantity`, `rate`, `total`,`Quot_amt`, `reqby`, `entrydate`, `status`,`remark`, `filename`,tcode,uom,ServiceTax) values('".$qid."','".$detailid."','".$row[2]."','".$row[3]."','".$row[4]."','".$row[5]."','".$row[6]."','".$tqtamt."','".$srno[0]."','".$dt."','u','".$rem."','".$image_name."','".$row[7]."','".$row[8]."','".$row[9]."')");
if(!$insqry)
{
...
$upqry=mysqli_query ($con,"Update quotation_details SET `description`='".$pr."',`quantity`='".$prc."',`rate`='".$rate."',`total`='".$amt."',tcode='".$tcode."',uom='".$descdet."',ServiceTax='".$stx1."' where id='".$detailid."' and qid='".$qid."'");
if(!$upqry)
{
