...
if($time==2 && $subject>0){
        ...
        $sql2="SELECT sum(points2) as points,p2 from quiz_result where points2>0 and MONTH(created_at) = MONTH(CURRENT_DATE()) AND YEAR(created_at) = YEAR(CURRENT_DATE()) and p2!='AI'  and is_count_points2=1 and p1 not like '%Gr%' and p2 not like '%Gr%' GROUP BY p2 ORDER BY points DESC LIMIT 10";
...
while($sql_result2=mysqli_fetch_assoc($sql2)){    
    ...
    if($p2 || !empty($p2)){
            ...
            $data[]=['id'=>$p2,'name'=>$name,'points'=>$points2, 'rank'=> $rank];
            $rank++;
    }
