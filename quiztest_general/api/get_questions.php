...
$select_subtopic_sql=mysqli_query($con,"SELECT distinct(sub_topic) from quiztest where topic='".$topic_in."'");  // and sub_topic in(220)                
...
for($loop=0;$loop<$desiredLength;$loop++){
    ...
    $sql=mysqli_query($con,"select distinct srno,a,b,c,d,final_ans,ideal_time,mcq,topic,sub_topic,imgf from quiztest WHERE topic='".$topic."' and sub_topic='".$main_subtopicArray[$loop]."'  order by rand() ASC LIMIT 1");    
    
    // echo "select distinct srno,a,b,c,d,final_ans,ideal_time,mcq,topic,sub_topic,imgf from approve_qstnx WHERE topic='".$topic."' and sub_topic=123 order by rand() ASC LIMIT 1";
  ...
  if($result['imgf']){
    ...
    $options=array("a"=>$result["a"],"b"=>$result["b"],"c"=>$result["c"],"d"=>$result["d"]);
        ...
        $data[]= ['data'=>['id'=>$result['srno'],'topic'=>$result['topic'],'sub_topic'=>$result['sub_topic'],'is_image'=>$is_image,'mcq'=>$mcq,'options'=>$options,'final_ans'=>$result['final_ans'],'ideal_time'=>$ideal_time]];
  }
...
$userid=$_POST['userid'];
        ...
        // $insert_sql="INSERT INTO quiz_result(standard,p1,p2,questions_ids,topic_ids,total_questions,answers) VALUES('".$std."','".$userid."','AI','".$q_id."','".$get_topic."','".$loop."','".$ans_id."')";
        // // echo $insert_sql;
