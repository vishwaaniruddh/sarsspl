...
foreach($q_arr as $key => $val){
    ...
    if($get_result['imgf']){
      $question_string="https://smartscoreanalytics.com/qstn_img/".$std."_".strtoupper($topic_name)."/".$get_result["imgf"];
      $is_image='1';
}
else{
    ...
    $get_correct_string=mysqli_query($con,"select $correct_option from quiztest where srno='".$qid."'");
    $get_correct_string_result=mysqli_fetch_assoc($get_correct_string);
    ...
    $get_selected_ans_string=mysqli_query($con,"select $mygroupSelectedOption from quiztest where srno='".$qid."'");
    $get_correct_ans_string_result=mysqli_fetch_assoc($get_selected_ans_string);
    $selected_string=$get_correct_ans_string_result[$mygroupSelectedOption];


$opp_string_sql=mysqli_query($con,"select $oppGroupSelectedOption from quiztest where srno='".$qid."'");

    $opp_string_sql_result=mysqli_fetch_assoc($opp_string_sql);
    $oppString=$opp_string_sql_result[$oppGroupSelectedOption];
    
    


    $sql=mysqli_query($con,"select * from group_response where groupid='".$group1."' and testid='".$testid."' and question_id='".$qid."'");
    ...
    $mygroupresponse='';            
        ...
        while($sql_result=mysqli_fetch_assoc($sql)){
            ...
            $get_string=mysqli_query($con,"select $response from quiztest where srno='".$qid."'");
            $get_string_result=mysqli_fetch_assoc($get_string);
...
$opp_string_sql=mysqli_query($con,"select $oppGroupSelectedOption from quiztest where srno='".$qid."'");
    ...
    $sql=mysqli_query($con,"select * from group_response where groupid='".$group2."' and testid='".$testid."' and question_id='".$qid."'");
