...
$check_sql=mysqli_query($con,"SELECT * from group_ready_notification where group_id='".$groupid."'");

if($check_sql_result=mysqli_fetch_assoc($check_sql)){
    


$readysql=mysqli_query($con,"SELECT sum(is_accepted) as state FROM group_ready_notification where group_id='".$groupid."' and is_accepted=1 ");

if($ready_result=mysqli_fetch_assoc($readysql)){
$state=$ready_result['state'];

    if($state>=1){
        $online='1';
        $update=mysqli_query($con,"update group_ready_notification set is_ready=1 where group_id='".$groupid."'");
    }
    else{
        $online='0';
        $update=mysqli_query($con,"update group_ready_notification set is_ready=0 where group_id='".$groupid."'");
    }
