...
$sql=mysqli_query($con,"SELECT CONCAT(member0,',',member1,',',member2,',',member3,',',created_by) as all_members,group_name,id,created_by FROM groups");
...
while($sql_result=mysqli_fetch_assoc($sql)){
  ...
  for($i=0;$i<count($member_arr);$i++){
            ...
            if($member_arr[$i]==$userid){
                    ...
                    $get_group_info_sql=mysqli_query($con,"select CONCAT(member0,',',member1,',',member2,',',member3) as group_members from groups where group_name='".$group_name."'");
                    
                                while($get_group_info_sql_result=mysqli_fetch_assoc($get_group_info_sql)){
