...
//echo "SELECT count(*) FROM quiz_result WHERE MONTH(created_at) = MONTH(CURRENT_DATE()) AND YEAR(created_at) = YEAR(CURRENT_DATE()) and (p1='".$userid."' OR p2='".$userid."') and player_won='".$userid."' and topic_ids='".$topic."'";
...
while($sql_result1=mysqli_fetch_assoc($sql1)){
    
     if($sql_result1['topic_ids']){
         $array[]=$sql_result1['topic_ids'];
...
//$pts=$points1;
    mysqli_query($con,"insert into leaderboards (playerid,quizid,points,subject,quizdate) values('".$sql_result['p1']."','".$testid."','".$points1."','".$subject."','".$date."')");
    }

     if($userid==$sql_result['p2']){
    $points1 = 16;
     $update_leaderboard=mysqli_query($con,"update quiz_result set is_count_points2=1 where id='".$testid."'");
     mysqli_query($con,"insert into leaderboards (playerid,quizid,points,subject,quizdate) values('".$sql_result['p2']."','".$testid."','".$points2."','".$subject."','".$date."')");  
