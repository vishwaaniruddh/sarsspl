...
if($testid){
    ...
    $sql=mysqli_query($con,"select p2 from quiz_result where id='".$testid."'");
    $sql_result=mysqli_fetch_assoc($sql);
    $ai_player=$sql_result['p2'];
}
else{
//$sql=mysqli_query($con,"select * from ai_players where standard='".$std."' order by rand() limit 1");
$ai_player = rand(701, 988);
//$sql_result=mysqli_fetch_assoc($sql);


//$ai_player=$sql_result['id'];    
}
...
for($loop=0;$loop<$desiredLength;$loop++){
        ...
        $options=array("a"=>$result["a"],"b"=>$result["b"],"c"=>$result["c"],"d"=>$result["d"]);
            ...
            $insert_sql="UPDATE quiz_result set questions_ids='".$q_id."',subject='".$subjectid."',total_questions='".$loop."',answers='".$ans_id."', p1_status=3, p1_status_time=now(), p2_status=3, p2_status_time=now() where id='".$testid."'";
            ...
            // set stat for ai player /// we dont want
            
            // $state="update ai_players set is_busy=1 where id='".$ai_player."'";
