...
$pre_sql=mysqli_query($con,"SELECT p1,p2,id from quiz_result where id='".$previous_testid."'");

$pre_sql_result=mysqli_fetch_assoc($pre_sql);
...
$rematch_sql=mysqli_query($con,"INSERT INTO rematch(player1,testid,player2,standard,	topic,subject,againstid,is_accepted) values('".$player1."','".$testid."','".$p2."','".$std."'	,'".$topic."','".$subject."','".$againstid."',0)");
...
$check_questions=str_replace( array('[',']','"') , ''  , $check_questions);
  ...
  $get_sql=mysqli_query($con,"select * from quiztest WHERE sub_topic='".$main_subtopicArray[$i]."' and srno not in('". $check_questions."') order by rand() ASC LIMIT 1");
