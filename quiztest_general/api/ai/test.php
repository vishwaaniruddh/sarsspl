...
//  $check_questions= "'" . implode ( "', '", $sr ) . "'"; 

// $check_questions=implode(',',$check_questions);
// echo $check_questions;
echo '<br>';
...
// var_dump($sr);
    ...
    $sql=mysqli_query($con,"select * from quiztest WHERE sub_topic='".$main_subtopicArray[$loop]."' and srno not in('". $check_questions."')  order by rand() ASC LIMIT 1");    
    
    echo "select * from quiztest WHERE sub_topic='".$main_subtopicArray[$loop]."' and srno not in(". $check_questions.")  order by rand() ASC LIMIT 1";
