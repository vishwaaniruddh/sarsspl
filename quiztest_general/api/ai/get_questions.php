...
$_sub_sql = mysqli_query($con,"SELECT UNDER FROM `project_catT` where id='".$subjectid."'");
if(mysqli_num_rows($_sub_sql)>0){
    $subjectrow=mysqli_fetch_assoc($_sub_sql);
    $subjectid = $subjectrow['UNDER'];
}
...
if(mysqli_num_rows($sqlm)>0){
    ...
    // echo $insert_sql; Inserted for result
    ...
    if(mysqli_num_rows($select_subtopic_sql)>0){
