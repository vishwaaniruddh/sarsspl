...
if(isset($name) && isset($lname) && isset($email) && isset($schname) && isset($class) && isset($mobile)){
    $checkuserSqlResult = mysqli_query($con,"SELECT `id` FROM `quiz_regdetails` WHERE `emailid` = '".$email."'");
    //$checkuserData = mysqli_fetch_assoc($checkuserSqlResult);
    if(mysqli_num_rows($checkuserSqlResult)==0){
        ...
        if(mysqli_query($con,$sql_login)===TRUE){
            ...
            $data=['email'=>$email,'password'=>$password,'username'=>$user_name,'code'=>200];
            echo json_encode($data);
          //echo '1';
        }else{
            $data=['code'=>201,'msg'=>'Something Went Wrong! Please try again'];
            echo json_encode($data);
          //echo '0';
        }    
    }else{
        $data=['msg'=>'Email Already Exist!!','code'=>203];
        echo json_encode($data);
    }
}
else{
   // echo json_encode('0');
   // echo '0';
   $data=['code'=>202,'msg'=>'Some fields are empty'];
    echo json_encode($data);
}
