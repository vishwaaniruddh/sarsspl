...
for($i=1;$i<3;$i++){
    ...
    if($subjectid > 0){
        $sql ="select * from quiz_result where p$i='".$userid."' and $friend_in in (".$result_ar.") and subject='".$subjectid."' order by id desc limit 20";    
    }
    else{
        $sql = "select * from quiz_result where p$i='".$userid."' and $friend_in in (".$result_ar.") order by id desc limit 20";
    }
...
$data[]=['testid'=>$testid,'p2'=>$opp,'name'=>ucfirst($name),'p1_correct_count'=>$correct_count,'topic'=>$topic,'subject'=>$subject,'p2_correct_count'=>$opp_count,'player_won'=>$player_won,'date'=>$date,'timestamp'=>$timestamp1];        
