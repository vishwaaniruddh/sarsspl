...
$sql=mysql_query("SELECT concat(member0,',',member1,',',member2,',',member3,',',created_by) as member from groups WHERE id='".$group1."'",$con);
...
foreach($member as $key => $value){
    if($value!=0){
        mysql_query("INSERT INTO group_response(testid,groupid,player) VALUE($testid,$group1,$value)",$con);        
    }
...
$result_ar = "'" . implode ( "', '", $arr ) . "'";
  ...
  $get_sql=mysql_query("select * from quiztest WHERE sub_topic='".$main_subtopicArray[$i]."' and srno not in(".$result_ar.")  order by rand() ASC LIMIT 1",$con);
...
//  echo $update_sql;
    ...
    $notification_sql1=mysql_query("SELECT concat(member0,',',member1,',',member2,',',member3,',',created_by) as members from groups where id='".$group1."'",$con);

while($not_result1=mysql_fetch_assoc($notification_sql1)){
    ...
    foreach($member_arr1 as $key =>$value){
        ...
        if($value!=0){
        $insert_sql1=mysql_query("INSERT into group_quiz_notification(groupid,testid,player,standard,topic,subject) VALUES('".$group1."','".$testid."','".$value."','".$standard."','".$topic."','".$subid."')",$con);            
        }
 ...
 $created_by_update=mysql_query("UPDATE group_quiz_notification set is_accepted=1, where player='".$userid."' and testid='".$testid."' and groupid='".$group1."'",$con);
