...
if($testid){
    ...
    $sql=mysql_query("select p2 from quiz_result where id='".$testid."'",$con);
    $sql_result=mysql_fetch_assoc($sql);
    $ai_player=$sql_result['p2'];
}
else{
$sql=mysql_query("select * from ai_players where standard='".$std."' and is_busy=0 order by rand() limit 1",$con);
$sql_result=mysql_fetch_assoc($sql);
