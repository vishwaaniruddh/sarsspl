...
// end  Get the correct Answer abd same answer is for AI id 100% correct
    ...
    //For Check Answers
      ...
      // 3) Get player2 response
            ...
            for($i=0;$i<$total;$i++){
                    $p2_responses[$i]=$final_ans[$i];
            
            if($i>=$loop_count){
                if($p2_responses[$i]==$final_ans[$i]){
                    $p2_responses[$i]=1;
                }
    ...
    if($fetch_result['p1_correct'] == $fetch_result['p2_correct']){
       ...
       $quize_result=['winner'=>$fetch_result['player_won'],'player1'=>$player1_per,'player2'=>$player2_per,'final_ans'=>$final_ans,'ai_ans'=>$p2_responses];
