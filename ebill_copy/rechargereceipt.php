...
<body onload="recbill('1')"><!--old -->
	...
	<center>
		...
		$req = mysqli_query($con, "select start_date,end_date,bill_date,due_date,opening_reading,closing_reading,unit,cust_id,atmid,trackerid,approvedamt from ebillfundrequests where req_no='" . $_GET['reqid'] . "'");
		...
		$ebill = mysqli_query($con, "select meter_no,Consumer_No from " . $reqro[7] . "_ebill where atmtrackid='" . $reqro[9] . "'");

		$ebillro = mysqli_fetch_row($ebill);
		...
		<center>
			...
			<form name="frmeb" method="post" action="processcoupan.php" onsubmit="return validate(this);">
				...
				<input type="hidden" name="page" id="page" value="<?php if (isset($_GET['page'])) {
					echo $_GET['page'];
				} ?>">
					...
					<tr>
						...
						<td valign="top">
							...
							<table>
								...
								<tr>
									<td> Number of receipt(only numbers)</td>
									<td><input type="text" name="reccnt" id="reccnt" value="1"
											onblur="recbill(this.value,'<?php echo $_GET['reqid']; ?>');"></td>
								</tr>

								<tr>
									<td colspan="2">
										<div id="billdet"></div><input type="hidden" name='paid' id='paid' value='paid'>
									</td>
								</tr>

								<tr>
									<td>Paid Date</td>
									<td><input type="text" name="pdt" id="pdt" value="<?php if (mysqli_num_rows($pd) > 0) {
										echo date('d/m/Y', strtotime($pdro[2]));
									} ?>" onclick="displayDatePicker('pdt');"></td>
								</tr>
								...
								<tr>
									<td>Remarks:</td>
									<td><input type="text" name="memo" id="memo" placeholder="memo"></td>
								</tr>

								<tr>
									<td colspan="2"><input type="submit" name="cmdsub" id="cmdsub" value="Save"></td>
								</tr>
