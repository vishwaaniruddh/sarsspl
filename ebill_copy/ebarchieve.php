...
function Remarc(reqid)
  {
  //alert(id+" "+stat);
  if (confirm('Are you sure you want to Remove this request from archieve?')) {
    // Save it!
...
//alert(xmlhttp.responseText);
if(xmlhttp.responseText=='1')
document.getElementById('arc'+reqid).innerHTML="Done";
else
alert("Some Error Occurred. Please Refresh your page and try again");
...
// alert("getcustbank.php?val="+val);
//alert("updatepaidebill.php?pamt="+pamt+"&pdt="+pdt+"&memo="+memo+"&reqid="+reqid+"&paid="+paid);
xmlhttp.open("GET","remarchieve.php?reqid="+reqid,true);
xmlhttp.send();
}
  }
...
<body <?php if(isset($_GET['cid'])){ ?> onload="searchById('Listing','1','');getbank($_GET['cid']);"<?php } ?>>
