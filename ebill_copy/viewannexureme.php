...
if(function_exists('date_default_timezone_set')) date_default_timezone_set($timezone);
	echo "Select * from siteinvoice where invid='".$id."' and compid='".$_GET['compid']."'";
 $query=mysqli_query($con,"Select * from siteinvoice where invid='".$id."' and compid='".$_GET['compid']."'");
...
//    $dd = datediff('y', $from, $to, false);
  ...
  //  echo $nod;
    ...
    ?>
    <center><a href="Editannexure.php?id=<?php echo $id; ?>">Edit Annexure</a></center>
</div>
<div id="annexure">
...
$brkcnt=0;
	...
	//echo "".$cid."-".$bk;
	        $i = 1;
	      	...
	      	$sql = "SELECT * FROM siteinvoiceatm where invid='".$id."' and status=0 and compid='".$_GET['compid']."'";
	...
	echo $sql;
		...
		$qryme=mysqli_query($con,"Select site_id,atm_id2,atmsite_address,city,state,zone,region,location,city_category,atm_id1,projectid,bank from ".$cid."_sites where id='".$row[2]."'");
		...
		?>
		 ...
		 <tr height="8px" >
         <td><?php echo $i; ?></td>
         <td><?php echo $qryout[10]; ?></td>
         <?php if($cid=='Prizm07'){ ?><td scope="col"><div align="center">&nbsp;<?php echo $cid; ?></div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[12]; ?> </div></td>
  ...
  <td><?php echo $qryout[8]; ?></td>
   ...
   {
   ?>
   <td scope="col"><div align="center">&nbsp;<?php echo $qryout[11]; ?></div></td>
  ...
  <td>&nbsp;<?php echo $qryout[8]; ?></td>
   <?php
         ...
         <td>&nbsp;<?php  if(strtotime($start_date)<strtotime($row[6])){ echo date('d/m/Y',strtotime($row[6])); }else{ echo date('d/m/Y',strtotime($start_date)); } ?> </td>
         <td>&nbsp;<?php if($row[5]!='0000-00-00'){ echo date('d/m/Y',strtotime($row[5]));}else{ echo date('d/m/Y',strtotime($end_date));  }  ?></td>
	 ...
	 /*if(strtotime($row[6],0)<strtotime($start_date,0)){
      ...
      if($row[11]=='')
                              ...
                              // echo $sub;
		 if($row[12]=="A" || $row[12]=='a'){$ca++; $ra=$row[11];
		 $numaa=0;
                      $counta=0;
     ...
     $aamt[$key]=(($aamt[$key])+$row[10]);
       ...
       $aamt[$key]=(($aamt[$key])+$row[10]);
        ...
        $aamt[$key]=(($aamt[$key])+$row[10]);
                            ...
                            else if($row[12]=="B" || $row[12]=='b'){  $cb++; $rb=$row[11];
                            //B Category Subtype
                             $numbb=0;
     ...
     $bamt[$key]=(($bamt[$key])+$row[10]);
       ...
       $bamt[$key]=(($bamt[$key])+$row[10]);
        ...
        $bamt[$key]=(($bamt[$key])+$row[10]);
                               ...
                               else if($row[12]=="C" || $row[12]=='c'){$cc++; $rc=$row[11];
                                  //C Category Subtype
                             $numcc=0;
   ...
   //echo $sub;
       $countna=0;
                                if(in_array($sub,$nacnt))
                                       {
                                 // echo "****".    $key=array_search($row[12], $nacnt);
     ...
     $naamt[$key]=(($naamt[$key])+$row[10]);
      ...
      $naamt[$key]=(($naamt[$key])+$row[10]);
       ...
       $naamt[$key]=(($naamt[$key])+$row[10]);
         ...
         if($row[5]=='0000-00-00')
         {
          $var= number_format(($dd/$nod)*$row[11], 2, '.', '');
         // $tot=$tot+(($dd/$nod)*$row[11]);
          if($row[12]=="A")$aa=$aa+$row[10];
                                       else if($row[12]=="B")$ab=$ab+$row[10];
                                       else if($row[12]=="C")$ac=$ac+$row[10];    
                                       else $an=$an+$row[10]; 
         }
	 else
	 {
          $var= number_format(($dd1/$nod)*$row[11], 2, '.', '');
        // $tot=$tot+(($dd1/$nod)*$row[11]);
          if($row[12]=="A")$aa=$aa+$row[10];
                                       else if($row[12]=="B")$ab=$ab+$row[10];
                                       else if($row[12]=="C")$ac=$ac+$row[10];    
                                       else $an=$an+$row[10]; 
		 ...
		 //$tot=$tot+(($var/$nod)*$row[11]);
                                     
                                       }
                                   else{
                                          number_format(($dd1/$nod)*$row[11], 2, '.', '');
										// $tot=$tot+(($dd1/$nod)*$row[11]);
                                       if($row[12]=="A")$aa=$aa+$row[10];
                                       else if($row[12]=="B")$ab=$ab+$row[10];
                                       else if($row[12]=="C")$ac=$ac+$row[10];
