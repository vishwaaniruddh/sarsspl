...
if ($_FILES['email_cpy']['name'] != '') {
	...
	if ($size > $maxsize) {
		...
		if ($fichier) {
			...
			//echo $image_name;
			$newname = "../operations/quotuploads_tis/cancel_tis/" . $image_name;
			//echo $newname;	
...
$insqry = mysqli_query($con, "INSERT INTO `cancelled_quotation_tis`(`qid`,`remark`, `reqby`, `cancdate`,filename) VALUES ('" . $qotid . "','" . $rem . "','" . $srno[0] . "','" . $dt . "','" . $image_name . "')");
if (!$insqry) {
	$error++;
}
$upqry = mysqli_query($con, "Update  `quotation1_tis` set status='c' where id='" . $qotid . "' ");
if (!$upqry) {
	$error++;
}
