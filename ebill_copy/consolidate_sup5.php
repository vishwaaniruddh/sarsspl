<?php 
include('config.php');
$accsop=mysqli_query($con,"select * from fundaccounts where aid='".$_REQUEST['sup']."'");
...
if($req=="")
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.supervisor=a.hname and a.aid='".$opening_fund['aid']."') and upby!='' and entrydt>='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['frmdt'])))." 00:00:00' AND  entrydt<='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['todt'])))." 23:59:59'");
else
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.req_no not in($req) and r.supervisor=a.hname and a.aid='".$opening_fund['aid']."') and upby!='' and entrydt>='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['frmdt'])))." 00:00:00' AND  entrydt<='".date('Y-m-d',strtotime(str_replace("/","-",$_POST['todt'])))." 23:59:59'");
...
//echo "SELECT sum(pamount) AS TotalItemsOrdered FROM onaccount_reversal where accid='".$opening_fund['aid']."'";
$rev_qry=mysqli_query($con,"SELECT sum(pamount) AS TotalItemsOrdered FROM onaccount_reversal where accid='".$opening_fund['aid']."' where pdate>'2017-04-01' ");

//echo "SELECT sum(pamount) AS TotalItemsOrdered FROM onaccount_reversal accid='".$opening_fund['aid']."'";
//echo "hello";
...
<td><?php echo $total+$total2+$total3+$total4-$paidamt6+$db+$cr; ?></td>
...
$outs=($total+$total2+$total3+$total4-$paidamt4)-($podqrya1[0]+$podqrya[0]+$paidamt+$paidamt2+$paidamt3+$paidamt6)+$db+$cr;
echo $outs; ?></td>
</tr>
<?php
	$i++;
