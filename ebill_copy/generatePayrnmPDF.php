...
<?php	
        $total=0;
$qry1=mysqli_query($con,"select max(tid) from rnmfundtransfers");        
...
//echo count($reqs)." ".$tid;
	...
	$sql="insert into rnmfundtransfers(tid,reqid,accid,pdate,chqname,chqno,dbtaccno) values('$tid','$reqs[$x]','$accname[$x]','$dt','$chqname','$chqno','".$_POST['dbtacc']."')";	
//echo $sql;	
        $table=mysqli_query($con,$sql); 

        $sql="insert into quotapproval(quotid,appby,apptime,level) values('$reqs[$x]','$userid','$dt','8')";		
        $table=mysqli_query($con,$sql); 
        $sql="update quotation set status='8',chqno='$chqno' where quotid='$reqs[$x]'";		
        $table=mysqli_query($con,$sql);
        }  
        }  
         //echo "select distinct(accid) from rnmfundtransfers where tid='$tid'";       
$qry1=mysqli_query($con,"select distinct(accid) from rnmfundtransfers where tid='$tid'");
$ct=1;
...
$qry2=mysqli_query($con,"select sum(approvedamt) from quotation where quotid in(select reqid from rnmfundtransfers where tid='$tid' and accid='$qrrow[0]')");
