...
if($req=="")
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.supervisor=a.hname and a.aid='".$opening_fund['aid']."') and upby!=''  and r.req_no in($fnreqid)) and upby!=''");
else
$ebpay=mysqli_query($con,"select * from ebpayment where  Bill_No in(select r.req_no from ebillfundrequests r,fundaccounts a where r.req_no not in($req) and r.supervisor=a.hname and a.aid='".$opening_fund['aid']."' and r.req_no in($fnreqid)) and upby!='' ");
...
echo ($total+$total2+$total3+$total4+$total5+$total6)-($paidamt+$paidamt2+$paidamt3+$paidamt4+$paidamt5+$paidamt6); ?></td>
</tr>
<?php
	$i++;
	}
