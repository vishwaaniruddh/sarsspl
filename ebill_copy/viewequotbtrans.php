...
$tid=$_GET['transid'];
include('config.php');
//echo "select * from ebfundtransfers where chqno='".$tid."' and status=0";
       $tra=mysqli_query($con,"select * from quotation1ftransfers where tid='".$tid."' and status!=0");
...
<tr><td width="600" >TO	</td><td>			<B>DATE :- <?php if($traro[3]!="0000-00-00"){ echo date('d-m-Y',strtotime($traro[3])); }?></B>	</td></tr>
...
<tr><td width="600" ></td><td>			</td></tr>																				
<tr><td width="600" >Please debit  on my a/c No. <?php echo $traro[6]; ?> & my account recognised as</td><td>			</td></tr>													
...
<?php	
    ...
    //echo "select distinct(accid) from ebfundtransfers where chqno='$tid' and status=0";
    
$qry1=mysqli_query($con,"select distinct(accid) from quotation1ftransfers where tid='$tid' and status!=0 and accid<>90");
$ct=1;
...
//$qry2=mysqli_query($con,"select sum(req_amt) from quotation1_req where qid in(select qid from quotation1ftransfers where tid='$tid' and accid='$qrrow[0]' and status!=0)");
$qry2=mysqli_query($con,"select sum(tamount) from quotation1ftransfers where tid='$tid' and accid='$qrrow[0]' and status!=0");
...
<?php
 ...
 //echo "select accid,reqid,narration from ebfundtransfers where chqno='$tid' and accid=90 and status=0";
 $qry3=mysqli_query($con,"select accid,qid from quotation1ftransfers where chqno='$tid' and accid=90 and status=0");
//$ct=1;
while($qrrow3=mysqli_fetch_array($qry3))
{
$branch3=mysqli_query($con,"select * from fundaccounts where aid='$qrrow3[0]'");
$brro3=mysqli_fetch_row($branch3);
//$deptde=mysqli_query($con,"select `desc` from department where deptid='2'");
//$dtro=mysqli_fetch_row($deptde);
//$crow=mysqli_fetch_row($qry1);	
//echo "select approvedamt from ebillfundrequests where req_no ='".$qrrow3[1]."'";
$qry3=mysqli_query($con,"select tamout from quotation1ftransfers where qid ='".$qrrow3[1]."'");
$qrr3=mysqli_fetch_array($qry3);
?><div class=article>
<div class=title><tr>
<td width="10" align='center'><?php echo $ct++; ?></td>
<td width="200" align='left'><?php echo $brro3[5]."-".$qrrow3[2]; ?></td>
<td width="180px"  align="center"><?php echo $brro3[2]; ?></td>
<td width="60" align='right' style='padding-right:15px'><?php echo number_format($qrr3[0],2); $total=$total+$qrr3[0]; ?></td>
<td width="200" align='left'><?php echo $brro3[4]; ?></td>
