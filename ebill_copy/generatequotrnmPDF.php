...
//echo count($reqs)." ".$tid;
	...
	$supvc=mysqli_query($con,"select supervisor from quotation1 where id='".$reqs[$x]."'");
	...
	$greamt=mysqli_query($con,"select req_amt from quotation1_req where qid='".$reqs[$x]."'");
            $reqamtw=mysqli_fetch_array($greamt);
	
	
	$sql="insert into quotation1ftransfers(tid,qid,accid,dbtaccno,tamount,entrydt,email_body,mail_by,pdate,chq_name,chqno,pay_typ) values('$tid','$reqs[$x]','".$supvca[0]."','".$dbt."','". $reqamtw[0]."','$dt','".$mbd."','".$maby."','".$trdt."','".$chqname."','".$chqno."','".$paytyps."')";	
//echo $sql."<br>";
        $table=mysqli_query($con,$sql); 

        $sql1="insert into quotation1pproval(qid,appby,entrydt,level) values('$reqs[$x]','$userid','$dt','2')";		
//echo $sql1."<br>";
        $table1=mysqli_query($con,$sql1);
 
        $sql3="update quotation1 set p_stat='2' where id='$reqs[$x]'";	
//echo $sql3."<br>";	
...
$qry1=mysqli_query($con,"select distinct(accid) from quotation1ftransfers where tid='$tid'");
$ct=1;
...
<input type="button" name="excl" id="excl" value="Export to Excel" onclick="window.open('cmsexp.php?tid=<?php echo $tid; ?>','_blank');" />
<br><br>
      <a href="view_quot_fr.php" >HOME</a>   
