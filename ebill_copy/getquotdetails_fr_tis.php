...
//echo $_POST['stat'];
	...
	//$qry="Select * from quotation1 where status!='c'  and  local_status='0' ";
       $qry="Select * from quotation1_tis where local_status='0' and ( ((status='a' or status='app') and category='a') or (status='y' and category='f'))";
	...
	$end=date('Y-m-d', strtotime($dt2));
     ...
     if( $_POST['stat']=='0' || $_POST['stat']=='100')
	   //  if( $_POST['stat']=='0')
	     {
	/*echo "Select * from quotation1 where status!='c' and DATE(entrydate) >='".$start."' and DATE(entrydate) <='".$end."'";*/

	$qry.=" and DATE(entrydate) >='".$start."' and DATE(entrydate) <='".$end."' and DATE(Archived_date)='0000-00-00' ";

          }
          else if(  $_POST['stat']=='101')
	
        {
          $qry.=" and DATE(Archived_date) >='".$start."' and DATE(Archived_date) <='".$end."'";
...
$qry.=" and id in($notin2) ";
	...
	$qry.=" and reqby='".$_POST['accname']."' ";
          ...
          if($_POST['rnmtyp']!="")
	 {
          $qry.= " and id in(select qid from quotation1_req_tis where reqby='".$_POST['rnmtyp']."' ) ";
         }
...
<?php }?>
	...
	while($row=mysqli_fetch_array($qrys))
	{
	  ...
	  $chcatm=mysqli_query($con,"select atm from quotation1_tis where atm='".$row[3]."' and p_stat='1'");
...
if($row[2]=='ICICI' || $row[2]=='RATNAKAR' || $row[2]=='ICICI_Direct' || $row[2]=='Knight_Frank' || $row[2]=='kotak')
{
?>
<table border='1'>
<?php
...
<?php
	    ...
	    {
	    $sup1=mysqli_query($con,"select chq_name from quotation1ftransfers_tis where qid='".$row[0]."'");
 ...
 echo  round($tamt[0]); $totamt=$totamt+round($tamt[0]);
	    ...
	    <td><?php
	    $expectamtqry=mysqli_query($con,"select expectedApprovalAmt from quotation_approve_details_tis where qid='".$row[0]."'");
	   ...
	   <?php 
	      ...
	      {
	      $amtqry=mysqli_query($con,"select app_amt,filename,app_by,approved_date,req_amt,ticket_no,remark from quotation_approve_details_tis where qid='".$row[0]."'");
...
if(($_SESSION['designation']=="20" || $_SESSION['designation']=="14") && $_SESSION['serviceauth']=='2' && ($_SESSION['dept']=='6' || $_SESSION['dept']=='7'))
...
$getrnmreq=mysqli_query($con,"select * from quotation1_req_tis where qid='".$row[0]."'");
...
{
$rejrem=mysqli_query($con,"select remark from quotation1_reject_tis where qid='".$row[0]."'");
