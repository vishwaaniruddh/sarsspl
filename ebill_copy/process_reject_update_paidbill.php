...
echo "0";
	...
	$qry1=mysqli_query($con,"INSERT INTO `rejected_update_ebill` (`req_no`, `email`, `ccemail`, `reason`, `remarks`,`entrydate`,`rejectedby`) VALUES ('$reqid', '$email', '$cc', '$reason', '$remarks','$dt',$srno[0])");
	if(!$qry1)
	...
	for($i=0;$i<count($reason1);$i++)
	{
		$qry3=mysqli_query($con,"select reason from rejection_updatebill where id=".$reason1[$i]);
