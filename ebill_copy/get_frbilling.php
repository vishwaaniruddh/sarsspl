...
if($_POST['sdate']!='' & $_POST['edate']!='')
 ...
 $qry="Select * from quotation1 where  ((status='a' or status='app') and category='a' )and billing_status='n' ";
...
if($_POST['cust']=="EUR08")
 ...
 $adq="SELECT qid FROM `quotation_approve_details` where filename!=''";
	...
	{
 $adq="SELECT qid FROM `quotation_approve_details` where 1 ";
 }

$qiddarr=array();
$adqry=mysqli_query($con,$adq);
while($atrws1=mysqli_fetch_array($adqry))
{

$qiddarr[]=$atrws1[0];
}

$qiddstr=implode(',',$qiddarr);
if($_POST['sdate']!='' & $_POST['edate']!='')
                 {
 ...
 if($_POST['quotid']!="")
	   {
	   $qry.=" and quot_id LIKE '"."%".$_POST['quotid']."%"."'";
	   }

           
          if($_POST['mnth']!="")
	   {
	   $qry.=" and month='".$_POST['mnth']."'";
	   }
  
  if($_POST['yr']!="")
	   {
	   $qry.=" and year='".$_POST['yr']."' ";
	   }
...
<button type="button" id="myButtonControlID" onClick="tableToExcel('custtable');">Export Table data into Excel</button>
...
?>
 <a href='../operations/quotuploads/approve/<?php echo $approw[8]; ?>' download="<?php echo $dwnlname; ?>">Download</a>
 <?php
 }
 ?>
...
<input type="checkbox" name="qidsel[]" id="qidsel<?php echo $srn;?>" value="<?php echo $row[0];?>" checked>
...
<tr height="45">
<td colspan="27" align="center">Invoice Number-<input type='text' name="invno" id="invno" /></td>
</tr>
<tr height="45">
<td colspan="27" align="center"><input type="button" name="subsb" id="subsb" onclick="subfunc();" value="Submit"></td>
</tr>
</table>
