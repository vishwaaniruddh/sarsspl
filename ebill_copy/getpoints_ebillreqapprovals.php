...
<?php
	...
	//echo "SELECT reason FROM `rejection_reasons` WHERE `type` = '$type' AND `desig` = '$desig' AND `service_auth` = '$service' AND `dept` = '$dept' AND `status` = 1";
	...
	//echo $row1[0];
	//echo "SELECT * FROM `rejection_reasons_detail` WHERE `id` IN ($row1[0])";
