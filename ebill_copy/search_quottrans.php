...
//echo "select sum(tamount),chqno,count(chqno) from quotation1ftransfers status!=0 and group by tid order by qid ASC";
				$sql=mysqli_query($con,"select sum(tamount),chqno,count(chqno),tid from quotation1ftransfers where status!=0 group by tid order by tid ASC");
 ...
 $tot="";
				...
				$req=mysqli_query($con,"select pdate from quotation1ftransfers where chqno='".$row[1]."' and tid='".$row[3]."'");
