...
function calccc(cnt){
//alert(cnt);
if (confirm('Amount might change if you proceed further. Are you sure you want to Proceed?')) {
    // Save it!
var tkdt=document.getElementById('tkdt'+cnt).value;
var frmdt=document.getElementById('frmdt'+cnt).value;
var enddt=document.getElementById('endt'+cnt).value;
//alert(tkdt);
var htdt=document.getElementById('hkdt'+cnt).value;
//alert(htdt);
var rate=document.getElementById('rt'+cnt).value;
//alert(rate);
...
var ex=xmlhttp.responseText;
var st=ex.split("****");
//document.getElementById('rt'+cnt).value;
document.getElementById('mt'+cnt).value=st[2];
//alert(mt);
document.getElementById('days'+cnt).value=st[0];

	
    }
  }
//alert("siteratechange.php?tkdt="+tkdt+"&htdt="+htdt+"&rate="+rate+"&frmdt="+frmdt+"&enddt="+enddt);
xmlhttp.open("GET","siteratechange.php?tkdt="+tkdt+"&htdt="+htdt+"&rate="+rate+"&frmdt="+frmdt+"&enddt="+enddt,true);
xmlhttp.send();
}
}
</script>
...
alert("reminvsite.php?id="+id+"&tbl=siteinvoiceatmme&stat=1&field=id");
xmlhttp.open("GET","reminvsite.php?id="+id+"&tbl=siteinvoiceatm&stat=1&field=id",true);
xmlhttp.send();
}
  }
function Editsite(id,cnt,invid)
  {
  //alert(id+" "+cnt);
  if (confirm('Are you sure you want to Edit this site?')) {
    // Save it!
var tkdt=document.getElementById('tkdt'+cnt).value;
//alert(tkdt);
var htdt=document.getElementById('hkdt'+cnt).value;
...
var rate=document.getElementById('rt'+cnt).value;
//alert(rate);
var mt=document.getElementById('mt'+cnt).value;
var mt2=document.getElementById('mt2'+cnt).value;
//alert(mt);
var ndays=document.getElementById('days'+cnt).value;
//alert(ndays);
//alert(tkdt+" "+htdt+" "+rate+" "+mt+" "+ndays);
  	if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
  xmlhttp=new XMLHttpRequest();
  }
...
//alert(xmlhttp.responseText);
document.getElementById('rem'+cnt).innerHTML=xmlhttp.responseText;
if(xmlhttp.responseText=='1')
document.getElementById('rem'+cnt).innerHTML="Edited Successfully";
else
alert("Some Error Occurred. Please Refresh your page and try again");
...
//alert("edtinvsite.php?id="+id+"&tkdt="+tkdt+"&htdt="+htdt+"&rate="+rate+"&amt="+mt+"&ndays="+ndays);
xmlhttp.open("GET","edtinvsite.php?id="+id+"&tkdt="+tkdt+"&htdt="+htdt+"&rate="+rate+"&amt="+mt+"&ndays="+ndays+"&amt2="+mt2+"&invid="+invid,true);
xmlhttp.send();
}
  }
...
if(function_exists('date_default_timezone_set')) date_default_timezone_set($timezone);
	//echo "Select * from siteinvoice where invid='".$id."'";
 $query=mysqli_query($con,"Select * from siteinvoice where invid='".$id."'");
...
//    $dd = datediff('y', $from, $to, false);
  ...
  //  echo $nod;
    ...
    ?>
    <center><a href="viewannexureme.php?id=<?php echo $id; ?>">View New Annexure</a></center>
</div>
<div id="annexure">
...
$brkcnt=0;
	...
	//echo "".$cid."-".$bk;
	        $i = 1;
	      	...
	      	$sql = "SELECT * FROM siteinvoiceatm where invid='".$id."' and status=0";
	...
	//echo $sql;
		$atm='';
		$brkcnt=0;
		$siteinv = mysqli_query($con,$sql);
		//echo mysqli_num_rows($result);
		while($row = mysqli_fetch_row($siteinv))
		{
		//echo "hi";
		//echo $qryme="Select site_id,atm_id2,atmsite_address,city,state,zone,region,location,city_category from ".$cid."_sites where id='".$row[2]."'";
		$qryme=mysqli_query($con,"Select site_id,atm_id2,atmsite_address,city,state,zone,region,location,city_category,atm_id1 from ".$cid."_sites where id='".$row[2]."'");
		 $qryout=mysqli_fetch_row($qryme);
		//echo $sit=$qryout[0];	
		//echo $atmm=$qryout[1];
		//echo $addm=$qryout[2];
		//echo $citm=$qryout[3];
		...
		?>
         ...
         <?php if($cid=='Prizm07'){ ?><td scope="col"><div align="center">&nbsp;<?php echo $cid; ?></div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $bk; ?> </div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[3]; ?></div></td> 
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[4]; ?> </div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[6]; ?> </div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[9] ?></div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[0]; ?></div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[1]; ?></div></td>
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[7]; ?></div></td>
  <td><?php echo $qryout[8]; ?></td>
   ...
   ?>
   <td scope="col"><div align="center">&nbsp;<?php echo $bk; ?></div></td>
   <td scope="col"><div align="center">&nbsp;<?php echo $qryout[5]; ?> </div></td>
   <td scope="col"><div align="center">&nbsp;<?php echo $qryout[9]; ?></div></td>
    <td scope="col"><div align="center">&nbsp;<?php echo $qryout[0]; ?></div></td>
    <td scope="col"><div align="center">&nbsp;<?php echo $qryout[1]; ?></div></td>
   <td scope="col"><div align="center">&nbsp;<?php echo $qryout[7]; ?></div></td>
   <td scope="col"><div align="center">&nbsp;<?php echo $qryout[2]; ?> </div></td>
     <td scope="col"><div align="center">&nbsp;<?php echo $qryout[3]; ?></div></td> 
  <td scope="col"><div align="center">&nbsp;<?php echo $qryout[4]; ?> </div></td>
  <td>&nbsp;<?php echo $qryout[8]; ?></td>
   <?php
         ...
         <td>&nbsp;<?php echo $bk; ?></td>
         <td>&nbsp;<?php echo $qryout[9]; ?></td>
          <td scope="col"><div align="center">&nbsp;<?php echo $qryout[0]; ?></div></td>
         <td>&nbsp;<?php echo $qryout[1]; ?></td>
         <!--<td><?php //echo $row[17]; ?></td>-->
         <td>&nbsp;<?php echo $qryout[2]; ?></td>
         <td>&nbsp;<?php //echo $row[10]; ?></td>
         <td>&nbsp;<?php //echo $row[24]; ?></td>
         <td>&nbsp;<?php //echo $row[10]; ?></td>
         <td>&nbsp;<?php echo $qryout[3]; ?></td>
         <td>&nbsp;<?php echo $qryout[4]; ?></td>
         <td>&nbsp;<?php echo "India"; ?></td>
         <td>&nbsp;<?php echo $qryout[5]; ?></td>
         <td>&nbsp;<?php echo $qryout[8]; ?></td>
         <?php } ?>
         <td>&nbsp;<input type="text" size="7px" id="tkdt<?php echo $i; ?>" onclick="displayDatePicker('tkdt<?php echo $i; ?>')" value="<?php echo date('d/m/Y',strtotime($row[6])); ?>" onblur="calccc('<?php echo $i; ?>');" width="40px"> </td>
         <td>&nbsp;<input type="text" name="frmdt<?php echo $i; ?>" id="frmdt<?php echo $i; ?>" readonly value="<?php if(strtotime($start_date)<strtotime($row[6])){ echo date('d/m/Y',strtotime($row[6])); }else{ echo date('d/m/Y',strtotime($start_date)); } ?>"><?php if(strtotime($start_date)<strtotime($row[6])){ echo date('d/m/Y',strtotime($row[6])); }else{ echo date('d/m/Y',strtotime($start_date)); } ?></td>
         <td>&nbsp;<input type="text" name="endt<?php echo $i; ?>" readonly id="endt<?php echo $i; ?>" value="<?php echo date('d/m/Y',strtotime($end_date)); ?>"><?php if($row[5]!='0000-00-00'){ echo date('d/m/Y',strtotime($row[5]));}else{ echo date('d/m/Y',strtotime($end_date));  }  ?></td>
         <td>&nbsp;<input type="text" size="7px" id="hkdt<?php echo $i; ?>" onblur="calccc('<?php echo $i; ?>');" onclick="displayDatePicker('hkdt<?php echo $i; ?>')" value="<?php if($row[5]!='0000-00-00'){ echo date('d/m/Y',strtotime($row[5]));} ?>" width="40px"></td>
          <!--<td>&nbsp;<?php echo $row[7]; ?></td>
           <td>&nbsp;<?php echo $row[9]; ?></td>
            <td>&nbsp;<?php echo $row[8]; ?></td>-->
        <td><input type=text name="days<?php echo $i; ?>" readonly id=days<?php echo $i; ?> size="2px" value="<?php
	 echo $row[7];
	 /*if(strtotime($row[6],0)<strtotime($start_date,0)){
		   if($row[5]=='0000-00-00')
		//  echo $dd;
		  else
		  {
		 ...
		 // echo strtotime($row[29])." ".
		  //echo strtotime($row[5])-strtotime($start_date);
		  }
		  }
                                   else{    $fr = strtotime($row[6], 0);
                                            $t = strtotime($end_date, 0);
                                            $difference = $t - $fr;
                                            $dd1 = intval($difference/(3600*24)+1);
                                        //   echo $dd1;
      ...
      if($row[11]=='')
                              ...
                              // echo $sub;
		 if($row[10]=="A" || $row[10]=='a'){$ca++; $ra=$row[9];
		 $numaa=0;
                      $counta=0;
     ...
     $aamt[$key]=(($aamt[$key])+$row[8]);
       ...
       $aamt[$key]=(($aamt[$key])+$row[8]);
        ...
        $aamt[$key]=(($aamt[$key])+$row[8]);
                            ...
                            else if($row[10]=="B" || $row[10]=='b'){  $cb++; $rb=$row[9];
                            //B Category Subtype
                             $numbb=0;
                              
                               // echo " subcat= ".$row[11]."****<br>";
     ...
     $bamt[$key]=(($bamt[$key])+$row[8]);
       ...
       $bamt[$key]=(($bamt[$key])+$row[8]);
        ...
        $bamt[$key]=(($bamt[$key])+$row[8]);
                               ...
                               else if($row[10]=="C" || $row[10]=='c'){$cc++; $rc=$row[9];
                                  //C Category Subtype
                             $numcc=0;
   ...
   //echo $sub;
       $countna=0;
                                if(in_array($sub,$nacnt))
                                       {
                                 // echo "****".    $key=array_search($row[10], $nacnt);
     ...
     $naamt[$key]=(($naamt[$key])+$row[8]);
      ...
      $naamt[$key]=(($naamt[$key])+$row[8]);
       ...
       $naamt[$key]=(($naamt[$key])+$row[8]);
                          ...
                          ?>"></td>
         <td>&nbsp;<input type="text"  readonly size="5px" name=rt<?php echo $i; ?> id="rt<?php echo $i; ?>" value="<?php echo $row[9]; ?>"></td>
         <td>&nbsp;<input type="hidden"  readonly id=mt2<?php echo $i; ?> value="<?php echo $row[8]; ?>"><input type="text" size="5px" name=mt<?php echo $i; ?> id="mt<?php echo $i; ?>" value="<?php  echo $row[8];
         $tot=$tot+$row[8];
         if(strtotime($row[6],0)<strtotime($start_date,0)){
         if($row[5]=='0000-00-00')
         {
          $var= number_format(($dd/$nod)*$row[9], 2, '.', '');
         // $tot=$tot+(($dd/$nod)*$row[9]);
          if($row[10]=="A")$aa=$aa+$row[8];
                                       else if($row[10]=="B")$ab=$ab+$row[8];
                                       else if($row[10]=="C")$ac=$ac+$row[8];    
                                       else $an=$an+$row[8]; 
         }
	 else
	 {
          $var= number_format(($dd1/$nod)*$row[9], 2, '.', '');
        // $tot=$tot+(($dd1/$nod)*$row[9]);
          if($row[10]=="A")$aa=$aa+$row[8];
                                       else if($row[10]=="B")$ab=$ab+$row[8];
                                       else if($row[10]=="C")$ac=$ac+$row[8];    
                                       else $an=$an+$row[8]; 
		 ...
		 //$tot=$tot+(($var/$nod)*$row[9]);
                                     
                                       }
                                   else{
                                          number_format(($dd1/$nod)*$row[9], 2, '.', '');
										// $tot=$tot+(($dd1/$nod)*$row[9]);
                                       if($row[10]=="A")$aa=$aa+$row[8];
                                       else if($row[10]=="B")$ab=$ab+$row[8];
                                       else if($row[10]=="C")$ac=$ac+$row[8];
      ...
      <td><div id="rem<?php echo $i;  ?>"><a href="#" onclick="removeatm('<?php echo $row[0]; ?>','<?php echo $i;  ?>')">Remove</a><br><br>
      <a href="#" onclick="Editsite('<?php echo $row[0]; ?>','<?php echo $i;  ?>','<?php echo $_GET['id']; ?>')">Edit</a>
      </div></td>
