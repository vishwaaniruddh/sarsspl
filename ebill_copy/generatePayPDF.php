...
<?php	
        $total=0;
$qry1=mysqli_query($con,"select max(tid) from ebfundtransfers");        
$qrrow=mysqli_fetch_row($qry1);
	...
	for($x=0;$x<count($reqs);$x++){
		...
		if($accname[$x]!='-1'){
			...
			$table=mysqli_query($con,$sql); 
			$qry=mysqli_query($con,"select iFund_status from ebillfundrequests where req_no='$reqs[$x]'");
			$row=mysqli_fetch_array($qry);
			if($row[0]==7)
			{
				$sql="update ebillfundrequests set chqno='$chqno',iFund_status='8' where req_no='$reqs[$x]'";
				mysqli_query($con,"Update oldebreq set status='8' where reqid='$reqs[$x]'");
			}
			else
...
$qry1=mysqli_query($con,"select distinct(accid) from ebfundtransfers where tid='$tid' and accid<>90");
$ct=1;
...
$qry2=mysqli_query($con,"select sum(approvedamt) from ebillfundrequests where req_no in(select reqid from ebfundtransfers where tid='$tid' and accid='$qrrow[0]')");
...
<?php
 ...
 //for cheque DD
 $qry2=mysqli_query($con,"select accid,reqid,narration from ebfundtransfers where tid='$tid' and accid=90");
//$ct=1;
...
$qry3=mysqli_query($con,"select approvedamt from ebillfundrequests where req_no ='".$qrrow2[1]."'");
