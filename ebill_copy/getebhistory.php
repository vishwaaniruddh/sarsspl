...
//echo $qry;
?>
<a href="javascript:void(0);" onclick="newwin('ebsitehist.php?atmid=<?php echo $atm; ?>&custid=<?php echo $custid; ?>&trackid=<?php  echo  $trackid; ?>','display',900,700)">View Full history</a><br>
...
<tr>
<td><b>Request ID</b></td><td><?php   echo $row[5];  ?></td></tr>
...
<tr><td><b>Bill From</b></td><td><?php if($row[9]!='0000-00-00'){ echo date('d, M Y',strtotime($row[9])); } ?></td></tr>
...
<?php if(mysqli_num_rows($transfer)>0){ $transro=mysqli_fetch_row($transfer); ?><tr><td><b>Transferred Amount</b></td><td><?php echo $row[6]; ?></td></tr>

<tr><td><b>Transferred Date</b></td><td><?php echo date("d, M Y",strtotime($transro[3])); ?></td></tr>
<?php  } ?>
...
if(mysqli_num_rows($inv)==0 && mysqli_num_rows($canc)==0){ ?><tr><td colspan="2"><?php if($row[10]!='0' & $row[11]!='y'){ ?>
<a href="editbill.php?reqid=<?php echo $row[5]; ?>" target="_blank">Edit Details</a>
<?php }

 ?>
<input type="button" id="showrem<?php echo $row[5]; ?>" onclick="showrem('<?php echo $row[5]; ?>')" value="Cancel"></td></tr>

<tr id="up1<?php echo $row[5]; ?>" style="display:none;"><td><b>Remark</b></td><td><textarea name="rem<?php echo $row[5]; ?>" id="rem<?php echo $row[5]; ?>"></textarea></td></tr>
<tr id="up2<?php echo $row[5]; ?>" style="display:none;"><td colspan="2"><input type="button" name="cancinv<?php echo $row[5]; ?>" id="cancinv<?php echo $row[5]; ?>" onclick="cancinv('<?php echo $row[5]; ?>')" value="Cancel from Invoicing"></td></tr><?php 
 }
