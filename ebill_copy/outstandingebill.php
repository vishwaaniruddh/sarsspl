...
echo $_POST['todt']; } ?>" >&nbsp;
 			...
 			if(isset($_POST['sub']))
				...
				// echo  "select sum(amt),custid from siteinvoice where  billdate between STR_TO_DATE('".$frm."','%d/%m/%Y') and STR_TO_DATE('".$to."','%d/%m/%Y') group by custid ";
				  $query=mysqli_query($con,"select sum(amount),customer_name,sum(servchrg) from send_bill where date between STR_TO_DATE('".$frm."','%d/%m/%Y') and STR_TO_DATE('".$to."','%d/%m/%Y') group by customer_name");
				 }
				 else
				 {
				 $query=mysqli_query($con, "select sum(amount),customer_name,sum(servchrg) from send_bill where customer_name='".$client."' and date between STR_TO_DATE('".$frm."','%d/%m/%Y') and STR_TO_DATE('".$to."','%d/%m/%Y') ");
				 }
		...
		//echo "select * from siteinvoice where custid='".$client."' and billdate between STR_TO_DATE('".$frm."','%d/%m/%Y') and STR_TO_DATE('".$to."','%d/%m/%Y') ";
					...
					{
					//echo "Select contact_first FROM contacts short_name='".$row[1]."' and type='c'";
