...
$qry = mysqli_query($con, "select * from Rnm_quotation_details_tis where id='" . $detailid . "' and qid='" . $qid . "'");
...
if ($num > 0) {
	...
	$insqry = mysqli_query($con, "Insert into quotation_edit_history_tis(`qid`, `quotationdetails_id`, `particular`, `description`, `quantity`, `rate`, `total`,`Quot_amt`, `reqby`, `entrydate`, `status`,`remark`, `filename`,tcode,uom)values('" . $qid . "','" . $detailid . "','" . $row[2] . "','" . $row[3] . "','" . $row[4] . "','" . $row[5] . "','" . $row[6] . "','" . $tqtamt . "','" . $srno[0] . "','" . $dt . "','d','" . $rem . "','" . $image_name . "','" . $row[7] . "','" . $row[8] . "')");
	if (!$insqry) {
	...
	//echo "DELETE FROM Rnm_quotation_details where id='".$detailid."' and qid='".$qid."'";
	$upqry = mysqli_query($con, "DELETE FROM Rnm_quotation_details_tis where id='" . $detailid . "' and qid='" . $qid . "'");
	if (!$upqry) {
	...
	$gtot2 = mysqli_query($con, "Select sum(Total) from Rnm_quotation_details_tis where qid='" . $qid . "'");
	$rowtot = mysqli_fetch_array($gtot2);

	//echo "Update rnm_invoice_details SET `ApprovalAmount`='".$rowtot[0]."' where  qid='".$qid."'";
	$upqry = mysqli_query($con, "Update rnm_invoice_details_tis SET `ApprovalAmount`='" . $rowtot[0] . "' where  qid='" . $qid . "'");
