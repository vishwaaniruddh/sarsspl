...
//echo "select sum(approvedamt),chqno,count(chqno) from ebillfundrequests where chqno!='' and chqno!='0' and reqstatus=8 group by chqno order by req_no ASC";
				$sql=mysqli_query($con,"select sum(approvedamt),chqno,count(chqno) from ebillfundrequests where chqno!='' and chqno!='0' and reqstatus=8 group by chqno order by req_no ASC");	
				while($row=mysqli_fetch_array($sql))
				{
				$srno=$srno+1;
				$req=mysqli_query($con,"select distinct(pdate) from ebfundtransfers where chqno='".$row[1]."'");
...
<td><?php echo $row[2]; ?></td><td><?php echo $reqro[0]; ?></td>
				...
				<td><?php if(mysqli_num_rows($req)>0){ ?><a href="viewtransdet.php?id=<?php echo $row[1]; ?>" target="_blank"><input type='button' name="view" id="view" value="View Details" /></a><?php } ?>
			...
			<?php } ?>		
				...
				<?php if($_SESSION['designation']=='6' && mysqli_num_rows($req)>0){ ?>	<td><a href="viewebtrans.php?transid=<?php echo $row[1]; ?>" target="_blank"><input type='button' name="view" id="view" value="View Bank Statement" /></td><?php }else{ echo "<td>&nbsp;</td>";} ?>
					</tr>
				<?php
				}
...
<td><?php echo $cntro[0]; ?></td>
					...
					<td><a href="viewtransdet.php?id=<?php echo $tidarr[$i]; ?>" target="_blank"><input type='button' name="view" id="view" value="View Details" /></a>
			<?php if($_SESSION['designation']=='2'){ ?>
			...
			<?php } ?>		
				...
				<?php if($_SESSION['designation']=='6'){ ?>	<td><a href="viewebtrans.php?transid=<?php echo $tidarr[$i]; ?>" target="_blank"><input type='button' name="view" id="view" value="View Bank Statement" /></td><?php } ?>
					</tr>
					<?php
					}}
					?>-->	
