...
//echo $str;
		...
		while($app=mysqli_fetch_row($tablex))
		{
			...
			$sql="Select * from ebillfundrequests where req_no='".$app[1]."'";
		        ...
		        $reqid[]=$row[0];
		        //echo "select paid_amount from ebpayment where Bill_No='".$app[1]."'";
...
//echo "SELECT sum(pamount) FROM `transfer` where  to_accid='".$aid."' and status=1 and pdate>=STR_TO_DATE('".$_REQUEST['frmdt']."','%d/%m/%Y') AND  pdate<=STR_TO_DATE('".$_POST['todt']." ','%d/%m/%Y')";
...
$rev_qry=mysqli_query($con,"SELECT sum(pamount) AS TotalItemsOrdered FROM onaccount_reversal where accid='".$opening_fund['aid']."' and status=8 and pdate>=STR_TO_DATE('".$_POST['frmdt']."','%d/%m/%Y') AND  pdate<=STR_TO_DATE('".$_POST['todt']." ','%d/%m/%Y')");
...
<td><?php echo $total+$total2+$total3+$total4-$paidamt6; ?></td>
