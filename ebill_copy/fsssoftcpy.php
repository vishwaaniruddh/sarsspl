...
$contents.="S.No. \t Address (*) \t ATM ID(*) \t Site Code(*) \t Payment Mode \t Vendor Code/RR Code \t Consumer No.(*) \t Bill date(*) \t Due Date(*) \t Actual Date of Payment[(*)Except Bill Desk] \t Billing Period(From)[(*)Except Bill Desk] \t Billing Period(To)[(*)Except Bill Desk] \t Opening Units[(*)Except Bill Desk] \t Closing Units[(*)Except Bill Desk] \t Units Consumed[(*)Except Bill Desk] \t Electricity charges(*) \t Extra Charges \t Reconnection Charges \t Disconnection Charges \t Security Deposit \t After Due Date Charges \t Penalty Charges(*) \t Total Electricity Payment (*) \t Service Charge(*) \t Total Payable(*) \t Remarks \t  EB Invoice Number(*) \t Supervisor" ;
while($ro=mysqli_fetch_row($qry))
{
$srno=0;
$inv=mysqli_query($con,"select * from send_bill_detail where send_id='".$ro[0]."' and status='0'");
while($invr=mysqli_fetch_array($inv))
{
$ebpay=mysqli_query($con,"select Paid_Date from ebpayment where Bill_No='".$invr[20]."'");
$ebpayr=mysqli_fetch_row($ebpay);
$site=mysqli_query($con,"select bank,site_type,atm_id1,atmsite_address,site_id,projectid from ".$ro[1]."_sites where trackerid='".$invr[2]."'");
$sitero=mysqli_fetch_row($site);
$ebill=mysqli_query($con,"select Consumer_No from ".$ro[1]."_ebill where atmtrackid='".$invr[2]."'");
...
$ebfund=mysqli_query($con,"select bill_date,due_date,opening_reading,closing_reading,supervisor from ebillfundrequests where req_no='".$invr['reqid']."'");
$ebfr=mysqli_fetch_row($ebfund);
$sum_row=$invr[12]+$invr['extrachrg']+$invr['recon_chrg']+$invr['discon_chrg']+$invr['sd']+$invr['after_duedt_chrg'];
//$tot=$tot+$sum_row+$invr[15];
$srno=++$srno;
$contents.="\n".$srno."\t";
...
//$contents.=$invr[10]."\t";
$contents.=$invr[12]."\t";
$contents.=$invr['extrachrg']."\t";
$contents.=$invr['recon_chrg']."\t";
$contents.=$invr['discon_chrg']."\t";
$contents.=$invr['sd']."\t";
$contents.=$invr['after_duedt_chrg']."\t";
$contents.="0\t";
//$contents.="0\t";
//$contents.="0\t";
//$contents.=$sum_row."\t";
