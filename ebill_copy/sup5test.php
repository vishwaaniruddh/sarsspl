...
$accsop=mysqli_query($con,"select * from fundaccounts where aid='".$_POST['sup']."'");
$rxop=mysqli_fetch_row($accsop);

?>
<tr>
<td colspan="2">
Opening Balance 2017-18
</td>
<td>
...
<?php 
echo "select * from newtransferto_supv where old_supvid='".$_POST['sup']."' or new_supvid='".$_POST['sup']."' order by id desc limit 0,1";
$newdeb=0;
$newcred=0;
$debcrqry=mysqli_query($con,"select * from newtransferto_supv where old_supvid='".$_POST['sup']."' or new_supvid='".$_POST['sup']."' order by id desc limit 0,1");
while($dbcrqrws=mysqli_fetch_array($debcrqry))
{
 
 
 
 $gtds=mysqli_query($con,"select amount from ebillfundrequests where req_no='".$dbcrqrws["req_no"]."' ");
 $gtdsrws=mysqli_fetch_array($gtds);

if($dbcrqrws['old_supvid']==$_POST['sup'])
{
    $newdeb=$gtdsrws[0];
...
<td >okkk</td>
<td ></td>
<td width="200"  align="center"><?php if($dbcrqrws["old_supvid"]==$_POST['sup']){ echo $gtdsrws[0];}else{ echo 0;} ?></td>
<td width="75"  align="center"><?php if($dbcrqrws["new_supvid"]==$_POST['sup']){ echo $gtdsrws[0];}else{ echo 0;} ?></td>
...
<tr><td colspan=2 align='right' >Total D=A+B+C+O+T+R</td><td align='CENTER' ><?php
 ...
 echo $total+$total2+$total3+$total4+$total5+$total6+$cr+$newcred-$db-$newdeb;
...
$to=$total+$total2+$total3+$total4+$total5+$total6+$cr+$newcred-$db-$newdeb;
