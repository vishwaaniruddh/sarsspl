...
if($res4)
{
//echo "insert into send_bill_detail(send_id,atm_id,electric_board,location,consumer_no,bill_date,due_date,units_consumed,usdate,uedate,month,paid_amount,paid_date,reqid) values('$send_id','$row1[14]','$row[2]','$location','$row[1]','$row1[2]','$row1[9]','$row1[3]','$row1[26]','$row1[7]','$month','$row3[1]','$row3[2]','".$bills[$i]."')";				 
...
if($cid!='AGS01')
{
  if(($cid!='EUR08' && $cid!='EPS' && $cid!='Tata05') && ($_POST['proj']=='MOF' || $_POST['proj']=='Mphasis')){
...
$to=$to+($rs[2]+$_POST['addsv']);
 echo number_format(($rs[2]+$_POST['addsv']),2); ?></td>
</tr>
<?php
} ?>
<tr align="right"><td colspan="4">Gross Amount</td>
<td valign="top"><?php echo number_format($to,2); ?></td></tr>
