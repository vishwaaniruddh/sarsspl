...
mysqli_
		...
		foreachmysqli_->errors as $key=>$values)
...
mysqli_
		if(empty($this->cust))
mysqli_
		$this->emysqli_]="Please select Your customer";
...
mysqli_
		...
		$this->errors[]="Please Select Atm ID for this PO";
...
$qry2ro=mysqli_fetch_row($qry2);
	...
	$sql = mysqli_query($con,"INSERT INTO `satyavan_accounts`.`alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `state`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`, `close_date`, `standby`, `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`subject`,`custdoctno`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->state."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$this->alertdt."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', 'service', '', '', '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$qry2ro[0]."_".date("Ymd").$num3."','".$this->sub."','".$this->doc."')");
	...
	//$qry4=mysqli_query($con,"Update alert set createdby='".$this->crby."_".date("Ymd").$num3."' where alert_id='".$row."'");
	...
	for($i=0;$i<count($this->assid);$i++)

		{
			...
			$qry2=mysqli_query($con,"select * from assets_specification where ass_spc_id='".$iniro[1]."'");
