...
<?php
	...
	$qry1="select * from quotation1_tis where 1 ";
// echo $_POST['sup'];

if($_POST['custr']!="")
{
$qry1.=" and cust='".$_POST['custr']."'";
}

 if($_POST['atm']!="")
	{
	$qry1.=" and atm='".$_POST['atm']."'";
	...
	//$qry1.=" and pdate>='".$strt."' and pdate<='". $endt."'";
          ...
          $qry1.=" and id in (Select qid from quotation1ftransfers_tis where status!='0' and date(pdate)>='".$strt."' and date(pdate)<='". $endt."')";
	
	}
 else
{
$qry1.=" and id in (Select qid from quotation1ftransfers_tis where status!='0' )";
}
...
<input type="button" id="exp" onclick="tableToExcel('custtable', 'Table Export Example')" value="Exp"/>	
...
while($row=mysqli_fetch_array($qrys))
{
	...
	<tr>
	    ...
	    <?php //echo "select cust_name from  $row[2]_sites where cust_id='".$row[2]."' ";
...
<?php
 ...
 echo  round($tamt[0]); $totamt=$totamt+round($tamt[0]);
	...
	?>
	 ...
	 <td  align="center" width="150">
	       <input type="button" name="vdet" id="vdet<?php echo $srno?>" value="View" onclick="vdtefunc('<?php echo $row[0];?>','<?php echo $qname[0];?>');">
	    </td> 
...
<?php $trandate=mysqli_query($con,"select * from  quotation1ftransfers_tis where qid='".$row[0]."'");
$tdarow=mysqli_fetch_array($trandate); ?>

<?php

//echo $row[0];
 if($tdarow[3]!='0000-00-00') { echo date('d-m-Y',strtotime($tdarow[3])); } ?>
...
<?php
 $amtqry1=mysqli_query($con,"select ticket_no,job,po from quotation_approve_details_tis where qid='".$row[0]."'");
 //echo "select ticket_no,job,po from quotation_approve_details where qid='".$row[0]."'";
