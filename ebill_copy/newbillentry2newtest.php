...
<body onload="showatm()">
	 ...
	 <?php if(isset($_SESSION['flag'])){  unset($_SESSION['success']);
	 ?>
...
<form id="form1" name="form1" method="post" action="processbillentrytest.php" onsubmit="return validate(this)" autocomplete="OFF" enctype="multipart/form-data">
    <input type="hidden" name="tidd" id="tidd" readonly="readonly" value="<?php echo $_GET["tid"];?>"/>
...
$qrchknew=mysqli_query($con,"Select * from online_transaction_new where id='".$reqidnew."'");
//echo "Select * from online_transaction_new where id='".$reqidnew."'";
$Num_Rows = mysqli_num_rows ($qrchknew);


if($Num_Rows>0)
{
    
    $getfrnew=mysqli_fetch_array($qrchknew);
  //  echo $getfrnew["Customer"];
?>
    ...
    $datepd = str_replace('-', '/', $getfrnew["PAID_DATE"]);
$dttpd= date('d/m/Y', strtotime($datepd));
...
<script>    
    ...
    {*/
    //alert('<?php echo $getfrnew["Customer"];?>');    
    document.getElementById("cust").value='<?php echo $getfrnew["Customer"];?>';
    document.getElementById("atmid").value='<?php echo $getfrnew["ATMID"];?>';
    document.getElementById("trsid").value='<?php echo $getfrnew["TRANS_ID"];?>';
    document.getElementById("bank").value='<?php echo $getfrnew["BANK"];?>';
    document.getElementById("bamt").value='<?php echo $getfrnew["BILL_AMOUNT"];?>';
document.getElementById("sv").value='<?php echo $getfrnew["SUPERVISOR"];?>';


document.getElementById("paiddt").value='<?php echo $dttpd;?>';

    getdetails('<?php echo $getfrnew["ATMID"];?>','atm_id1');
