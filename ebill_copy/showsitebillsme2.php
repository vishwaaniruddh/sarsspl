...
$custname = $_POST['cid'];
 $bkme = $_POST['formCountries'];
   ...
   $bknew2=array();
   $bnk='-1';
   for($i=0;$i<count($bkme);$i++){
   if(isset($bkme[$i]))
   $bknew2[]=$bkme[$i];
   }
    $bknew2=implode(",",$bknew2);
    $bnk=$bknew2;
    $bk=str_replace(",","','",$bknew2);
if($bk!='-1')
$bk="'".$bk."'";
...
<div id="annexure">
<!--<p align="center" >Customer ID &nbsp;<?php echo $cid; ?> <br> Bank &nbsp; <?php if($bk==-1) echo "All" ; else echo $bknew; ?>
  <br> Bill From <?php echo $from; ?> To <?php echo $to; ?></p>-->
  <p align="center" height="8px"><?php if($bk==-1){ echo "All"; } else{ echo $bknew; } ?><?php echo " ".ucfirst($service) ; ?> billing data for the month of <?php echo date("M y",strtotime($start_date)); ?></center>
...
$qrstr=" and housekeeping='Y'";
	echo "".$cid."-".$bk;
	        $i = 1;
	       if($bk==-1 && $cty==-1) {
	      	$sql = "SELECT * FROM ".$custname."_sites where active='Y' and $service='Y'".$qrstr."";
		...
		$sql = "SELECT * FROM ".$custname."_sites where active='Y'and city='$cty' and $service='Y' ".$qrstr."";
...
$sql.=" and bank in ($bk)";
		...
		if($service=='caretaker')
		{
		$sql.=" and takeover_date<='".$end_date."' order by takeover_date ASC";
		}
		else
		$sql.=" order by ".$service."_tkdt ASC";
...
$ssubcat='';
		...
		{
		$start_date=$date_parts1[2]."-".$date_parts1[1]."-".$date_parts1[0];
$frdt='';
$todt='';
$rtchng=mysqli_query($con,"select * from ratechngsite where trackerid='".$row[54]."' and custid='".$custname."' and service='".$service."' and stdt between '".$start_date."' and '".$end_date."'");
$rtcro=mysqli_fetch_row($rtchng);
 ...
 ?></td>
  ...
  <td><?php echo $row[27]; ?></td>
   <?php
         ...
         <td><?php echo $row[27]; ?></td>
         <?php } ?>
         <td align='left'><?php   if($row[$hkdtsr]!='0000-00-00'){ echo date('d/m/Y',strtotime($row[$hkdtsr])); }else{ echo "-";}  ?></td>
         <td><?php if(mysqli_num_rows($rtchng)==0){  if($row[$hkdtsr]>$start_date){ echo date('d/m/Y',strtotime($row[$hkdtsr]));}else{ echo date('d/m/Y',strtotime($start_date));}}else{ $tttm=strtotime(date("Y-m-d", strtotime($rtcro[1])) . " +1 day"); echo  date('d/m/Y',$tttm); } ?></td>
         <td><?php if(mysqli_num_rows($rtchng)==0){ if($row[$hkhosr]!='0000-00-00' && $row[$hkhosr]<$end_date){ echo date('d/m/Y',strtotime($row[$hkhosr]));}else{ echo date('d/m/Y',strtotime($end_date));  } }else{ echo date('d/m/Y',strtotime($end_date)); }  ?></td>
         <td><?php if($row[$hkhosr]!='0000-00-00' && $row[$hkhosr]<$end_date){ echo date('d/m/Y',strtotime($row[$hkhosr]));} ?></td>
        <td><?php
//echo $ndays."<br>";
if(mysqli_num_rows($rtchng)>0){
$start_date=date('Y-m-d',$tttm) ;
}
//echo $start_date;
...
if(mysqli_num_rows($rtchng)>0)
{
 echo $var= number_format(round(($dd1/$nod)*$row[$sr]), 2, '.', '');
          ...
          if($row[27]=="A")$aa=$aa+round(($dd1/$nod)*$row[$sr]);
                                       ...
                                       else if($row[27]=="C")$ac=round($ac+($dd1/$nod)*$row[$sr]);    
...
//echo "hi";
      ...
      <!-- <?php if($cid=='Diebold'){ ?>  <td>&nbsp;</td><?php } ?>-->
		...
		$start_date=$date_parts1[2]."-".$date_parts1[1]."-".$date_parts1[0];
		//echo "select * from ratechngsite where  custid='".$custname."' and service='".$service."' and stdt between '".$start_date."' and '".$end_date."'";
		$nxtst=mysqli_query($con,"select * from ratechngsite where  custid='".$custname."' and service='".$service."' and stdt between '".$start_date."' and '".$end_date."'");
while($nxtstr=mysqli_fetch_array($nxtst))
{

		$brkcnt=$brkcnt+1;
		$sttt="select * from ".$nxtstr[4]."_sites where trackerid='".$nxtstr[3]."'";
		if($bk!='-1')
$sttt.=" and bank in ($bk)";
		if($_POST['zone']!='')
		{
		$sttt.=" and zone='".$_POST['zone']."'";
		}
		if($_POST['project']!='' && $_POST['project']!='-1')
			$sttt.=" and projectid='".$_POST['project']."'";
			if($cid=='Tata05')
			{
			if($_POST['pono']!='' && $_POST['pono']!='-1')
			$sttt.=" and atm_id1 in (select atm_id1 from tatapo where po='".$_POST['pono']."')";
			}
		$site=mysqli_query($con,$sttt);
		if(mysqli_num_rows($site)>0){
		//echo $sttt;

$sitero=mysqli_fetch_row($site);
$atm=$atm.",".$sitero[53];
$tkdt=$tkdt.",".$sitero[$hkdtsr];
$hddt=$hddt.",".$sitero[$hkhosr];
...
//echo $row[54];
 ?></td>
         <td><?php echo $sitero[52]; ?></td>
         <?php if($cid=='Prizm07'){ ?><td scope="col"><div align="center"><?php echo $sitero[0]; ?></div></td>
  <td scope="col"><div align="center"><?php echo $sitero[10]; ?> </div></td>
  <td scope="col"><div align="center"><?php echo $sitero[23]; ?></div></td> 
  <td scope="col"><div align="center"><?php echo $sitero[13]; ?> </div></td>
  <td scope="col"><div align="center"><?php echo $sitero[12]; ?> </div></td>
  <td scope="col"><div align="center"><?php echo $sitero[16]; ?></div></td>
  <td scope="col"><div align="center"><?php echo $sitero[15]; ?></div></td>
  <td scope="col"><div align="center"><?php echo $sitero[17]; ?></div></td>
  <td scope="col"><div align="center"><?php echo $sitero[24]; ?></div></td>
  <td><?php echo $sitero[27]; ?></td>
  
   <?php } 
    elseif($cid=='FIS03')
   {
   ?>
   <td scope="col"><div align="left"><?php echo $sitero[10]; ?></div></td>
   <td scope="col"><div align="left"><?php echo $sitero[12]; ?> </div></td>
   <td scope="col"><div align="left"><?php echo $sitero[16]; ?></div></td>
    <td scope="col"><div align="left"><?php echo $sitero[15]; ?></div></td>
    <td scope="col"><div align="left"><?php echo $sitero[17]; ?></div></td>
   <td scope="col"><div align="left"><?php echo $sitero[24]; ?></div></td>
   <td scope="col"><div align="left"><?php echo $sitero[25]; ?> </div></td>
     <td scope="col"><div align="left"><?php echo $sitero[23]; ?></div></td> 
  <td scope="col"><div align="left"><?php echo $sitero[13]; ?> </div></td>
  <td><?php echo $sitero[27]; ?></td>
   <?php
   }
  else{ ?>
         
         <td><?php echo $sitero[10]; ?></td>
         <td><?php echo $sitero[16]; ?></td>
          <td scope="col"><div align="left"><?php echo $sitero[15]; ?></div></td>
         <td><?php echo $sitero[17]; ?></td>
         <!--<td><?php //echo $sitero[17]; ?></td>-->
         <td><?php echo $sitero[25]; ?></td>
         <td><?php //echo $sitero[10]; ?></td>
         <td><?php echo $sitero[24]; ?></td>
         <td><?php //echo $sitero[10]; ?></td>
         <td><?php echo $sitero[23]; ?></td>
         <td><?php echo $sitero[13]; ?></td>
         <td><?php echo "India"; ?></td>
         <td><?php echo $sitero[12]; ?></td>
         <td><?php echo $sitero[27]; ?></td>
         <?php } ?>
         <td align='left'><?php   if($sitero[$hkdtsr]!='0000-00-00'){ echo date('d/m/Y',strtotime($sitero[$hkdtsr])); }else{ echo "-";}  ?></td>
         <td><?php   if($sitero[$hkdtsr]>$start_date){ echo date('d/m/Y',strtotime($sitero[$hkdtsr]));}else{ echo date('d/m/Y',strtotime($start_date));} ?></td>
         <td><?php echo date('d/m/Y',strtotime($nxtstr[1]));   ?></td>
         <td><?php  if($sitero[$hkhosr]!='0000-00-00'){ echo date('d/m/Y',strtotime($sitero[$hkhosr])); } ?></td>
        <td>
        <?php
           if(strtotime($sitero[$hkdtsr],0)<strtotime($start_date,0)){
           //echo "hi";
		  
		  $fr = strtotime($start_date, 0);
		 $t = strtotime($nxtstr[1], 0);
                                            $difference = $t - $fr;
                                            $dd1 = intval($difference/(3600*24)+1);
                                 echo $dd1; 
$ndays=$ndays.",".$dd1;        
...
$ssubcat=$ssubcat.",".$sub;
      //echo "<br>".$nod;
		 if($row[27]=="A" || $row[27]=='a'){$ca++; $ra=$nxtstr[7];
		 $numaa=0;
                      $counta=0;
                                if(in_array($sub,$acnt))
                                       {
     ...
     $aamt[$key]=(($aamt[$key])+round(($dd1/$nod)*$nxtstr[7]));
        ...
        $aamt[$key]=(($aamt[$key])+round(($dd1/$nod)*$row[$sr]));
                            ...
                            else if($row[27]=="B" || $row[27]=='b'){  $cb++; $rb=$nxtstr[7];
                            //B Category Subtype
                             $numbb=0;
  ...
  // echo $sub;
     ...
     $bamt[$key]=(($bamt[$key])+round(($dd1/$nod)*$nxtstr[7]));
        ...
        $bamt[$key]=(($bamt[$key])+round(($dd1/$nod)*$nxtstr[7]));
                               ...
                               else if($row[27]=="C" || $row[27]=='c'){$cc++; $rc=$nxtstr[7];
                                  //C Category Subtype
                             $numcc=0;
   ...
   //echo $sub;
     ...
     $camt[$key]=(($camt[$key])+round(($dd1/$nod)*$row[$sr]));
        ...
        $camt[$key]=(($camt[$key])+round(($dd1/$nod)*$nxtstr[7]));
       ...
       $naamt[$key]=(($naamt[$key])+round(($dd1/$nod)*$nxtstr[7]));
        ...
        <td><?php echo $nxtstr[7]; ?></td>
        <td><?php
         echo $var= number_format(round(($dd1/$nod)*$nxtstr[7]), 2, '.', '');
					 $tot=$tot+round(($dd1/$nod)*$nxtstr[7]);
                                       if($row[27]=="A")$aa=$aa+round(($dd1/$nod)*$nxtstr[7]);
                                       else if($row[27]=="B")$ab=$ab+round(($dd1/$nod)*$nxtstr[7]);
                                       else if($row[27]=="C")$ac=$ac+round(($dd1/$nod)*$nxtstr[7]);
                                       else $an=$an+round(($dd1/$nod)*$nxtstr[7]); 
                                       ?>
        </td>
        </tr>
<?php
