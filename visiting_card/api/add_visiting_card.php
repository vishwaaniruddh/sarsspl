...
if ($id) {
    ...
    if (mysqli_num_rows($visitsql) > 0) {
        ...
        // Handling Visiting Card Image Update
        // if (isset($_FILES['visiting_card_img']['name']) && !empty($_FILES['visiting_card_img']['name'])) {
        //     // Move uploaded background image to the correct directory
        //     $visiting_card_img_folder = $base_folder . "visiting_card_img/";

        //     if (!is_dir($visiting_card_img)) {
        //         mkdir($visiting_card_img_folder, 0777, true); 
        //     }

        //     $visiting_card_img = basename($_FILES['visiting_card_img']['name']);
        //     $visiting_card_img_tmp = $_FILES['visiting_card_img']['tmp_name'];
        //     $visiting_card_img_target = $visiting_card_img_folder . $visiting_card_img;

        //     if (move_uploaded_file($visiting_card_img_tmp, $visiting_card_img_target)) {
        //         // Remove old background image if exists
        //         if ($existing_data['visiting_card_img'] && file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . $existing_data['visiting_card_img'])) {
        //             unlink($_SERVER['DOCUMENT_ROOT'] . '/' . $existing_data['visiting_card_img']);
        //         }
        //         // Update the database field with the new image path
        //         $visiting_card_img_path = "visiting_card/assets/$year/$month/$day/$id/visiting_card_img/" . $visiting_card_img;
        //         $complete_visiting_card_img_path = $baseurl.$visiting_card_img_path;
        //         $update_fields[] = "visiting_card_img = '$complete_visiting_card_img_path'";
        //     } else {
        //         $response['Code'] = 201;
        ...
        $sql = "INSERT INTO visiting_card_details (user_id, first_name, last_name, company_name, business_type, residence_address, office_address, contact_no1, contact_no2, email_id, website, profile_image, background_img, font_color, font_size, keywords, status, created_at, visiting_card_img) 
                VALUES ('$user_id', '$first_name', '$last_name', '$company_name', '$business_type', '$residence_address', '$office_address', '$contact_no1', '$contact_no2', '$email_id', '$website', '$complete_profile_image_path', '$complete_background_image_path', '$font_color', '$font_size', '$keywords', '$status', '$datetime','$complete_visiting_card_img_path')";
