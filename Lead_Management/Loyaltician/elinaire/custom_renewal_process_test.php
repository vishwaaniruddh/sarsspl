...
$lastreceipt = mysqli_query($conn,"SELECT max(receipt_no) as receiptNo FROM `Members` ");
$lastreceiptNo = mysqli_fetch_assoc($lastreceipt);

$lastreceiptNo = $lastreceiptNo['receiptNo'];

$receiptsql = mysqli_query($conn,"select CountRecipt from PaymentReceipt where Program_ID = '1' ");
$receiptsql_result = mysqli_fetch_assoc($receiptsql);

$receiptNo = $receiptsql_result['CountRecipt'];

$receiptNo = (int)$receiptNo + 1;

var_dump($lastreceiptNo);
echo '<br>';
...
if ($CertificatesCheck1 == 'on') {
    // Renewal
    echo 'renewal';
    $renewalsql = "SELECT fromSerialNo,toSerialNo,issued_voucher_code,voucher_code FROM `voucher_issued_renewal` where 
        Program_ID='1' and voucher_code='R'";
    $renewalsql = mysqli_query($conn, $renewalsql);
    $renewalsql_result = mysqli_fetch_assoc($renewalsql);
    // $voucher_code = $renewal_result['voucher_code'];

    $issued_voucher_code = $renewalsql_result['issued_voucher_code'];
    $newissued_voucher_code = $issued_voucher_code + 1;
    
    $_newissued_voucher_code = 'R'.$newissued_voucher_code;
    // mysqli_query($conn, "update voucher_issued_renewal  set issued_voucher_code='" . $newissued_voucher_code . "' where Program_ID='1' and voucher_code='R'");
    // mysqli_query($conn, "update Members set renewal_voucher_code='" . $_newissued_voucher_code . "',RenewalCheck1=1 where Static_LeadID='" . $id . "'");
    // mysqli_query($conn, "insert into BarcodeScan(Voucher_id) values('" . $_newissued_voucher_code . "')");

    var_dump($_newissued_voucher_code);
}
 die;
// return ; 
...
//     // Promotional
//     echo 'promotional';
//     $promotionalsql = "SELECT fromSerialNo,toSerialNo,issued_voucher_code,voucher_code FROM `voucher_issued_renewal  ` where 
...
//     $newissued_voucher_code = $issued_voucher_code + 1;
//     mysqli_query($conn, "update voucher_issued_renewal   set issued_voucher_code='" . $newissued_voucher_code . "' where Program_ID='1' and voucher_code='52'");
...
";
    ...
    if (mysqli_query($conn, $update_sql)) {
        ...
        // update receipt no
        mysqli_query($conn,"update PaymentReceipt set CountRecipt = '".$receiptNo."' where Program_ID = '1' ");
