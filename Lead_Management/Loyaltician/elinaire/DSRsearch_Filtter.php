...
while($_row=mysqli_fetch_array($QuryGetLead)){
   ...
   if(is_null($_row['promotional_voucher_code'])){  $pvc= "";}else{$pvc=$_row['promotional_voucher_code'];}

   if(is_null($_row['renewal_voucher_code'])){  $rvc= "";}else{$rvc=$_row['renewal_voucher_code'];}
 ...
 $array[]= ['Primary_nameOnTheCard'=>$_row['Primary_nameOnTheCard'],'Type'=>'Primary','level_name'=>$sql3fetch['level_name'],'GenerateMember_Id'=>$_row['GenerateMember_Id'],'R'=>$R,'booklet_Series'=>$_row['booklet_Series'] ,'promotional_voucher_code'=>$pvc ,'renewal_voucher_code'=>$rvc,'TypeNR'=>$type,'MembershipDts_PaymentMode'=>$_row['MembershipDts_PaymentMode'],'MembershipDts_InstrumentNumber'=>$_row['MembershipDts_InstrumentNumber'],'Member_bankName'=>$_row['Member_bankName'],'Recipt'=>$_row['receipt_no'],'MembershipDts_NetPayment'=>$_row['MembershipDts_NetPayment'],'MembershipDts_GST'=>$_row['MembershipDts_GST'],'MembershipDts_GrossTotal'=>$_row['MembershipDts_GrossTotal'],'MemshipDts_Remarks'=>$_row['MemshipDts_Remarks'],'entryDate'=>$ddd,'Primary_Anniversary'=>$Primary_Anniversary,'Primary_DateOfBirth'=>$Primary_DateOfBirth,'Qry'=>$q,'FromDat'=>$FromDt1,'Todt'=>$Todt1,'cancel_Filtter'=>$cancel_Filtter,'canceledMember'=>$cnel];
}
