...
// echo "select * from BarcodeScan where Voucher_id like '" . $Voucher . "%'"; die;

// $barcode_sql = mysqli_query($conn, "select * from BarcodeScan where Voucher_id like '" . $Voucher . "%'");
//     // echo mysqli_num_rows($barcode_sql); die;
...
$count = 0;
    ...
    while ($barcode_sql_result = mysqli_fetch_assoc($barcode_sql)) {
        ...
        $tdStyle = '';
        if ($srno % 5 == 4) {
            $tdStyle = 'style="padding-left: 15px;"'; // Adjust the padding value as needed
        }
