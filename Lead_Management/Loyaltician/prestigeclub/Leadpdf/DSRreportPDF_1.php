...
$this->SetTextColor(255,255,255);
 $this->Rect(40,43,217,7,'F','',$fill_color = array(0, 0, 0));
...
<td>Promotional Voucher</td>
<td>Renewal Voucher</td>
<td>Type(N/R)</td>
...
$LessCancelationAmt+=$CancelFetch[1];
    ...
    // Calculate total amount and total VAT & gross total
    $totalAmount += $_row['MembershipDts_NetPayment'] + $_row['Spouse_Fee'];
    $totalVAT += $_row['MembershipDts_GST']+ $_row['Spouse_VAT'];
    $grossTotal += $_row['MembershipDts_GrossTotal'] + $_row['Spouse_Fee'] + $_row['Spouse_VAT'];
...
<td>'. $_row['renewal_voucher_code'].'</td>
...
<td>'. $_row['MembershipDts_Discount_Remark'].'</td>
...
$srno++;
$useBelowSpouse++;
$Spouse_Total = $_row['Spouse_Fee'] + $_row['Spouse_VAT']; 
...
<td>'. ' '.'</td>
<td>'.$_row['Spouse_Fee'].'</td>
<td>'.$_row['Spouse_VAT'].'</td>
<td>'.$Spouse_Total.'</td>
<td>'.$cnel.'</td>
</tr>
';
...
$htmtab1_body .= '
    ...
    <tr style="background-color: #f2f2f2;font-size:10px">
        <td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td colspan="1" style="text-align: center;"><b>Total:</b></td><td></td><td></td><td></td>
        
        <td>'.$totalAmount.'</td>
        <td>'.$totalVAT.'</td>
        <td>'.$grossTotal.'</td><td></td><td></td>
    </tr>
...
// $LessCancelAMT=$TotalAmoutCount-$LessCancelationAmt+3480;
if($Spouse_Total){
    $LessCancelAMT=$TotalAmoutCount-$LessCancelationAmt+$Spouse_Total;
}else{
...
// $TotalAmoutCount_final =$TotalAmoutCount + 3480; 
if($Spouse_Total!=0){
    $TotalAmoutCount_final =$TotalAmoutCount + $Spouse_Total; 
} else {
    $TotalAmoutCount_final =$TotalAmoutCount; 
}
...
<tr>
<th>Spouse</th><td>'.$useBelowSpouse.'</td><td>'.$Spouse_Total.'</td>
</tr>
<tr>
...
<tr style="background-color:gray">
<th>Total</th><td>'.$TotalComplementory.'</td><td>'.$TotalAmoutCount_final.'</td>
</tr>
...
if($From=="" && $To==""){
 ...
 // echo "Select Count(MembershipDetails_Level) as level from Members where MembershipDetails_Level='1' and date('entryDate') between '".$FromDaat."' and '".$Todaat."'  ";
  ...
  $QcountIvory=mysqli_query($conn,"Select Count(MembershipDetails_Level) as level from Members where MembershipDetails_Level='3' and (date(entryDate) between '".$FromDaat."' and '".$Todaat."' or date(CancelationDate) between '".$FromDaat."' and '".$Todaat."') ");
  $QfetchIvory=mysqli_fetch_array($QcountIvory);

  $TotalLevelQuery=$QfetchFirst['0']+$QfetchGold['0']+$QfetchPlatinum['0']+$QfetchIvory['0'];

$totallevel = $QfetchGold['0']+$QfetchPlatinum['0']+$QfetchIvory['0'];
