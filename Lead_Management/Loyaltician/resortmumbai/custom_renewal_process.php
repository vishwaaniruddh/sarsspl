...
include('config.php');
include ('renewal_welcome_template.php');
...
$sql_level_id = mysqli_query($conn,"SELECT * FROM `Level` where Leval_id='".$member_type."' ");
$sql4fetch = mysqli_fetch_array($sql_level_id);
...
if($_REQUEST['CertificatesCheck1']== 'on'){
    $CertificatesCheck1 = $_REQUEST['CertificatesCheck1'];
}

if($_REQUEST['PromotionalCheck1']== 'on'){
...
if($CertificatesCheck1=='on'){
    // Renewal
    echo 'renewal';
        $promotionalsql="SELECT fromSerialNo,toSerialNo,issued_voucher_code,voucher_code FROM `voucher_issued_additional` where 
        Program_ID='1' and voucher_code='51'";
      	$promotionalsql = mysqli_query($conn,$promotionalsql);
      	$promotionalsql_result=mysqli_fetch_assoc($promotionalsql);
      	
      	$issued_voucher_code = $promotionalsql_result['issued_voucher_code'];
      	$newissued_voucher_code = $issued_voucher_code+1 ; 
      	mysqli_query($conn,"update voucher_issued_additional set issued_voucher_code='".$newissued_voucher_code."' where Program_ID='1' and voucher_code='51'");
      	mysqli_query($conn,"update Members set renewal_voucher_code='".$newissued_voucher_code."',RenewalCheck1=1 where Static_LeadID='".$id."'");
...
if($PromotionalCheck1=='on'){
    // Promotional
        echo 'promotional';
            $promotionalsql="SELECT fromSerialNo,toSerialNo,issued_voucher_code,voucher_code FROM `voucher_issued_additional` where 
            Program_ID='1' and voucher_code='52'";
            ...
            $promotionalsql = mysqli_query($conn,$promotionalsql);
      	    ...
      	    $issued_voucher_code = $promotionalsql_result['issued_voucher_code'];
            $newissued_voucher_code = $issued_voucher_code+1 ; 
            mysqli_query($conn,"update voucher_issued_additional set issued_voucher_code='".$newissued_voucher_code."' where Program_ID='1' and voucher_code='52'");
...
$Primary_Gmail_1 = $booklet_sql_result['Primary_Email_ID2'];

$Primary_nameOnTheCard = $booklet_sql_result['Primary_nameOnTheCard'];
...
unset($value);
    ...
    if ($sql4fetch['Leval_id'] == '1') {
        classic($Primary_Gmail_1, $Primary_nameOnTheCard, $new_member_id, $next_expiry_date, $MembershipDts_PaymentMode, 1, $Static_LeadID, $AdditionalRenewalAssignBooklet, $MembershipDts_NetPayment, $MembershipDts_GrossTotal, $newbookletno);
    
    } else if ($sql4fetch['Leval_id'] == '2') {
        signature($Primary_Gmail_1, $Primary_nameOnTheCard, $new_member_id, $next_expiry_date, $MembershipDts_PaymentMode, 2, $Static_LeadID, $AdditionalRenewalAssignBooklet, $MembershipDts_NetPayment, $MembershipDts_GrossTotal, $newbookletno);   
    
    } else if ($sql4fetch['Leval_id'] == '3') {
        ivory($Primary_Gmail_1, $Primary_nameOnTheCard, $new_member_id, $next_expiry_date, $MembershipDts_PaymentMode, 3, $Static_LeadID, $AdditionalRenewalAssignBooklet, $MembershipDts_GrossTotal, $newbookletno);
