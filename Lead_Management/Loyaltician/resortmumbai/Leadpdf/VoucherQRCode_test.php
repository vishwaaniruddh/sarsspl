...
for ($kk = 0; $kk < count($exp); $kk++) {
    ...
    // echo $kk; die;
     ...
     if (($kk + 1) % 50 == 1) {
        $pdf->AddPage();
        $pdf->setMargins(5, 10, 5, 10); // Set initial margins for a new page
        $pdf->setCellPaddings(1, 1, 1, 1);
        $pdf->setCellMargins(1, 1, 1, 1);
        $pdf->Ln(1);
    } else if (($kk + 1) % 50 == 2) {
...
// die;
    ...
    while ($barcode_sql_result = mysqli_fetch_assoc($barcode_sql)) {
        ...
        if ($srno % 5 == 0) {
         ...
         // Adjusting margin for the 5th row
        $tdStyle = '';
        if ($srno % 5 == 4) {
            $tdStyle = 'style="padding-left: 30px;"'; // Adjust the padding value as needed
        }
        
        $htmtab1_body .= '<td ' . $tdStyle . ' cellpadding="0" cellspacing="0">
    ...
    // if ($RenewalCheck1 == "1") {
    //     for ($r = 0; $r < count($renewalvoucher); $r++) {
    //         $NoOfVoucher = $renewalvoucher[$r];
            
    //         // echo $NoOfVoucher; die;
