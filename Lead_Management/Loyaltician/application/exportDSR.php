...
$conntents .= "Sr. No. \t Name on the Card \t Type \t Level \t Membership No.\t Expiry Date \t Booklet Number \t Additional Voucher \t Type(N/R) \t Payment Mode \t Instrument No \t Authorisation \t Receipt No  \t  Amount  \t GST \t Total Amount \t Remarks \t";
...
function getNew_booklet($member_id)
{
    ...
    $sql_result = mysqli_fetch_assoc($sql);
    $booklet = $sql_result['new_booklet_series'];

    if ($booklet) {
        return $booklet;
    } else {
        $sql = mysqli_query($conn, "select * from Members where Static_LeadID = '" . $member_id . "'");
        $sql_result = mysqli_fetch_assoc($sql);
        return $sql_result['booklet_Series'];
    }
}
...
while ($_row = mysqli_fetch_array($table)) {
    ...
    if (is_null($_row['promotional_voucher_code'])) {
        $pvc = " - ";

    } else {

        $promo_voucher = json_decode($_row['promotional_voucher_code']);
        $pvc = implode(" ", $promo_voucher);
    }
    ...
    $conntents .= "\n" . $cnt . "\t";
    $conntents .= $_row['Primary_nameOnTheCard'] . "\t";
    $conntents .= '' . "\t";
    $conntents .= $sql3fetch['level_name'] . "\t";
    $conntents .= $_row['GenerateMember_Id'] . "\t";
    ...
    if ($booklet) {
        ...
        $conntents .= $_row['booklet_Series'] . "\t";
    }

    $conntents .= $pvc . "\t";

    $conntents .= $type . "\t";
    $conntents .= $_row['MembershipDts_PaymentMode'] . "\t";
    $conntents .= $_row['MembershipDts_InstrumentNumber'] . "\t";
    $conntents .= '' . "\t";
    $conntents .= '' . "\t";

    $conntents .= $_row['MembershipDts_NetPayment'] . "\t";
    $conntents .= $_row['MembershipDts_GST'] . "\t";
    $conntents .= $_row['MembershipDts_GrossTotal'] . "\t";
    $conntents .= $_row['MemshipDts_Remarks'] . "\t";

}

$conntents = strip_tags($conntents);
