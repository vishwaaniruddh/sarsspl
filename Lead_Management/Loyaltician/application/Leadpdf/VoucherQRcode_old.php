...
// '<table  border="1" width="90%"   style="padding:8px;" >';
...
$getLevel= substr($Voucher, 1, 1);
 ...
 if($_rowMemb['MembershipNumber']!=""){
     $Memb_details=mysqli_query($conn,"Select GenerateMember_Id,Primary_nameOnTheCard,entryDate,ExpiryDate from Members where GenerateMember_Id='".$_rowMemb['MembershipNumber']."' ");
...
//while($_row=mysqli_fetch_array($sql)){
 ...
 $L=4;
   ...
   //$htmtab1_body.='<td style="width:25%;height:5px;"  ><img src="'.$ccc.'" style="float:left;width:350%" /><span style="font-size:10px;float:right;margin-top:-20px">'.$detailsMemb['Primary_nameOnTheCard'].''.$detailsMemb['GenerateMember_Id'].''.$codeContents.'</span></td> ';
 //  $htmtab1_body.='<td   ><div ><span style="float:left;"><img src="'.$ccc.'" width="50px"/></span><span  style="font-size:10px;float:right;margin-top:-50px">'." ".''.$detailsMemb['GenerateMember_Id'].'</span></div></td> ';
  // $htmtab1_body.='<td valign="top"><div><span class="inline"><img src="'.$ccc.'" alt="Smiley face" width="42" height="42" style="float:left;"/></span><span style="line-height:-80%;">Im follow</span></div></td> ';
