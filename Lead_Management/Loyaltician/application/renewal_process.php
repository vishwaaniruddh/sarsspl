...
<table id="example" class="table  " style="width:99%">
  ...
  <tbody>
    ...
    while ($sourceF = mysqli_fetch_array($sourceQ)) {
      ...
      ?>
      <tr>
        <td><?php echo $sourceF['Primary_nameOnTheCard']; ?></td>
        ...
        <?php if ($_SESSION['usertype'] == 'Admin') {
          if ($n <= '0') { ?>
            <td><input type="checkbox" name="check[]" value="<?php echo $sourceF[5]; ?>"></td>
          <?php } else { ?>
            <td><?php echo $FRenwal2[0]; ?> </td> <?php }
        } ?>
...
<?php if ($_SESSION['usertype'] == 'Admin') { ?>
  <div align="center"> <button id="myButtonControlID" class="btn btn-primary" onClick="delfunc();">Delegate</button></div>
<?php } ?>