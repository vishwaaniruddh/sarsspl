...
for ($kk = 0; $kk < count($exp); $kk++) {
    ...
    if ($_rowMemb['MembershipNumber'] != "") {
        $Memb_details = mysqli_query($conn, "Select GenerateMember_Id,Primary_nameOnTheCard,entryDate,ExpiryDate, PromotionalCheck1,promotional_voucher_code,RenewalCheck1,renewal_voucher_code,MealCheck1,meal_voucher_code from Members where GenerateMember_Id='" . $_rowMemb['MembershipNumber'] . "' ");
        ...
        $meal_voucher_code = $detailsMemb['meal_voucher_code'];
...
$srno++;
    ...
    if ($MealCheck1 == "1") {
        ...
        $barcode_sql = mysqli_query($conn, "select * from BarcodeScan where Voucher_id in($meal_voucher_code)");
        while ($barcode_sql_result = mysqli_fetch_assoc($barcode_sql)) {
            $Voucher_id = $barcode_sql_result['Voucher_id'];
