...
$pdf->Ln(1);
    ...
    if ($_rowMemb['MembershipNumber'] != "") {
        $Memb_details = mysqli_query($conn, "Select GenerateMember_Id,Primary_nameOnTheCard,entryDate,ExpiryDate,PromotionalCheck1,RenewalCheck1,promotional_voucher_code,renewal_voucher_code,MealCheck1,meal_voucher_code from Members where GenerateMember_Id='" . $_rowMemb['MembershipNumber'] . "' ");
        ...
        $renewalvoucher = json_decode($renewal_voucher_code);
        
        $MealCheck1 = $detailsMemb['MealCheck1'];
        // $_meal_voucher_code = $detailsMemb['meal_voucher_code'];
          
        $R = date('F, Y', strtotime($detailsMemb['ExpiryDate']));
    }

// echo $promotional_voucher_code; die;
...
// if ($RenewalCheck1 == "1") {
//         for ($j = 0; $j < count($renewalvoucher); $j++) {
//             $NoOfVoucher = $renewalvoucher[$j];
//             $codeContents = $NoOfVoucher;
//             $fileName = $NoOfVoucher . '.png';
//             $pngAbsoluteFilePath = $tempDir . $fileName;
//             $ccc = $pngAbsoluteFilePath;
...
$srno++;
 ...
 if ($MealCheck1 == "1") {
        // $meal_voucher_code = json_encode($meal_voucher_code);
        // $meal_voucher_code = str_replace(array('[', ']', '"'), '', $meal_voucher_code);
        // $arr = explode(',', $meal_voucher_code);
        // $meal_voucher_code = "'" . implode("', '", $arr) . "'";
        
        $meal_voucher_code = $_meal_voucher_code = $detailsMemb['meal_voucher_code'];

        $barcode_sql = mysqli_query($conn, "select * from BarcodeScan where Voucher_id = '".$meal_voucher_code."' ");
        while ($barcode_sql_result = mysqli_fetch_assoc($barcode_sql)) {
            $Voucher_id = $barcode_sql_result['Voucher_id'];
