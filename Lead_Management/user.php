    ...
    function validation()
{
     ...
     if(UserName=="")
     {
     swal("please fill up User Name");
     return false;
     } 
    else if(Password=="")
     {
     swal("please fill up password");
     return false;
     } 
     else if(UserType=="")
     {
...
<script>
 ...
 if(msg==1){
     ...
     window.open("viewuser.php","_self");
 }else{
     swal("error");
 }
...
">
      ...
      <div><input type="text" name="drop" id="drop"/></div>
      <h4 align="center"><font color="red">Create Employee First!!</font></h4>
        <h1>Add User</h1>

        <fieldset>
         
        
          <label for="state"><b>User Name:</b></label>
          <select class="rounded" name="UserName" id="UserName">
              <option value="">Select User Name</option>
          <?php 
          
          $abc="select * from SalesAssociate where SalesmanId NOT IN(select reg_id from Users)";
          
          $runabc=mysqli_query($conn,$abc);
          while($fetch=mysqli_fetch_array($runabc)){?>
          <option value="<?php echo $fetch['SalesmanId'];?>"><?php echo $fetch['FirstName'].' '.$fetch['LastName']?></option>
         <?php } ?>
          </select>
          ...
          <label for="state"><b>UserType:</b></label>
          <select class="rounded" name="UserType" id="UserType">
