...
class Customers extends CI_Controller
{
    ...
    public function Bill_Entrytwo()
    {
        ...
        $data['Doctors'] = $this->Doctors_M->get_doctors();
        $data['billno'] = $this->Customers_M->get_billno();
        $data['connect'] = "Customers/CreateBillTwo";
        $this->load->view("partials/Dash_connect", $data);
    }
    
    public function Bill_Entrytwo_test()
    {
        $data['title'] = "Bill entry test ";

        $data['Customers'] = $this->Customers_M->get_customers();
        $data['Doctors'] = $this->Doctors_M->get_doctors();
        $data['billno'] = $this->Customers_M->get_billno_test();
        $data['connect'] = "Customers/CreateBillTwo_test";
        $this->load->view("partials/Dash_connect", $data);
    }
    ...
    public function create_billtwo()
    {
        ...
        $refby = $this->input->post('doc_id',true);
        ...
        public function create_billtwo_test()
    {
        $bill_id = $this->input->post('bill_id', true);
        $bill_date = $this->input->post('bill_date', true);
        $supp_id = $this->input->post('cust_id', true);
        ...
        $qrypur = $this->db->insert('Customers_sales_test', $instdata);
        ...
        if ($qrypur) {
            ...
            for ($i = 0; $i < count($myitemid); $i++) {
                ...
                if ($numcount) {
                    ...
                    $this->session->set_flashdata($stflash);
                    redirect(base_url('Sale/BillRetailer_test'));
                ...
                $this->db->insert('Customers_sales_details_test', $datainsert);
            }
        ...
        $this->session->set_flashdata($stflash);
        redirect(base_url('Sale/View_BillRetailer_test'));
    }
    ...
    public function View_BillTwo_test()
    {
        $all_bills = array();
        $all_billsdata = $this->Customers_M->get_all_sales_two();
        ...
        $data['all_bills'] = $all_bills;
        $data['title'] = "Home :Account";
        $data['connect'] = "Customers/View_billTwo_test";
        $this->load->view("partials/Dash_connect", $data);
    }
    ...
    public function View_bill_details($billid)
    {
        ...
        // $data['doctor_name']  = $this->Customer_M->get_docname($bills_data->refby);
    ...
    public function Edit_BillTwo($billid)
    {
        var_dump($_POST); die;
    ...
    public function ViewAllBill_test()
    {
        $data['Customers'] = $this->Customers_M->get_customers();
        $data['Payment_details'] = array();
        $data['title'] = "View All Bill Test";
        $data['connect'] = "Customers/ViewAllBill_test";
        $this->load->view("partials/Dash_connect", $data);
    }
    ...
    public function Getbillsajex_test()
    {
        $cust_id = $this->input->post('cust_id', true);
        $start = $this->input->post('start', true);
        $end = $this->input->post('end', true);
        ...
        $all_bills = array();
        $all_billsdata = $this->Customers_M->GetBillAjex_test($cust_id, $start, $end, $bill_type);

        foreach ($all_billsdata as $key => $bills) {
            $supplier = $this->Customers_M->getall_customers_deatils_test($bills->cust_id);
            $bills->customer_name = $supplier->first_name . " " . $supplier->last_name;
            array_push($all_bills, $bills);
        }
        $data['Bill_deatils'] = $all_bills;
        ...
        $this->load->view("Customers/Getbillsajex_test", $data);
        // echo json_encode($data['Bill_deatils']);
    }
