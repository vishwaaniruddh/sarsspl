...
<center><form name="frm" method="post" action="<?php $_SERVER['PHP_SELF'] ?>">
...
$sql.=" order by contact_first";
$cust=mysqli_query($con,$sql);
...
?>
<table border="1" id="custtable"><tr><th>Request ID</th><th>Atm ID</th><th>Last Paid Date</th><th>Due Date</th><th>Paid Amount</th><th>Consumer Number</th><th>Bill Date</th>
<th>Bank</th><th>Site Type</th></tr>
<?php
...
//echo "select p.Paid_Date,p.Paid_Amount,p.Bill_No,f.bill_date,f.due_date,f.req_no from ebpayment p,ebillfundrequests f where p.Bill_No=f.req_no and f.atmid = '".$atm[$i]."' and f.bill_date ='".$qrro[0]."'<br>";
$duedt=mysqli_query($con,"select p.Paid_Date,p.Paid_Amount,p.Bill_No,f.bill_date,f.due_date,f.req_no,f.cust_id,f.trackerid from ebpayment p,ebillfundrequests f where p.Bill_No=f.req_no and f.atmid = '".$atm[$i]."' and f.bill_date ='".$qrro[0]."'".$dt);
...
//$site=mysqli_query($con,"select bank,site_type from ".$duedtro[6]."_sites where trackerid='".$duedtro[7]."' and bank like 'CORPORATION'");
