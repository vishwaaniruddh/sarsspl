...

{
	...
	echo '<hmysqli_query($con,cessfull!</h1>';
...
mysqli_query($con,
...

	{
	 ...
	 $result= mysqli_query($con,"INSERT INTO  atm(atm_id,bank_name,area,pincode,city,state,address,ref_id,po,cust_id,servicetype,podate,state1)  VALUES('$ab[1]','$ab[2]','$ab[3]','$ab[4]','$ab[5]','$ab[6]','$ab[7]','$ab[8]','$po','$cust','".$service."','".$dt2."','$ab[6]')");
	...
	$astdata=mysqli_query($con,"select * from assets");
...
mysqli_query($con,
	...
	$qr=mysqli_query($con,"select ass_spc_id from assets_specification where assets_id='".$astrow[0]."' and name LIKE '".$ast[$b]."%'");
	...
	/*$qry2=mysqli_query($con,"select * from site_assets where cust_id='".$cust."' and po='".$po."' and assets_spec='".$astspecid[$b]."'");
	...
	$insert=mysqli_query($con,"insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid) values('$cust','$po','".$astrow[1]."','".$astspecid[$b]."','".$astwarr[$b]."','".$astcnt[$b]."','".$atmid."')");
...
//	echo $ab[1]." ".date('Y-m-d', strtotime($ab[9]))." ".$x."<br>";
...

	{
	 ...
	 $result= mysqli_query($con,"INSERT INTO  Amc(po,cid,atmid,bankname,area,pincode,city,state,address,Ref_id,servicetype,state1) VALUES('$po2','$cust','$ab[1]','$ab[2]','$ab[3]','$ab[4]','$ab[5]','$ab[6]','$ab[7]','$ab[8]','".$service."','$ab[6]')");
...
//echo $ab[9]." ";
	...
	//echo "<br>INSERT INTO `satyavan_accounts`.`amcpurchaseorder` (`id`, `cid`, `po`, `startdt`, `expdt`,`amcsiteid`) VALUES (NULL, '".$cust."', '".$po2."', '".$start."','".date('Y-m-d', strtotime("+12 months $start"))."','".$id."')<br>";
	 ...
	 $qry=mysqli_query($con,"INSERT INTO `satyavan_accounts`.`amcpurchaseorder` (`id`, `cid`, `po`, `startdt`, `expdt`,`amcsiteid`) VALUES (NULL, '".$cust."', '".$po2."', '".$start."','".date('Y-m-d', strtotime("+12 months $start"))."','".$id."')");
...

{
	...
	$q=mysqli_query($con,"Insert into servicemonth(`po`,`date`,`type`,`siteid`) Values('".$po2."','".date("Y-m-d",strtotime("+".$j." months", $today))."','AMC','".$id."')");
...

{
 ...
 $cnt=10;
	...
	$insert=mysqli_query($con,"insert into `amcassets`(amcid,amcpoid,assets_name,assetspecid,quantity,siteid) values('$cust','$po2','".$astrow[1]."','".$astspecid[$b]."','".$astcnt[$b]."','".$id."')");
