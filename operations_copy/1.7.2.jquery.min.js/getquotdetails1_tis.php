...
//echo $custs;
        ...
        $qry="select * from quotation1_tis where status!='c'";
...
$Num_Rows = mysqli_num_rows ($table);
	...
	$getcalldetcntt="select count(id) from quotation1_tis where status!='c' and call_status='0' ";
        
        

        $getcalldetcntt1="select count(id) from quotation1_tis where status!='c' and call_status='1' ";

         
       $totcalldets="select count(id) from quotation1_tis where status!='c' ";
...
$qry.=" ORDER BY cust ASC ";
	...
	while($row=mysqli_fetch_array($qrys))
	{
	  ...
	  $trdet=mysqli_query($con,"select tamount,pdate,chqno from quotation1ftransfers_tis where qid='".$row[0]."'");
...
?>
	  ...
	  <td  align="center" width="150">
	   ...
	   <?php 
	      ...
	      {
	      $amtqry=mysqli_query($con,"select app_amt,filename,app_by,approved_date,req_amt,remark from quotation_approve_details_tis where qid='".$row[0]."'");
...
<?php echo $rowamt[5]; ?>
 ...
 ?>
 <a href='quotuploads_tis/approve/<?php echo $rowamt[1]; ?>' download>Download</a>
 <?php
 }
 ?>
...
<input type="button" name="closecalldiv" id="closecalldiv" value="Cancel"  onclick="closefuncdivhide('clsfdiv<?php echo $srno?>');"/></td>
     ...
     ?>
 <a href='quotuploads_tis/close/<?php echo $ghrw[7]; ?>' download>Download</a>
 <?php
