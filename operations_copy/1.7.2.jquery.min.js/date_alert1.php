...
$table=mysqli_query($con,"Select * from alert where entry_date BETWEEN STR_TO_DATE('".$adate1."','%d/%m/%Y') AND STR_TO_DATE('".$adate2."%','%d/%m/%Y')");
...
<th wimysqli_query($con,</th>

<th wmysqli_query($con,ank</th>
...
$atm=mysqli_query($con,"select atmid from Amc where amcid='".$row[2]."'");

if($row[17]=='new')

$atm=mysqli_query($con,"select atm_id from atm where track_id='".$row[2]."'");



$atmrow=mysqli_fetch_row($atm);

$qry3=mysqli_query($con,"select cust_name from customer where cust_id='".$row[1]."'");
