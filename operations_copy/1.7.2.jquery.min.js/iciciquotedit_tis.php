...
if(conf==true)
{
  ...
  $.ajax({
            type: "POST",
            url: "process_addicici_det_tis.php",
            data: {matgarr:matgarr,servnoarr:servnoarr,matextarr:matextarr,gpricearr:gpricearr,qntyarr:qntyarr,unitarr:unitarr,amtarr:amtarr,remarr:remarr,qid:quotid},
             beforeSend: function()
                ...
                window.open('iciciquotedit_tis.php?qid='+quotid,'_self');
...
window.open('iciciquotedit_tis.php?qid='+qid,'_self');
            }           
       });
...
if(conf==true)
{
  ...
  $.ajax({
            type: "POST",
            url: "process_icicisolid_editt_tis.php",
...
$getsum=mysqli_query($con,"select sum(amt) from icici_quot_details_tis where qid='".$qid."'");
$srow=mysqli_fetch_array($getsum);

$getqry=mysqli_query($con,"select * from icici_quot_details_tis where qid='".$qid."'");
...
<input type="button" name="bck" id="bck" value="Back" onclick="window.open('icici_quot_view_tis.php?qoid=<?php echo $qoid;?>'+'&strdt=<?php echo $sts;?>'+'&endt=<?php echo $ends;?>'+'&oatm=<?php echo $atms;?>','_self');"
/>
</center>
