...
<?php
	...
	$rev_str="SELECT * FROM `transfer` where to_accid='".$fundacc['aid']."' ";
	...
	if(isset($_REQUEST['sup']) && $_REQUEST['sup']!="")
		$rev_str.="and from_accid='".$_REQUEST['sup']."' ";
...
if($showpaging)
{
	...
	while($rev=mysqli_fetch_array($rev_qry))
	{
		...
		{
			$fundacc_qry=mysqli_query($con,"SELECT hname FROM `fundaccounts` WHERE `aid` = '".$rev['from_accid']."'");
			$fundacc=mysqli_fetch_array($fundacc_qry);
...
?>
	<tr>
        ...
        <td width="150px">
            ...
            <a href="javascript:void(0);" id='feefbk<?php echo $count; ?>' onclick="newwin('viewefeed_transfer.php?reqid=<?php echo $ebfundreq['req_no']; ?>','display','500',400)">Click to view all Feedbacks</a>
		</td>
        <?php
        	if($_REQUEST['status']==2)
			{
        ...
        <td id="app<?php echo $i; ?>">        	
            ...
            <div id="showrem<?php echo $count; ?>" style="display:none">
                ...
                <input type="button" onclick="showrem('showrem<?php echo $count; ?>')" value="Cancel" style="background:#FFFF99" />
            </div>
        </td>
        <?php
			}
        ...
        <?php
			...
			{
		?>
        <td><form method="post" name="ebform" action="process_ebfundtrans_transfer_app.php"><input type="text" name="reqid" value="<?php echo $ebfundreq['req_no']; ?>"/><input type="submit" value="Approve" name="sub"/></form></td>
        <td><form method="post" name="ebform" action="process_ebfundtrans_transfer_rej.php"><input type="text" name="reqid" value="<?php echo $ebfundreq['req_no']; ?>"/><input type="submit" value="Reject" name="sub"/></form></td>
        <?php
			}
		?>
    </tr>
