...
return true;
	...
	<form name="frm1" method="post" action="processrnmsite.php" enctype="multipart/form-data">
	  <h2 class="style1">New RNM Site Entry</h2>
...
//echo $bb;
			   ...
			   <select name="bank" id="bank" >  <option value="" >Select Bank</option>
				...
				<?php
				$bkk="select bank_name from bank order by bank_name ASC";
			   //echo $pojct;
			  $bkkr=mysqli_query($con,$bkk);
 while($bkkro=mysqli_fetch_array($bkkr))
			  {
			  ?>
			  <option value="<?php echo $bkkro[0];  ?>"><?php echo $bkkro[0];  ?></option>
			  <?php
			  }
				/*for($i=0;$i<count($bank);$i++)
				{ ?>
			...
			<!--   <tr><td>City category:</td><td>
			   ...
			   <tr><td>
				 ...
				 <center>ATM ID: <input type="text" maxlength="50" size="12" name="atmid" id="atmid">&nbsp;<br>    ATM ID2: <input type="text"maxlength="50" size="12" name="atmid2">&nbsp;<br></td></center><td>
				 Local Branch: <input type="text" name="localbranch">&nbsp;</td><td width="252">
				&nbsp;Site ID: 
				<input type="text" name="siteid"></td><td width="260">
		   ...
		   $resultcity = mysqli_query($con,$sqlcity );
	...
	
			{
			   ...
			   <?php
				...
				<td></td>
				</tr><tr><td>Request Date: </td><td> <input type="text" name="takeoverdate" id="takeoverdate"  onclick="displayDatePicker('takeoverdate')" /></td><td></td><td> </td></tr>
				
				
				
					   <tr><td colspan="4">Choose File : <input type="file" name="userfile" id="userfile" /></td></tr>
							   <tr><td colspan="4" align="center"><input type="submit" value="submit" name="submit" onclick="return validateform();" /></td></tr>
				 </table>
