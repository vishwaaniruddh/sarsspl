<?php
	...
	$ins=mysqli_query($con,"INSERT INTO `transfer`(`reqid`, `from_accid`,`to_accid`, `payment_type`, `pdate`,`pamount`, `chqname`, `chqno`, `remark`, `entrydate`, `reqby`) VALUES ('".$_REQUEST['reqid']."','".$fundacc['aid']."','".$_REQUEST['to_accid']."','".$_REQUEST['paytype']."','".date('Y-m-d',strtotime(str_replace('/','-',$_REQUEST['pdate'])))."','".$_REQUEST['pamount']."','".$_REQUEST['bank']."','".$_REQUEST['chqno']."','".$_REQUEST['remark']."','".date('Y-m-d H:m:s')."','".$srno[0]."')");
	if($ins)
	{
		$_SESSION['success']=1;
	}
