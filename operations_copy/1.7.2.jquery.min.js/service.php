...
var v;
v=str[0];
document.getElementById("cust").value=str[0];
//alert(v);
...
*/
document.getElementById('ref').value=str[2];
...
document.getElementById("state").value=str[4];
  ...
  xmlHttp.open("GET", "get_ref3.php?ref="+id+"&type="+type+"&cid="+cid, true);
alert("get_ref3.php?ref="+id+"&type="+type+"&cid="+cid);
  xmlHttp.send(null);
...
function searchval(what,val,call,cust)
{
	//alert(what+" "+val);
...
var cl=document.getElementById(cust).value;
alert("servicesearch.php?val="+txt+"&what="+what+"&calltype="+call+"&cust="+cl);
xmlhttp.open("get","servicesearch.php?val="+txt+"&what="+what+"&calltype="+call+"&cust="+cl,false);
...
<body <?php if(isset($_GET['id'])){ ?>  onload="GetRef('<?php echo $_GET['id'] ?>','<?php echo $_GET['type'] ?>','<?php echo $_GET['cid']; ?>');assets('<?php echo $_GET['id'] ?>','<?php echo $_GET['type'] ?>')" <?php } ?>>
<center>
...
?>
<option value="<?php echo $row[1]; ?>" <?php  if(($_POST['cust'])==$row[1]){ ?>
selected="selected" <?php }  ?>><?php echo $row[1]; ?></option>


<?php } ?>
</select>-->
<input type="text" name="cust" id="cust" value="<?php if(isset($_POST['cust'])){ echo $_POST['cust']; } ?>" readonly="readonly" />
...
<input type="text" name="po" id="po_no" value="<?php if(isset($_POST['po'])){ echo $_POST['po']; } ?>" readonly="readonly" />
</td></tr>
...
<!--<select name="ref" id="ref" onchange="assets(this.value,'amc');atmid(this.id,'amc')">
<?php if(isset($_POST['ref'])){ 
$today=new DateTime(date("Y-m-d"));
$pcb='';
$qry=mysqli_query($con,"select * from Amc where amcid='".$_POST['ref']."'");
$roqry=mysqli_fetch_row($qry);
?>
<option value="<?php  echo $_POST['ref']; ?>" selected="selected"><?php  echo $roqry[3]; ?></option>
<?php } ?>
</select>-->
<input type="text" name="ref" id="ref" value="<?php if(isset($_POST['ref'])){ echo $_POST['ref']; } ?>" readonly="readonly" />
</td></tr>
<tr><td>Service Type</td><td colspan="3" id="reference">
<?php
...
{
?>
<option value="<?php  echo $serrow[0]; ?>" <?php if(($_POST['servicetp'])==$serrow[0]){ ?> selected="selected"<?php } ?>><?php  echo $serrow[1]; ?></option>
<?php
}
?>
</select></td></tr>
...
$res=mysqli_query($con,"SELECT * FROM amcassets where siteid='".$_POST['ref']."'");
...
$qry=mysqli_query($con,"select * from amcpurchaseorder where amcsiteid='".$_POST['ref']."'");
...
{
//echo "hi";
$qry=mysqli_query($con,"select * from installed_sites where ref_id='".$r[0]."'");	
...
?>
<tr><td><?php echo $cnt+1; ?></td><td><input type="checkbox" name="assets[]" id="assets[]" onClick="astselect('assets<?php echo $cnt ?>');"   />

<input type="hidden" name="assid[]" value="<?php echo $row[0]; ?>" />
<?php echo $row2[1]." (".$ro[2].")"; ?></td><td><?php if($expdt>=$today) { echo "UW<input type='hidden' name='pcb[]' value='' id='pcb[]'>"; } else {
if($pcb!='pcb')
 $pcb='pcb'; 
...
<input type="text" name="cnt" value="<?php echo $cnt; ?>" id="cnt" />
<?php
}
...
<tr>
<td height="35" colspan="4"><input type="submit" value="submit" class="readbutton" /></td>
</tr>
...
<table border="0">
<tr><td><?php //echo $_SESSION['custid'];
$cl=str_replace(",","','",$_SESSION['custid']);
 $cl="'".$cl."'";
 ?>
...
$strr="select short_name,contact_first from contacts where 1";

if($_SESSION['custid']!='All' && $_SESSION['custid']!='')
$strr.=" and short_name in ($cl)";

$strr.=" order by short_name ASC";
//echo $strr;
$clr=mysqli_query($con,$strr);
while($clro=mysqli_fetch_array($clr)){
?>
...
<tr><td><input type="text" name="find" id="find" style="padding-top:3px"></td></tr>
<tr><td>
<input type="button" name="searchbtn" id="searchbtn" value="Search" onclick="searchval('searchby','find','service','client');" />
</td></tr>
<tr style="display:block"><td><div style=" width:208px; height:770px; overflow:auto;" id="searchbar"></div></td></tr></table>
</td></tr></table></div>
