...
//$table=mysqli_query($con,"select * from alert where state='$br1[$i]'");
...
elsemysqli_query($con,

$sql=mysqli_query($con,om transfersites where tobranch in (".$br2.")";
...
$qr=mysqli_query($con,"select * from alert where alert_id='".$row[3]."'");
...
$atm=mysqli_query($con,"select atmid from Amc where amcid='".$ro[2]."'");

elseif($ro[17]=='new')

$atm=mysqli_query($con,"select atm_id from atm where track_id='".$ro[2]."'");

elseif($ro[17]=='new temp')

$atm=mysqli_query($con,"select atmid from tempsites where atmid='".$ro[2]."'");

//echo "select atm_id from atm where track_id='".$ro[2]."'";

$atmrow=mysqli_fetch_row($atm);

	$qry=mysqli_query($con,"select cust_name from customer where cust_id='".$ro[1]."'");
...
<td width="125" valign="top">&nbsp;<?php // echo $row[17]." ".$row[2];
  ...
  if($ro[17]=='new'){ echo $ro[2]; } else {   echo $atmrow[0];  }?></td>

<td width="75" valign="top">&nbsp;<?php echo $ro[3] ?></td>

<td width="75" valign="top">&nbsp;<?php echo $ro[7] ?></td>
