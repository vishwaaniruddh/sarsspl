...
if($_POST) {	
  ...
  $gstn 				= $_POST['gstn'];
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {		
		//  product table
		$updateProductQuantitySql = "SELECT product.quantity FROM stocklink_product product WHERE product.product_id = ".$_POST['productName'][$x]."";
		$updateProductQuantityData = $connect->query($updateProductQuantitySql);			
			
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			// order item table add product quantity
			$orderItemTableSql = "SELECT order_item.quantity FROM stocklink_order_item order_item WHERE order_item.order_id = {$orderId}";
			...
			$updateQuantitySql = "UPDATE stocklink_product SET quantity = $editQuantity WHERE product_id = ".$_POST['productName'][$x]."";
		...
		for($x = 0; $x < count($_POST['productName']); $x++) {			
			...
			while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
				$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
					...
					$updateProductTable = "UPDATE stocklink_product SET quantity = '".$updateQuantity[$x]."' WHERE product_id = ".$_POST['productName'][$x]."";
					$connect->query($updateProductTable);

					// add into order_item
				$orderItemSql = "INSERT INTO stocklink_order_item (order_id, product_id, quantity, rate, total, order_item_status) 
