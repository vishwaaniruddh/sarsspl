...
if ($_FILES) {
    $type = pathinfo($_FILES['categoryfile']['name'], PATHINFO_EXTENSION);
    $url = '../assests/images/stock/'.uniqid(rand()).'.'.$type;

    if (in_array($type, ['csv', 'xls', 'xlsx'])) {
        if (is_uploaded_file($_FILES['categoryfile']['tmp_name'])) {
            if (move_uploaded_file($_FILES['categoryfile']['tmp_name'], $url)) {
                ...
                if (!empty($no_error_data)) {
                    foreach ($no_error_data as $value) {
                        $select_sql = "SELECT * FROM stocklink_categories WHERE categories_name ='$value[0]'";
                        $result = $connect->query($select_sql);
                        if ($result->num_rows == 0) {
                            $sql = "INSERT INTO stocklink_categories (categories_name, categories_active, categories_status) VALUES ('$value[0]', '$value[1]', '$value[1]')";
